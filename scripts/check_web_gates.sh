...
#    node の wasm はブラウザの wasm ではない（C-055 の「似た形のベンチは根拠にならない」）。
#    このスクリプトが答えるのは「wasm にした C コアが参照実装と同じ数を出すか」だけで、
#    **出音・レイテンシ・モバイルはこのスクリプトでは未検証**。
#    ⚠️ **ブラウザ自体は別途測ってある** — M-95（Chrome 152。PCM は node と bit 一致 /
#    短文 0.008〜0.019 ×RT）と M-96（聴取。両レーンとも「問題なかった」/ 途切れ無し。
#    ⚠️ 1 名・対照なし・盲検なし）。**ただしそれはゲートではない**（人と手作業が要る）。
#
...
/* --- 長さの上限に当てる 3 本 --------------------------------------------------
 * ⚠️ **かな経路では 350 ids に届かない**（512 B の行上限で最大 343 ids。実測）ので、
 *    ids 上限の拒否は**辞書経路でしか踏めない**。
 *
 * ⚠️ **上限は 2 つあり、先に当たった方しか測れない**（ここを 6 日間間違えていた。C-072）:
 *      (1) 形態素 44   … D-052 / SAAN_KANJI_MAX_INPUT_TOK。**NJD に入る前**に落とす
 *      (2) ids 350     … SAAN_MAX_IDS。G2P の**後**に saan_web.c / main.c が落とす
 *    かつてここは `'今日は良い天気ですね。'.repeat(9)` 1 本だけで (2) を測っていたが、
 *    2026-09-10 に (1) が入ってからは **(1) が先に発火して (2) を 1 度も測っていなかった**。
 *    この文は 7 形態素で 50 ids しか作らないので、**何回繰り返しても (2) には届かない**
 *    （×6 = 42 形態素 308 ids で通り、×7 = 49 形態素で (1) に当たる。実測）。
 *
 * カタカナ語は 1 形態素あたりの mora が多いので ids/形態素 が大きく、(1) に当たらずに
 * (2) へ届く。実測（438,750 entries / 手元）:
 *      ×8 →  8 形態素 325 ids  通る
 *      ×9 →  9 形態素 365 ids  (2) で拒否
 * ⚠️ **ids の値は辞書で変わる**（44,000 entries だと 343 / 387 と測れた）ので、
 *    **値そのものは合否にしない。** 見るのは「どちらの上限で拒まれたか」。 */
const IDS_OVER  = 'コンピューターグラフィックス'.repeat(9);   /* 9 形態素 / 365 ids */
const IDS_UNDER = 'コンピューターグラフィックス'.repeat(8);   /* 8 形態素 / 325 ids */
const TOK_OVER  = '今日は良い天気ですね。'.repeat(9);          /* 63 形態素 */
/* ids が `^ _ $` の 3 個だけ = 入力が丸ごと落ちた（saan_web.c の SAAN_WEB_IDS_FRAME_ONLY）。
 * 「通った」の下限に使う — 3 ids で通るのは成功ではない。 */
const IDS_FRAME_ONLY = 3;
...
for (const [name, file, want] of [['W8A32', 'saan_web_w8a32', 0], ['W8A8', 'saan_web_w8a8', 1]]) {
  ...
  if (haveDict) {
    ...
    /* --- (2) ids 上限 350 の拒否 ---------------------------------------
     * ⚠️ **「拒否された」だけでは足りない。** どちらの上限で拒まれたかを見る。
     *    かつての判定は `rc < 0 && msg に 'ids が'` の 1 本で、外れたときに
     *    **「ids 350 超えが通った」と表示していた**（rc = -11 で拒否されているのに）。
     *    嘘の診断を読んで原因を探すことになるので、3 つに分ける。 */
    const over = M.__synth(IDS_OVER);
    if (over.rc < 0 && over.msg.indexOf('ids が') >= 0)
      ok(`${name}: ids 上限 350 で拒否（${over.msg.slice(0, 26)}…）`);
    else if (over.rc < 0)
      ng(`${name}: **拒否はされたが ids 上限ではない** rc=${over.rc} msg=${over.msg}`
         + ' — 先に別の上限（形態素 44 = D-052）に当たっている。**ids 上限は測れていない**');
    else
      ng(`${name}: ids 上限を超えた入力が通った rc=${over.rc} ids=${over.ids}`);

    /* ⚠️ **拒否だけ並べると「全部拒む」実装で満点になる。** 1 語ぶん短いものが
     *    **通る**ことを要求して、上の拒否が「長いから」であって
     *    「この入力が嫌われているから」でないと言う。
     *    これは同時に **IDS_OVER が形態素上限に当たっていない**ことの証拠でもある
     *    （当たっているなら、1 語短いこれも当たるはず）。 */
    const under = M.__synth(IDS_UNDER);
    if (under.rc === 0 && under.ids > IDS_FRAME_ONLY && under.ids <= 350)
      ok(`${name}: 1 語短いと通る（${under.ids} ids ≤ 350）— 上の拒否は長さが理由`);
    else
      ng(`${name}: ids 上限の 1 つ手前が通らない rc=${under.rc} ids=${under.ids} ${under.msg}`
         + ' — この入力では ids 上限の境目を踏めていない');

    /* --- (1) 形態素上限 44（D-052 / M-98）の拒否 -------------------------
     * ⚠️ **ここにゲートが 1 本も無かった。** PSRAM 無しの板で Open JTalk の
     *    一時ヒープを縛るための上限なのに、「拒否されること」を誰も測っていなかった。 */
    const tok = M.__synth(TOK_OVER);
    if (tok.rc < 0 && tok.msg.indexOf('形態素') >= 0)
      ok(`${name}: 形態素上限 44 で拒否（D-052。${tok.msg.slice(0, 26)}…）`);
    else
      ng(`${name}: 63 形態素の入力が形態素上限で拒まれない rc=${tok.rc} msg=${tok.msg}`);
  }
}
...
if (!haveDict) {
  note('⚠️ **辞書が無いので「漢字 == かな」と長さの上限 2 本は回せなかった。**');
  note('   これは skip ではない: csrc/k1_dict.bin（13,702,320 B）を置けば回る。');
  note('   ⚠️ **リリースに在る。** v0.3.0 以降の `k1-dict-438750.bin` が手元の');
  note('      csrc/k1_dict.bin と SHA-256 で bit 一致する（実測）。pages.yml は');
  note('      前からそこから落としている。**「在るのに落としていない」形だった**（C-072）。');
...
⚠️ **ここで判定していないもの（ゲートにしていない）**:
   1. **ブラウザ**。node の wasm はブラウザの wasm ではない。
      ⚠️ **ここでは見ていないが、別途測ってある**（M-95 Chrome 152 / M-96 聴取）。ゲートではない
   2. **音**。⚠️ 聴取は M-96（1 名・対照なし・盲検なし）。**このスクリプトは音を出さない**
   3. **モバイルと Safari**。1 種類も測っていない
   4. ~~Pages の `Content-Encoding`~~ → M-94 §12 で測った（`.bin` にも gzip が付く）。
      ⚠️ **それでも `.gz` を置く**（配信側の方針に依存させない）
