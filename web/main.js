 ...
 *    Pages が `.bin` に `Content-Encoding: gzip` を付けるかは**未確認**なので、
 *    大きい辞書だけは自前で `.gz` を置いて server に圧縮を頼らない。
 * ───────────────────────────────────────────────────────────────────────── */
const ASSETS = {
  lanes: {
...
const PRESETS = [
  '今日は良い天気ですね。',
  /* 上と同じ文のかな中間表現（= かな経路）。
   * この worktree の wasm(W8A32) を node で走らせて、上の漢字文と PCM が一致することは見た。
   * ⚠️ **ブラウザでは確かめていない。** */
  'きょ][おわよ][いて][んきです°ね',
  '音声合成をマイコンの上で走らせます。',
  '明日の天気は晴れのち曇りでしょう。',
 ...
 *    **このプロジェクトはブラウザを 1 種類も測っていない**（web-demo-plan §8）。
