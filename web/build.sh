...
#
# ⚠️ **ここで出す .wasm のサイズも node の速度も、ブラウザの数字ではない**（C-055）。
#    ブラウザの実測は **M-95**（Chrome 152。PCM は node と bit 一致 / 短文 0.008〜0.019 ×RT）、
#    聴取は **M-96**（両レーンとも「問題なかった」。⚠️ 1 名・対照なし・盲検なし）。
#    ⚠️ **モバイルと Safari は依然として未測定。**
set -eu
