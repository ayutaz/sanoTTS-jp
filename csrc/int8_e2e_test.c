...
/* 判定のしきい値。
 * ⚠️ **実測に合わせた回帰検出用であって、独立な合格基準ではない。**
 * 成果物を v2（Stage 3 = 40k）から **v3（80k）**に差し替えた時点で締め直した:
 *
 *   | 成果物 | 平均 | 最小 | 25 dB 未満 |
 *   |---|---:|---:|---:|
 *   | v2 | 25.88 dB | 23.27 dB | 9 / 24 文 |
 *   | **v3** | **28.11 dB** | **25.72 dB** | **0 / 24 文** |
 *
 * v3 は量子化にも強くなったので、v2 に合わせた 25.0 / 23.0 のままでは
 * **2 dB 以上の劣化を見逃す**。余裕はそれぞれ 1.11 dB / 0.72 dB。
 * ⚠️ **W8A8（`-DSAAN_INT8_ACT=1`）はこの基準を満たさない**（平均 24.24 / 最小 21.94）。
 * ただし **SCOREQ では W8A32 と差が無い**（M-55、陽性対照つき）ので、
 * W8A8 の可否はこの SNR ゲートでは判断しないこと。 */
#define GATE_MEAN_DB 27.0
#define GATE_MIN_DB  25.0
