...
| `csrc` | `line` `fft` `pad` `g2p` `erf`（GELU の erf 近似 vs libm）**`range`**（S9 の範囲版カーネル vs `[0,T)` 版。2026-09-03 追加）**`qeos`**（疑問 EOS 4 種 + U+301C の正規化。**陽性対照つき**。D-062 / M-127。2026-09-11 追加）**`dur`**（MEM-7 = duration net の窓分割。`K` 不変 / ハロー必要十分 / 陽性対照 3 本 / `arena_peak` == 実測 `a.peak` / **G-DUR6 = D-029 の予算 200 KB**（⚠️ `stream_test` の G1 から移した。あちらはコーパスが要って回らない = C-106）。D-067 / M-144。2026-09-18 追加）**`pullptr`**（MEM-8 = `pull_ptr` がコピー版と同じ列を出すか。陽性対照つき。D-068 / M-145。2026-09-18 追加）。**どれも重み blob も辞書も要らない** | cc のみ | **163 s**（⚠️ `qeos` / `dur` / `pullptr` を足した後は未計測） |
...
| **`web`** | **`bash web/build.sh`** + **wasm ゲート 10 本**（G-W7 / **G-W9** / **G-W9b** / G-W1 / G-W2 / **G-W2b** / G-W3 / G-W4 / G-W5 / **G-W6**。`scripts/check_web_gates.sh`。⚠️ **G-W9b は `web/index.html` 本文の arena を見る** — それまで .c だけ見ていたので html の 180,224 B が**3 回の arena 変更を生き延びていた** = M-145）。emcc を **6.0.9 に固定**して入れ、重みは `golden` と同じ `v1.0.0` の資産。⚠️ **辞書 `k1-dict-438750.bin`（13.7 MB）も同じタグから落とす**（C-095。無いと G-W6 の辞書依存 4 件が `if (haveDict)` ごと飛ぶ） | ネットワーク + emsdk | **未計測**（W トラックで追加） |
...
→ 全部まとめて **残タスク 16**（[`../../CLAUDE.md`](../../CLAUDE.md) / [C-083](../../docs/decisions.md#c-083)）。
...
- **漢字経路が合っている**とは言えない。⚠️ 2 つに分かれる:
  - ~~**G-W6 の「漢字 == かな」** … 辞書 13.7 MB が CI に無いので回らない~~
    → ✅ **回るようにした**（[C-095](../../docs/decisions.md#c-095) /
    [M-136](../../docs/measurements.md#m-136)。OK 75 → **79**）。辞書は **v0.3.0 以降
    すべてのリリースに在り**、手元の実体と SHA-256 が bit 一致する。
    ⚠️ **「リリースにも無い」と書いてあったのが誤りで、同じ commit の `pages.yml` が
    同じ資産を落としていた。**
  - **`ids 350` の枝** … 辞書を置いても**まだ覆えていない**（D-052 の形態素上限 44 が
    先に発火する）。⚠️ **ゲートが自分で note を出す**ので、緑でも覆えていないと読める
