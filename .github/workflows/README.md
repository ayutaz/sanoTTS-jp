...
| `csrc` | `line` `fft` `pad` `g2p` `erf`（GELU の erf 近似 vs libm）**`range`**（S9 の範囲版カーネル vs `[0,T)` 版。2026-09-03 追加）**`qeos`**（疑問 EOS 4 種 + U+301C の正規化。**陽性対照つき**。D-062 / M-127。2026-09-11 追加）**`dur`**（MEM-7 = duration net の窓分割。`K` 不変 / ハロー必要十分 / 陽性対照 3 本 / `arena_peak` == 実測 `a.peak`。D-067 / M-144。2026-09-18 追加）**`pullptr`**（MEM-8 = `pull_ptr` がコピー版と同じ列を出すか。陽性対照つき。D-068 / M-145。2026-09-18 追加）。**どれも重み blob も辞書も要らない** | cc のみ | **163 s**（⚠️ `qeos` / `dur` / `pullptr` を足した後は未計測） |
