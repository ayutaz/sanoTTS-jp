...
| **`web`** | **wasm ゲート G-W1〜G-W5**（`scripts/check_web_gates.sh`）。emcc を **6.0.9 に固定**して入れ、重みは `golden` と同じ `v0.3.0` の資産 | ネットワーク + emsdk | **未計測**（W トラックで追加） |
...
- **音が正しい**とは言えない。⚠️ **ブラウザの音は誰も聴いていない**（G32 と同じ空白）
- **漢字経路が合っている**とは言えない。G-W6（K-7 G25）は `kanji_e2e_vectors.bin`（19 MB）が
  git にもリリースにも無いので CI では回らない。**手元でしか回らない**
