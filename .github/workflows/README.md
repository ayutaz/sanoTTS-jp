...
| **`web`** | **`bash web/build.sh`** + **wasm ゲート 8 本**（G-W7 / G-W1 / G-W2 / **G-W2b** / G-W3 / G-W4 / G-W5 / **G-W6**。`scripts/check_web_gates.sh`）。emcc を **6.0.9 に固定**して入れ、重みは `golden` と同じ `v0.3.0` の資産。⚠️ **辞書 `k1-dict-438750.bin`（13.7 MB）も同じタグから落とす**（C-072。無いと G-W6 の辞書依存の 3 検査が `if (haveDict)` ごと飛ぶ） | ネットワーク + emsdk | **未計測**（W トラックで追加） |
...
- **漢字経路が合っている**とは言えない。⚠️ 2 つに分かれる:
  - ~~**G-W6 の「漢字 == かな」** … 辞書 13.7 MB が CI に無いので回らない~~
    → ✅ **回るようにした**（C-072 / M-112）。辞書は **v0.3.0 のリリースに前から在り**、
    手元の `csrc/k1_dict.bin` と SHA-256 が bit 一致する。⚠️ **「リリースにも無い」と
    書いてあったのが誤りで、同じ commit の `pages.yml` が同じ資産を落としていた。**
    ⚠️ **飛ばしていた 6 日間に「ids 350 超えの拒否」が空虚に通っていた**
    （D-052 の形態素上限が先に発火していた）
