...
#if SAAN_KANJI
    /* ⚠️ **`#if SAAN_KANJI` だけでは足りない。** 辞書が開けていない
     *    （焼き損ね / 容量違い / SHA-256 不一致 = D-063）ときに
     *    「漢字をそのまま入力する」と案内すると、**打ってから拒否される**。
     *    案内は**実行時の状態**に合わせる。 */
    if (g_dict_ok) {
        ...
        ESP_LOGW(TAG, "⚠️ **漢字対応ビルドだが、辞書が使えない。**"
                      " 漢字・カタカナ・句読点は喋れない（上のログに理由がある）。");
        ESP_LOGW(TAG, "   受け付けるのは**かな中間表現とひらがなだけ**。");
        ESP_LOGW(TAG, "   辞書を焼き直すか、**その板の容量に合った**辞書を選ぶこと"
                      "（16 MB = k1-dict-438750.bin / 8 MB = k1-dict-228000-8mb.bin ほか）。");
    }
#else
