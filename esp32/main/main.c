...
static void print_usage(void) {
    ...
    /* ⚠️ **文単位の割合は 14.77%%**（44/298）。かつて 15.44%% と書いていたが
     *    誤りで C-058 で訂正した。**judgement には音素単位（0.32%%）を使う** —
     *    文単位だと 1 音素の差も全崩れも同じ 1 件になる（D-044 はそれで決めた）。 */
    ESP_LOGI(TAG, "⚠️ 端末の辞書は枝刈りしてあるので、**ホストと 14.77%% の文で"
