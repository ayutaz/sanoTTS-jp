 ...
 *    **出荷構成（438,750 entries。D-044）では音素の 0.32%%／文の 14.77%%**（M-77）。
 *    **それは既知の代償**であって欠陥ではない。
 *    ⚠️ ここに書いていた **17.79%% は 370,863 entries のときの値**（M-74 / C-050）で、
 *    **今の構成の数ではない**。⚠️ **判断には音素単位（0.32%%）を使う** —
 *    文単位だと 1 音素の差も全崩れも同じ 1 件になる（C-058）。
