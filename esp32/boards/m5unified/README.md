...
| | **v4**（配布中） | v3 |
|---|---:|---:|
| 満チャンク 1 pull の xRT | **0.474**（要件 RTF ≤ 0.5。⚠️ v1.1.0 は 0.448。⚠️ **MEM-7 / MEM-8 の後は実機未測定**） | 0.446 |
| 発話全体（合成合計 / 音声長） | 0.618〜0.793（5 文） | 0.541〜0.712（4 文） |
| アンダーラン | **0** | 0 |
| 初回 pull / 鳴らし始めまで | 245.76 ms / **385 ms** | 244.65 ms / 384 ms |
| arena（静的確保 / 実測 peak） | **139,264 B**（漢字構成。⚠️ **M-144 の値で実機未測定**）/ 実測 peak 110,592 B は arena 151,552 B のときの値（M-142） | 180,224 B / 157,360 B |
