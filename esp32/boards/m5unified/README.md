...
⚠️ **どの重みを焼いたかで値が変わる。** 配布中は **v4**（`v1.0.0`）。

| 重み | 構成 | FNV-1a | sample | \|max\| | Σx² |
|---|---|---|---:|---:|---:|
| **v4**（配布中） | W8A8 + PIE（**既定**。ESP32-S3） | **`0x390bf4b2aef8f2ec`** | 27,136 | 7,583 | 84,292,798,374 |
| **v4** | W8A32（`-DSAAN_ENABLE_PIE=0`。Core2 もこちら） | **`0x9cbe622a4a53af7e`** | **27,648** | 8,023 | 82,400,207,857 |
| v3（`v0.1.0`〜`v0.3.1`） | W8A8 + PIE | `0xa69a7ebbb5ccb05f` | 27,136 | 9,627 | 74,264,237,672 |
| v3 | W8A32 | `0xe4b645c30835d42d` | 27,136 | 9,529 | 74,155,591,505 |

起動時の 1 文（`きょ][おわよ][いて][んきです°ね`）でこの値が出れば、G2P → 合成 → int16 まで
bit 一致している。
⚠️ **v4 では 2 つのレーンで sample 数が違う**（v3 は偶然どちらも 27,136 だった）。
**レーンをまたいで比べない。**
⚠️ **v4 の W8A32 は QEMU でしか測っていない**（[M-132](../../../docs/measurements.md#m-132)。
W8A8+PIE の側は同じ手順で実機値を再現しているので、手順は信用してよい）。
⚠️ **checksum は M-82 以降、同じ重みの中では 1 bit も動いていない** —
S1〜S5b と T1〜T5 は全部「出力を変えない」変更だった。**v3 → v4 の差は重みの差だけ。**
⚠️ 配布イメージ v0.2.0 までの旧コアは `0x04de91103a0e49f9` / `0x78c209af06affc01`（M-62）。
⚠️ **「音が鳴った」は証拠にならない。**

## 実測（CoreS3 / W8A8+PIE / 漢字辞書込み）

**配布中の v4** = [M-130](../../../docs/measurements.md#m-130)
（生ログ [`reports/m130_cores3/device_v4_kanji.log`](../../../reports/m130_cores3/device_v4_kanji.log)）。
**v3** = [M-90](../../../docs/measurements.md#m-90) を並べてある。

| | **v4**（配布中） | v3 |
|---|---:|---:|
| 満チャンク 1 pull の xRT | **0.473**（53 ids。[M-147](../../../docs/measurements.md#m-147) = MEM-7 / MEM-8 の後の実機実測。⚠️ **253 ids からは中央値 0.522** で要件を超える） | 0.446 |
| 発話全体（合成合計 / 音声長） | 0.618〜0.793（5 文） | 0.541〜0.712（4 文） |
| アンダーラン | **0** | 0 |
| 初回 pull / 鳴らし始めまで | 245.76 ms / **385 ms** | 244.65 ms / 384 ms |
| arena（静的確保 / 実測 peak） | **139,264 B**（漢字構成）/ **実測 peak 113,072〜115,056 B**（6 つの長さで `arena_peak(n)` と一致 = [M-147](../../../docs/measurements.md#m-147) の実機実測） | 180,224 B / 157,360 B |
| かな G2P（44 B → 53 ids） | **0.101 ms** | 0.102 ms |
| 漢字 G2P（33 B → 53 ids） | **25.77 ms** | 25.69 ms |
| タスクスタック残り | **9,108** / 16,384 B | 10,996 / 16,384 B |
| flash | app 1.43 MB（factory 2.75 MB）+ 辞書 13.83 MB = 16 MB ちょうど | 同じ |

✅ **「起動直後の内部 DRAM free」は出荷版（`v1.2.0`）で測ってある**: **181,119 B / 最大ブロック 131,072 B**
（[M-147](../../../docs/measurements.md#m-147)。⚠️ `v1.1.0` は 160,639 / 114,688、v3 は 132,039 / 86,016）。
⚠️ **上の表は v4 の初回実機測定（M-124 / M-130）のままで、測定点が違う行がある。**
