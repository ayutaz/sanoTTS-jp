...
| | 何が知りたいか | 実機が要るか |
|---|---|---|
| **1** | ⭐ **音の感想**（読み間違い / 途切れ / 抑揚 / 金属的な尾） | ❌ **要りません**。[Releases](https://github.com/ayutaz/sanoTTS-jp/releases/latest) の `saanotts-jp-v4-samples.zip` を聴くだけ |
...
| **5** | **Arduino / PlatformIO ライブラリが実時間に間に合うか**（[`../arduino/`](../arduino/README.md)。D-065 / M-137）。✅ **PCM は ESP-IDF ビルドと bit 一致している**（QEMU）が、⚠️ **xRT もアンダーランも鳴らし始めまでの時間も未測定** — **同じ PCM が出ることと、間に合って出ることは別**。⚠️ **漢字を実際に喋らせてもいない** | ✅ 要る |
...
✅ **リリースの `saanotts-jp-v4-int8.bin` は形式 v2 です**（654,032 B。v0.3.0 以降。
...
**`saanotts-jp-v4-stage4.pt`** から書き出せます（要 numpy + torch。
...
⚠️ **重みの版で値が変わります。** v1.0.0 から配っているのは **v4**
（[D-057](../docs/decisions.md#d-057)）。**v0.3.x のイメージを焼いたなら v3 の行**を見てください。

| 重み | 構成 | `出力 PCM ... FNV-1a` | sample | `\|max\|` | `Σx²` |
|---|---|---|---:|---:|---:|
| **v4**（配布中） | W8A8 + PIE（**既定**） | **`0x390bf4b2aef8f2ec`** | 27,136 | 7,583 | 84,292,798,374 |
| **v4** | W8A32（`-DSAAN_ENABLE_PIE=0`。Core2 もこちら） | **`0x9cbe622a4a53af7e`** | **27,648** | 8,023 | 82,400,207,857 |
| v3（v0.3.x） | W8A8 + PIE | `0xa69a7ebbb5ccb05f` | 27,136 | 9,627 | 74,264,237,672 |
| v3 | W8A32 | `0xe4b645c30835d42d` | 27,136 | 9,529 | 74,155,591,505 |

⚠️ **v4 では 2 つのレーンで sample 数が違います**（v3 は偶然どちらも 27,136 でした）。
**sample 数まで一致して初めて同じレーンを見ています**（[M-132](../docs/measurements.md#m-132)）。

一致すれば G2P → 合成 → int16 まで QEMU の記録と bit 一致しています。**音が鳴っただけでは証拠になりません。**

**私の実測（CoreS3 / W8A8+PIE / 漢字辞書込み。M-90。⚠️ これは v3 の重み）:**
...
**漢字対応ビルドなら、次の 2 行が同じ checksum になるはずです**
（**v4** = [M-130](../docs/measurements.md#m-130) で実機確認 / v3 = M-90）:

```
かな> きょ][おわよ][いて][んきです°ね     → 0x390bf4b2aef8f2ec   ← v4（配布中）
かな> 今日は良い天気ですね。              → 0x390bf4b2aef8f2ec（同値）
```

⚠️ **v0.3.x のイメージなら `0xa69a7ebbb5ccb05f`**（重みが v3 なので）。
...
✅ **MEM-7 / MEM-8 は実機で測り終えた**（2026-09-18。[M-147](../docs/measurements.md#m-147)。残タスク #20 は閉じた）。
**この 6 行がそれを埋めた行である。** 別の板で確かめるときも同じ 6 行を見る。
見るべき比較対象:

| ログ | 出荷版（`v1.2.0` / arena 139,264 B）の実測 = [M-147](../docs/measurements.md#m-147) |
|---|---|
| PCM FNV-1a | **`0x390bf4b2aef8f2ec`**（53 ids。`reports/m147_device/dev_restore_verify.log`）。**変わったら PCM が動いた = 重大** |
| 定常 xRT | **0.473**（53 ids。要件 ≤ 0.5）。⚠️ **253 ids からは中央値 0.522 で外れる**（平均は 0.499 を越えない） |
| アンダーラン | **0**（測った全長で） |
| 鳴らし始めまで | **369 ms**（要件 ≤ 0.8 s） |
| 内部 DRAM free | **181,119 B** / 最大ブロック **131,072 B**（⚠️ `v1.1.0` は 160,639 / 114,688） |
| arena 高水位（発話後） | **113,072 B**（53 ids）〜 **115,056 B**（303 ids）。**`arena_peak(n)` の予測と 6 点とも一致** |
...
| **`出力 PCM ... FNV-1a`** | **v4**（v1.0.0 = 配布中）: **`0x390bf4b2aef8f2ec`**（W8A8+PIE）/ **`0x9cbe622a4a53af7e`**（W8A32）<br>v3（v0.3.x）: `0xa69a7ebbb5ccb05f` / `0xe4b645c30835d42d`<br>配布イメージ（v0.2.0 まで）: `0x04de91103a0e49f9` / `0x78c209af06affc01` | 上の**基準の 1 行を打ったとき**の値。⚠️ **どの重みを焼いたかで変わる**（[M-132](../docs/measurements.md#m-132)）。⚠️ 一致しなくても即バグではない（下記） |
...
| **FNV-1a** | **v4**（配布中）: **`0x390bf4b2aef8f2ec`**（PIE）/ **`0x9cbe622a4a53af7e`**（W8A32）<br>v3: `0xa69a7ebbb5ccb05f` / `0xe4b645c30835d42d`<br>配布イメージ v0.2.0: `0x78c209af06affc01` | **`きょ][おわよ][いて][んきです°ね` と打ったときと同じ値になるはず**（v4 = [M-130](../docs/measurements.md#m-130) / v3 = M-86 / M-90 で実測） |
...
- `saan_stream_init` / arena 139,264 B（漢字構成。かな構成は 118,784 B）/ 合成 106 frames = 27,136 sample ✅（**実機の高水位 113,072 B**。[M-147](../docs/measurements.md#m-147)）
