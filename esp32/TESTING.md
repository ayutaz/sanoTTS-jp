...
⚠️ **第三者の実機報告が 2 件あります**（CoreS3: W8A8+PIE で **定常 1.554〜1.558× RT** / AtomS3: **1.718× RT**。
どちらも実時間に**間に合っていない**。checksum は M-62 と一致）。本リポジトリでは**まだ再現していません**。
...
|---|---|---:|---:|
| W8A8 + PIE（`-DSAAN_ENABLE_PIE=1`） | **`0xa69a7ebbb5ccb05f`** | 9627 | 74,264,237,672 |
| W8A32（PIE 無し。Core2 もこちら） | **`0xe4b645c30835d42d`** | 9529 | 74,155,591,505 |

一致すれば G2P → 合成 → int16 まで QEMU の記録（M-81）と bit 一致しています。**音が鳴っただけでは証拠になりません。**
...
| **`出力 PCM ... FNV-1a`** | 配布イメージ（v0.2.0 まで）: **`0x04de91103a0e49f9`** / ソースから（S3 以降）: **`0xa69a7ebbb5ccb05f`** | 上の**基準の 1 行を打ったとき**の QEMU 実測値（M-62 / M-81）。⚠️ 一致しなくても即バグではない（下記） |
...
| **FNV-1a** | 配布イメージ v0.2.0: **`0x78c209af06affc01`** / ソースから（S3 以降・PIE 無し）: **`0xe4b645c30835d42d`** | **`きょ][おわよ][いて][んきです°ね` と打ったときと同じ値になるはず**（QEMU 実測） |
