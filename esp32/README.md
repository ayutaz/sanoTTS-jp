...
（`../docs/measurements.md` M-79 / M-80）。**その報告はもう現在地ではない。**
...
| arena（静的確保） | 212,992 B | **かな 118,784 / 漢字 139,264 B**。静的 DIRAM は **漢字 211,535 B（実機実測。`v1.1.0` の 260,855 から −49,320 = −18.9%。うち MEM-7 / MEM-8 が −20,480）/ かな 186,683 B**。1 発話のピークは **113,072〜115,056 B** で `arena_peak(n)` と 6 点一致 = [M-147](../docs/measurements.md#m-147) |
...
要件の分母は 2026-09-11 に決めた = D-049（**定常**）。M-88 / M-90）。
...
| **arena** | 208 KB（212,992 B） | **かな 116 KB / 漢字 136 KB** | M-89 / [M-142](../docs/measurements.md#m-142) / [M-144](../docs/measurements.md#m-144) |
...
| **定常 xRT** | **0.445** | **0.473**（53 ids。[M-147](../docs/measurements.md#m-147) = MEM-7 / MEM-8 の後の実機実測。⚠️ **253 ids からは中央値 0.522**） |
...
### 3. arena は **かな 116 KB / 漢字 136 KB** を `.bss` に静的確保する

⚠️ **2026-09-18 に構成別にした**（[M-144](../docs/measurements.md#m-144) / [D-067](../docs/decisions.md#d-067)）。
**効いたのは MEM-7 = duration net の窓分割**で、**PCM を 1 bit も変えていない**
（変更前の commit のソースを取り出して同じ棒で走らせ、両レーン × 4 つの長さで
`log_d` / `d_hat` / PCM checksum が一致）。
履歴（どれも PCM 不変）: 208 KB → 176 KB（T2 / T4。M-89）→ 148 KB（MEM-5 / MEM-6 /
漢字の配列を実寸に。M-140 / M-142）→ **かな 116 KB / 漢字 136 KB**。

⚠️⚠️ **確保量を決めるのは `saan_stream_arena_used()` ではない**（[C-100](../docs/decisions.md#c-100)）。
あれは **init が終わった後**の `a->used` で、(a) init の途中の duration の一時領域と
(b) **conv が arena の上に取る activation 作業領域**（W8A8 で 2,464 B）を含まない。
⚠️ **(b) は `saan_stream_arena_peak()` にも入っていなかった**（[C-102](../docs/decisions.md#c-102)）。
**MEM-7 で duration が小さくなった瞬間に踏むところだった。**

⚠️ **`saan_stream_arena_needed()` も使ってはいけない。**
n_ids=350 に対し **180,688 B (176.5 KB)** を返す緩い上限（duration と streaming を
**和で**足す）で、実測の **1.58 倍**ある。

根拠は**実測**（`make -C csrc arena` の 2 レーン。2026-09-18）:

| 項目 | W8A32 | **W8A8（実機の構成）** |
|---|---:|---:|
| `saan_stream_arena_needed(350)`（緩い上限） | 180,688 B | 180,688 B |
| `saan_stream_arena_used(350)`（init 後・ホスト） | 114,128 B | 114,128 B |
| **`saan_stream_arena_peak(350)`**（ホスト） | **114,128 B** | **116,592 B** |
| init も pull も通る最小 arena（1 KB 刻みの走査） | **114,688 B** | **116,736 B** |
| MEM-7 の前の `arena_peak(350)` | 137,216 B | 149,824 B |

**ターゲット（32 bit）は `struct saan_stream_impl` のポインタ幅ぶん 1,168 B 小さい**:
`arena_used(350)` = **112,960 B** / **`arena_peak(350)` = 115,424 B**。

**下限は 4 つあって、構成で効くものが違う**:

| | 下限 | ターゲットの値 | いつ効くか |
|---|---|---:|---|
| (a) | `SAAN_KANJI_WORKBYTES` | 121,856 | 漢字（(a') の下） |
| **(a')** | **Viterbi が `SAAN_KANJI_KEY_MAX` を受ける量** = prefix + 64·(KEY_MAX+1) | **138,304** | **漢字ビルド** |
| **(b)** | `saan_stream_arena_peak(350)` | **115,424** | **かなビルド** |
| (c) | `saan_stream_arena_used(350)` | 112,960 | (b) に含まれる |

| 構成 | 既定 | 効く下限 | 余り |
|---|---:|---|---:|
| **かな**（`esp32/` の既定） | **118,784 B（116 KB）** | (b) 115,424 | 3,360 |
| **漢字**（`-DSAAN_KANJI=1`） | **139,264 B（136 KB）** | (a') 138,304 | 960 |

⚠️ **漢字側を 120 KB まで下げていないのは意図である**（[C-101](../docs/decisions.md#c-101)）。
下げると `jdict_analyze` の `cap` が 2,208 → 1,568 になり、**`key_n` が 784〜1024 の入力が
今は通るのに「経路が張れない」になる** = **受け付ける入力が変わる**。
辞書 blob が無いと再現できないので下げていない。

**起動時に検査して落とす**（`esp32/main/main.c`）— 組み合わせ間違いを実行時まで持ち越さない。
⚠️ **下は arena 148 KB のとき（M-142）の実機出力**である（出荷版 = arena 139,264 B の実測は
[`reports/m147_device/dev_restore_verify.log`](../reports/m147_device/dev_restore_verify.log)。
**数字を書き換えてはいけない** — 書き換えると「どの実機も出していない出力」になる = [C-090](../docs/decisions.md#c-090)）:

```
I (1044) saanotts: arena は最長入力（350 ids）に足りる: 要 149824 B / 確保 151552 B（余り 1728 B）
I (1518) saanotts: 漢字経路の作業領域 121856 B（最低限）/ Viterbi に渡る 78848 B （arena 151552 B のうち）
```

この 2 行から (a') が出る: prefix = 151,552 − 78,848 = **72,704 B** /
(a') = 72,704 + 64·(1024+1) = **138,304 B**。

⚠️ **「結果」ブロックの「arena 高水位（発話後）」を見ること**（C-102）。
init 直後に出る `peak` は **pull を 1 度も通っていない**ので、必ず `peak == used` になり、
**下限を 2,464 B 小さく見せる。**

⚠️ **元に戻すには 3 つ一緒に**（片方だけだと起動時に止まる）:

```bash
idf.py -DSAAN_MEM_HEAD_PF=0 -DSAAN_MEM_PIPE_HALO=0 -DSAAN_DUR_K=100000 \
       -DSAAN_ARENA_BYTES=180224 build
```

**次に詰めるなら**（どれも未実装。理由つきで [D-067](../docs/decisions.md#d-067) に置いた）:
`o1539` の粒度 4 → 2（−12,304 B・**速度代償が未測定**）/ `olw` の周期性（⚠️ **加算順が効く** — 素直に表にすると 256 位相のうち 91 で最終ビットが違う。[D-067](../docs/decisions.md#d-067)）/
`s_k4` のポインタ化（漢字の作業領域 −34,944 B だが **arena で買えるのは 8,192 B**）。
⚠️ **かつては 208 KB (212,992 B) だった。** T2（S9 = 捨てる出力を計算しない）と
T4（`cdel` 6 本をリング 1 本に / iSTFT の `re`・`im`・`frm` を `w_e` と共用）で
176 KB に下がった（M-89）。
⚠️ **漢字ビルドだけ 204 KB に落としていた分岐は消えている**（いまの構成別の既定は別の根拠）。
...
⚠️ **上の「クラッシュ帯 175〜191 KB」は当時の確保順の話で、いまの 116 / 136 KB とは無関係**
...
| 7 | **内部 DRAM の残り** | `起動直後: 内部 DRAM free ... / 最大ブロック ...` | arena を引いた後どれだけ残るか。✅ **M5 の漢字ビルド（arena 136 KB）は実機で 181,119 B / 最大ブロック 131,072**（[M-147](../docs/measurements.md#m-147)。⚠️ arena 148 KB のときは 160,639 / 114,688 = M-142、v1.1.0 は 131,799 / 86,016） |
