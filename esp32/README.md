...
| arena（静的確保 / 実機の used） | 212,992 / 195,808 B | **151,552 / 110,592 B**（⚠️ v1.1.0 は 180,224 / 156,688） |
...
| **arena** | 208 KB（212,992 B） | **148 KB（151,552 B）** | M-89 / [M-142](../docs/measurements.md#m-142) |
...
### 3. arena は 148 KB を `.bss` に静的確保する

⚠️ **2026-09-17 に 176 KB → 148 KB にした**（[M-140](../docs/measurements.md#m-140) /
[M-142](../docs/measurements.md#m-142)）。MEM-5（hout を 4 列ずつ）/ MEM-6（パイプ段は
ハローだけ持つ）/ 漢字の配列を実寸に、の 3 つで **PCM を 1 bit も変えずに** 28,672 B 詰めた。

⚠️⚠️ **確保量を決めるのは `saan_stream_arena_used()` ではない**（[C-100](../docs/decisions.md#c-100)）。
あれは **init が終わった後**の `a->used` で、`saan_run_duration` が init の途中で
取って返す 3×`[32][n_ids]`（350 ids で **134,400 B**）を含まない。
**見るのは `saan_stream_arena_peak(n_ids)`** = max(duration フェーズ, streaming フェーズ)。

⚠️ **`saan_stream_arena_needed()` も使ってはいけない。**
n_ids=350 に対し **256,720 B (250.7 KB)** を返す緩い上限（duration と streaming を
**和で**足す）で、実測の **1.87 倍**ある。

148 KB (151,552 B) の根拠は**実測**（`make -C csrc arena`。2026-09-17）:

| 項目 | W8A32 | **W8A8（実機の構成）** |
|---|---:|---:|
| `saan_stream_arena_needed(350)`（緩い上限） | 256,720 B | 256,720 B |
| `saan_stream_arena_used(350)`（init 後） | 114,128 B | 114,128 B |
| **`saan_stream_arena_peak(350)`**（init の途中込み） | **137,216 B** | **149,824 B** |
| init も pull も通る最小 arena（1 KB 刻みの走査） | 137,216 B | **150,528 B** |
| 148 KB で通った最大 n_ids | **350** | **350** |
| 最初に clean fail した n_ids | **400**（`SAAN_ERR_ARENA`。1000 まで**クラッシュ 0 件**） | 400 |
| 実機の used / peak（53 ids・M5 構成） | — | **110,592 B**（M-142） |

設計上限は 350 ids（D-017 の `max_spec_length=700` 相当）なので **1,728 B の余裕**。
⚠️ **`arena_peak(350)` は 16 B 刻みで実測した「init が通る最小」と完全一致する**
（W8A32 137,216 / W8A8 149,824）。1 KB 刻みのゲートが 150,528 と報告するのは走査の粒度。
...
⚠️ **下限は duration の一時領域**（`arena_peak` の 89.7%）。**streaming のバッファ
（112,960 B）も漢字経路（121,856 B）ももう壁ではない**ので、
**MEM-6 で削った 21,472 B は arena には効いていない**（速度には効いた: xRT 0.483 → 0.474）。
次に詰めるなら **duration をチャンク化**する（受容野 ±12 トークン = token パイプと同じ手口。
[M-142](../docs/measurements.md#m-142) §6）。⚠️ **未実装。**

⚠️ **元に戻すには 3 つ一緒に**（片方だけだと起動時に止まる）:

```bash
idf.py -DSAAN_MEM_HEAD_PF=0 -DSAAN_MEM_PIPE_HALO=0 -DSAAN_ARENA_BYTES=180224 build
```

⚠️ **かつては 208 KB (212,992 B) だった。** T2（S9 = 捨てる出力を計算しない）と
T4（`cdel` 6 本をリング 1 本に / iSTFT の `re`・`im`・`frm` を `w_e` と共用）で
176 KB に下がり（M-89）、今回さらに 148 KB になった。
⚠️ **漢字ビルドだけ 204 KB に落としていた分岐は消えている。**
...
⚠️ **上の「クラッシュ帯 175〜191 KB」は当時の確保順の話で、いまの 148 KB とは無関係**
...
| 7 | **内部 DRAM の残り** | `起動直後: 内部 DRAM free ... / 最大ブロック ...` | 148 KB の arena を引いた後どれだけ残るか（M5 の漢字ビルドで **160,639 B** / 最大ブロック 114,688。M-142。⚠️ v1.1.0 は 131,799 / 86,016） |
