 ...
 *
 * M5Stack-Avatar（スタックチャンの顔）を出し、SanoTTS-jp で喋りながら
 * 口をリップシンクさせる例（M5Stack CoreS3 / 16 MB flash）。
 *
 *   起動時に 1 文喋り、以後は画面をタッチするたびに次の文を喋る（4 文をループ）。
 *   喋っている文は吹き出しにも出る（日本語フォント `efontJA_16`）。
 *
 * 元は https://github.com/nnn112358/SanoTTS-jp-M5StackCoreS3-platformio
 * （同じ作者・MIT）の `src/main.cpp`。**CoreS3 の実機で動かしたスケッチ**を
 * ライブラリの example に持ってきたもの（動画: https://x.com/nnn112358/status/2099915646528323965）。
 * ⚠️ **xRT / アンダーラン / 鳴らし始めまでの時間は測っていない**（音が出て口が動くところまで）。
 *
 * ## platformio.ini
 *
 *   [env:m5stack-cores3]
 *   ; ⚠️ 公式の espressif32 は arduino-esp32 2.0.17 で止まっているので pioarduino (3.x) を使う
 *   platform = https://github.com/pioarduino/platform-espressif32/releases/download/55.03.311/platform-espressif32.zip
 *   board = m5stack-cores3
 *   framework = arduino
 *   ; 漢字用（16 MB flash）。このライブラリの extras/partitions/ からプロジェクト直下にコピーする
 ...
 *
 * ⚠️ M5Stack-Avatar の PlatformIO 上の名前は `meganetaaan/M5Stack-Avatar`
 *    （リポジトリは stack-chan/m5stack-avatar に移っているが、レジストリの owner は旧名のまま）。
 *
 * ## Arduino IDE / arduino-cli
 *
 *   ライブラリマネージャでの名前は **`M5Stack_Avatar`**（アンダースコア。`M5Stack-Avatar` では見つからない）:
 *
 *      arduino-cli lib install "M5Unified@0.2.22" "M5GFX@0.2.29" "M5Stack_Avatar@0.10.0"
 *      arduino-cli compile -b esp32:esp32:m5stack_cores3:PartitionScheme=huge_app M5Stack-Avatar-Talk
 *
 *   ⚠️ 漢字を使うにはカスタムのパーティション表（`sanotts_16mb.csv`）が要り、
 *      Arduino IDE ではそれが確実に効かない（ライブラリの README を参照）。漢字は PlatformIO を勧める。
 *
 * ## 辞書（漢字を喋らせるなら）
 *
 *   HelloKanji と同じ。dict パーティション（0x2D0000）に辞書を **1 回だけ**焼く:
 *
 *      gh release download v1.0.0 -R ayutaz/sanoTTS-jp -p k1-dict-438750.bin
 *      esptool --chip esp32s3 -p /dev/ttyACM0 write_flash 0x2D0000 k1-dict-438750.bin
 *
 *   ⚠️ **辞書を焼かなくても起動する。** kanjiReady() が false になり、このスケッチは
 *      かな中間表現（`きょ][おわよ][いて][んきです°ね`）だけを喋る。
 *
 * ## リップシンクの仕組み
 *
 *   SanoTTSSpeakerM5 を継承した LipSyncSpeaker がスピーカーへ流す PCM を横取りし、
 *   チャンク（2,048 sample = 92.88 ms。`SanoTTSSpeakerM5::kMaxChunk`）ごとの RMS を
 *   「再生開始からのサンプル位置」つきで記録する。loop() 側で「今鳴っているチャンク」の
 *   値を拾って avatar.setMouthOpenRatio() に渡す。
 *
 *   tts.say() は喋り終わるまでブロックするので、TTS は別タスク（core 0）で回し、
 *   loop()（core 1）は M5.update() と口の更新だけをする。
 *   ⚠️ **TTS を loop() で呼んではいけない。** Avatar の描画タスクは core 1 で走るので、
 *      loop() を数秒ブロックすると口が動かない。
 *
 *   口の開きが小さすぎる / 大きすぎるときは kMouthRefRms を調整する
 *   （シリアルに出る `absmax` が目安。int16 のピーク値）。 */
#include <Arduino.h>
#include <M5Unified.h>
#include <Avatar.h>
#include <SanoTTS.h>
