...
| | 状態 | 根拠 |
|---|---|---|
| **かな中間表現 → 音声** | ✅ **実機**（M5 CoreS3） | [M-90](docs/measurements.md#m-90) |
| **漢字かな交じり文 → 音声**（端末内で形態素解析 + アクセント推定） | ✅ **実機** | [M-90](docs/measurements.md#m-90) |
| **実時間の要件**（xRT ≤ 0.5） | ✅ **0.446**（満チャンク 1 pull）。⚠️ **発話全体では 0.54〜0.71 で未達**。分母は未決（[D-049](docs/decisions.md) は欠番のまま） | [M-90](docs/measurements.md#m-90) / [C-054](docs/decisions.md#c-054) |
| **メモリ**（SRAM 512 KB に収まる） | ✅ 実行時 **157 KB** | [M-89](docs/measurements.md#m-89) |
| **ブラウザ**（同じ C99 コードを wasm に） | ✅ 実機の PCM と bit 一致 | [M-95](docs/measurements.md#m-95) |
| **アクセント型の再現** | ✅ 教師と **37/37 で符号一致** | [M-59](docs/measurements.md#m-59) |
...
|---|---|---|---|---|
| **M5Stack CoreS3** | ESP32-S3 | 16 MB | 8 MB Quad | ✅ **実機で確認**（[M-90](docs/measurements.md#m-90) / [M-105](docs/measurements.md#m-105)）。**出荷構成** |
| **M5Stack ATOMS3 + Voice Base** | ESP32-S3 | 8 MB | **無し** | ✅ **第三者の実機で 4 MB / 2 MB 辞書が鳴った**（[M-109](docs/measurements.md#m-109)。⚠️ **未再現**） |
...
| M5Stamp-C5 | **ESP32-C5**（RISC-V） | 4 MB | 無し | ❌ **動かない見込み**。**FPU が無く**（`rv32imac`）ソフト浮動小数点が **106 か所**、RAM も **要求 370,980 B > SRAM 393,216 B**（[M-106](docs/measurements.md#m-106) §1）。⚠️ **ビルドは通るが、通ることと動くことは別** |
| ESP32-P4 | RISC-V | — | — | 未着手。**FPU も PIE（`xesppie`）もある**ので有望（[M-106](docs/measurements.md#m-106)）。⚠️ **PIE の命令セットは Xtensa 版と別物**で書き直しが要る |
| ARM Cortex-M / RP2040 など | — | — | — | ❌ **移植していない**。⚠️ **C99 コアはクロスコンパイルが通る**（Xtensa / rv32imac / rv32imafc で 5/5）が、**bare-metal の libc と HAL が無い** |

⚠️ **必要な下限**（[M-106](docs/measurements.md#m-106) §1）: **flash 約 1 MB（かな）/ 4 MB（漢字）**・**ハードウェア浮動小数点**・**200 MHz 以上**。RAM は**漢字経路で合計 370,980 B**（静的 289,568 + Open JTalk の一時ヒープ 81,412）。⚠️ **かな経路だけの RAM は測っていない。**
...
| （実装済み・未出荷）**16 MB + `rec5`** | 13,766,400 B | **538,000** | **0.60%** | ⚠️ **焼いていない**（[M-108](docs/measurements.md#m-108)） |
