...
| 実行時 RAM | **静的 DIRAM 183,979 B**（M5 CoreS3 / かな構成のビルド実測。SRAM 512 KB の **35%** / DIRAM プール 341,760 B の **53.8%**）。うち arena が **118,784 B**（かな）/ **139,264 B**（漢字）= [M-145](docs/measurements.md#m-145)。⚠️ 1 発話の実測ピーク **110,592 B** は arena 151,552 B のときの値（[M-142](docs/measurements.md#m-142)）で、**M-144 / M-145 の構成では実機未測定** |
