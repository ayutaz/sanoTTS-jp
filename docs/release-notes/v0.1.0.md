...
⚠️ **この表は公開時点（2026-08-30）の記述**で、そのまま残してあります。
**その後に実機で動き、速度も測れました** → 下の「🧠 メモリと速度」の
**✅ 2026-09-03 追記**。
...
### ✅ 2026-09-03 追記 — 公開時点の記述は上にそのまま残してある

**このリリースの資産は 1 バイトも変わっていません。** 以下は後から分かったことです。

| 公開時点の記述 | その後に分かったこと |
|---|---|
| **実機で動かしたことがありません** | ✅ **動いた。** 2026-09-02 に M5Stack CoreS3 で走らせ（[M-82](https://github.com/ayutaz/sanoTTS-jp/blob/main/docs/measurements.md#m-82)）、漢字経路も通した（[M-83](https://github.com/ayutaz/sanoTTS-jp/blob/main/docs/measurements.md#m-83)）。⚠️ **ソースからのビルド**で、v0.1.0 の資産そのものではありません |
| **速度: ⚠️ 未測定** | ✅ **測った。** 満チャンク 1 pull の **xRT 0.446**（[M-90](https://github.com/ayutaz/sanoTTS-jp/blob/main/docs/measurements.md#m-90)。W8A8 + PIE を既定にし、積和以外を削ったあと）。⚠️ **この改善は v0.1.0 の資産には入っていません。** v0.1.0 相当のコアは第三者報告で 1.554〜1.718× RT（未再現） |
| — | ⚠️ **このリリースの `saanotts-jp-v3-int8.bin` は blob v1** です。2026-09-02 以降のコア（S4 以降）は `SAAN_ERR_VERSION` で読み込みを拒みます |

⚠️ **音は誰も聴いていません**（G32）。実機で鳴らしてはいますが、聴取はゼロのままです。
...
1. **`uv sync` が通らない** — `pyproject.toml` の `[tool.uv.sources]` が piper-plus への
...
⚠️ `--intermediate` と `--text` は**同じ入力に対して WAV がバイト単位で一致する**
（教師がある環境で確認済み。M-64）。
...
**配布物そのものと `SHA256SUMS.txt` は最初から下の値で正しい**ので、
