...
### ✅ 2026-09-03 追記 — 実機で動き、速度も測れた

⚠️ **公開時点（2026-08-30）の記述は上にそのまま残してある。** 以下は後から足した追記で、
**このリリースの資産は変わっていない。**

| 公開時点の記述 | その後に分かったこと |
|---|---|
| **実機では一度も動かしていません** | ✅ 動いた。第三者が**配布 firmware そのもの**を **AtomS3**（PSRAM 無し）で走らせ、checksum は QEMU の記録と一致した（⚠️ **私は未再現**）。自分の手では 2026-09-02 に M5Stack CoreS3 で走らせている（M-82。⚠️ こちらは配布物ではなく**ソースからのビルド**） |
| **残っている未検証項目は速度だけです** | ✅ 測った。**そして間に合っていなかった。** 第三者報告は AtomS3 **1.718× RT** / CoreS3 **1.554× RT**（どちらも W8A8+PIE・S1 前のコア = このリリースの firmware。⚠️ **未再現**）。差は積和ではなく量子化 / GELU / テンソル検索 / 重みのコピー（M-80）。S1〜S5b でそこを削り、自分の CoreS3 で **0.446** まで下げた（M-90）。⚠️ **これは v0.1.1 の資産には入っていない** |
| **I2S の GPIO は仮置き** | M5Stack（CoreS3 / Core2）については `esp32/boards/m5unified/` で **M5Unified 経由の内蔵スピーカー出力**を用意した。⚠️ **音は誰も聴いていない**（G32） |

⚠️ このリリースの `saanotts-jp-v3-int8.bin` は **blob v1** で、2026-09-02 以降のコア（S4 以降）は
`SAAN_ERR_VERSION` で読み込みを拒む。**焼くだけの firmware は自己完結しているので影響を受けない。**
