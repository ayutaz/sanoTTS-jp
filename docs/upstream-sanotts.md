...
この線引きは **D-032** として凍結し、`.claude/hooks/guard_bash.py` が機械的に強制する（ドキュメント取得は通し、ソース取得と `git clone` と `uv add sanotts` を deny。回帰 105 ケース）。
...
**2026-09-03 追記**: 穴はもう 1 段深く埋まった。かな中間表現の 877 B に加えて、
**漢字かな交じり文をそのまま受け取る経路**（辞書 13.7 MB + Viterbi + Open JTalk の
アクセント規則）が **M5Stack CoreS3 の実機で動いている**（M-90）。
端末側の判定は 3 値（かな / 辞書 / 拒否）で、`!` のような印は要らない。
⚠️ **上流の言語フロントエンドと直接比べられる数字は取っていない**（言語が違う）。

## 上流申告値 vs うちの実測値

| 項目 | 上流（**申告値・未再現**） | うち（**自己実測**） | 出典 |
|---|---:|---:|---|
| ESP32-S3 / int8 + PIE | **0.22× RT**（float との相関 0.985） | **0.446× RT**（CoreS3 / W8A8+PIE / S1〜S5b + T1〜T5 + 64 B 行、n=4 文。上流申告の **2.0 倍遅い**）。⚠️ **満チャンク 1 pull の値**で、発話全体では 0.541〜0.712。経過: 第三者報告 1.554（S1 前・未再現）→ 0.926（S1〜S5a、M-82）→ 0.497（M-88）→ **0.446**（M-90） | M-90 / M-88 / M-82 |
...
| RAM（ストリーミング） | 130〜160 KB | **196.9 KB**（350 ids の arena 192.8 KB + FFT stack 4.1 KB。S1 前）→ 現在は **350 ids の `arena_peak` が 115,440 B**（ターゲット / W8A8）。実機向けの静的確保は **かな 118,784 B / 漢字 139,264 B**、静的 DIRAM 全体で **漢字 211,535 B（実機実測）/ かな 186,683 B**（⚠️ タスクスタックは別勘定）。⚠️ **180,224 / 157,360 B と書いてあったのは v1.1.0 の値** | M-42 / M-89 / [M-147](measurements.md#m-147) |
...
⚠️ **xRT の行は「同じ測り方」ではない。** うちの 0.446 は **満チャンク 1 pull（8 フレーム）の中央値**で、
warmup を含む発話全体では **0.541〜0.712**（M-90、4 文）。上流の 0.22 が**どちらの定義か公開されていない**ので、
「2.0 倍遅い」は上限・下限のどちらでもない**目安**として読むこと。
⚠️ **速度の要件（RTF ≤ 0.5）は満チャンク基準で満たしている**が、発話全体の分母（warmup を含めるか）は未定のまま。
