...
この線引きは **D-032** として凍結し、`.claude/hooks/guard_bash.py` が機械的に強制する（ドキュメント取得は通し、ソース取得と `git clone` と `uv add sanotts` を deny。回帰 100 ケース）。
...
| RAM（ストリーミング） | 130〜160 KB | **196.9 KB**（350 ids の arena 192.8 KB + FFT stack 4.1 KB。S1 前）→ 現在は **350 ids の arena 160,768 B（157 KB）**。実機は静的確保 180,224 B / 実測 used 157,360 B（⚠️ タスクスタックは別勘定） | M-42 / M-89 |
