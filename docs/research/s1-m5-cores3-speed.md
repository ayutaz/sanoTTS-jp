...
**進捗**: §5 の S1〜S4 と S5a、§6 の取り込みは実装済み（[M-81](../measurements.md#m-81) /
[`../plan/s1-speed-implementation-plan.md`](../plan/s1-speed-implementation-plan.md) の進捗表）。
**2026-09-02、ユーザーの CoreS3 で自分で測った**（[M-82](../measurements.md#m-82)）: **定常 xRT 1.554（報告値・S1 前）→ 0.926（−40%）**、
checksum は QEMU と完全一致。ただし 1.0 ぎりぎりで、**実機の内訳は QEMU と別物**だった
（**MAC 63.9% で 1.61 cyc/MAC** = flash 律速の疑い / **GELU 14.0% で 118 cyc/要素**）。
次の順序は M-82 §4（flash vs SRAM の切り分け → GELU の表を DRAM に → S7 / S5b / S6）。
...
- ~~実機の内訳は無い~~ → **取った**（M-82 §3）。§3.3 の QEMU の割合は実機とは**別物**だった（MAC 54.7% → 63.9%、GELU は消えず 14.0%）。**QEMU は「何が呼ばれているか」の道具で、「どこで時間が消えるか」の道具ではない**
- ~~1.55× RT を私は再現していない~~ → S1〜S5a 後のコアで **0.926**（M-82）。S1 前のコアは焼いていないので 1.554 そのものは未再現
