...
作成 2026-09-02 / 決着 2026-09-03。関連: [M-80](../measurements.md#m-80)（自己実測 / ホスト + QEMU）。

> ## ✅ 決着した（2026-09-03）— **満チャンク xRT 0.446。要件 RTF ≤ 0.5 を満たす**
>
> **本文の「1.554× RT で実時間に間に合っていない」は、すべて S1 前の値である。** 現在地は下表。
>
> | | 第三者報告（S1 前） | S1〜S5a（[M-82](../measurements.md#m-82)） | **現在（[M-90](../measurements.md#m-90)）** |
> |---|---:|---:|---:|
> | 満チャンク 1 pull の xRT | 1.554（旧定義。⚠️ C-054） | 0.926（旧定義）/ 0.81（満チャンク換算） | **0.446** |
> | 1 step | 34.6 M cyc（算術） | 18,378,513 cyc | **11,659,500 cyc**（[M-89](../measurements.md#m-89)） |
> | アンダーラン | 10 / 14 | 1 / 14 | **0**（全文） |
> | 鳴らし始めまで | — | 719 ms | **384 ms**（初回 pull 244.65 ms。⚠️ **434 ms は M-88 / M-89 のかな構成**の値） |
> | arena（静的確保 / 実測 used） | 212,992 / 195,808 B | 212,992 / 195,808 B | **180,224 / 157,360 B** |
> | 内部 DRAM の空き（起動直後） | 102,895 B（報告） | 99,987 B | **132,039 B** |
>
> **PCM の checksum は M-82 から 1 bit も変わっていない**（W8A8+PIE `0xa69a7ebbb5ccb05f` / W8A32 `0xe4b645c30835d42d`）。
> 実装の内訳は §5 の結果欄、経緯は [S2 計画](../plan/s2-fast-kanji-m5-plan.md) と [C-054](../decisions.md) / [C-055](../decisions.md)。
>
> ⚠️ **発話全体（全 pull の dt の和 ÷ 音声長）で見るとまだ 0.5 を超える**: **0.541〜0.712**（M-90 の 4 文）/ 0.550〜0.693（M-88 の 3 文）。
> warmup 38 フレームぶんが初回 pull に乗るためで、**文が長いほど 0.5 に近づく**。
> ✅ **要件の分母は定常に決まった**（[D-049](../decisions.md#d-049)。2026-09-11）。**発話全体は参考値**である。
> ⚠️ **音は聴いていない**（G32）。アンダーラン 0 は「pull の計算時間 < そのチャンクの音声長」の回数で、途切れの有無は人が聴くまで分からない。
...
checksum は QEMU と完全一致。ただし 1.0 ぎりぎりで、**実機の内訳は QEMU と別物**だった
（**MAC 63.9% で 1.61 cyc/MAC** = flash 律速の疑い / **GELU 14.0% で 118 cyc/要素**）。
⚠️ **この「0.926」と「flash 律速の疑い」は翌日の測定で半分が違っていた**（[C-054](../decisions.md)）:
0.926 は末尾の端数 pull で膨らんだ値で満チャンクは 0.81、MAC の 1.61 cyc/MAC は flash が無くても
78 cyc/dot の固定費が出て flash はその上に 38.5% 乗るだけ、GELU 118 cyc/要素の原因は表の置き場ではなく
`saan_erf_approx` が非インラインだったこと（[M-85](../measurements.md#m-85)）。
...
**移植が正しいことの根拠として使ってよいのはこの一致だけ**で、速度の数字は**当時は**誰も再現していなかった。
⚠️ **1.554 そのものは今も未再現**（S1 前のコアを自分では焼いていない）。自分で測ったのは S1〜S5a 後の 0.926 以降で、
S1 前の同一コードで測ったのは W8A32 の 4.877（[M-83](../measurements.md#m-83) §2、報告値 4.834 と同水準）だけである。
...
**実機で測った答え**（[M-82](../measurements.md#m-82) §3 → [C-054](../decisions.md) → [M-85](../measurements.md#m-85)）:

| # | 実機の判定 |
|---|---|
| H1 | ✅ 当たり。S2 で除算と `rintf` が消えた。QUANT は M-88 で 750,077 cyc（STEP の 6.4%）まで落ちた |
| H2 | ⚠️ **当たりだが原因が違った。** 118 cyc/要素の主因は表の置き場（flash）ではなく `saan_erf_approx` の**非インライン化**（C-054 / M-85 E 節）。表を DRAM に置いても **1 cyc も動かない** |
| H3 | ✅ 当たり。S1 で pull 中の LOOKUP が 0 回に（M-81） |
| H4 | ✅ 当たり。S4（blob v2 の事前整列）で実行時の転置コピーが消えた |
| H5 | ⚠️ **半分。** 全部キャッシュヒットでも **78 cyc/dot = 1.63 cyc/MAC** の固定費が出る（M-85 D 節）。PIE 命令の発行自体は律速ではない |
| H6 | ⚠️ **半分。** flash は固定費の**上に 259 cyc / 32 B 行 = 38.5%** 乗るだけで、床そのものではない。PSRAM に写しても **−0.9%** |
| H7 | ✅ 当たり。S6（= T3）で TOKEN が −64.8%（M-88） |

**表に無かった最大の項が 1 つあった**: **MAC の 37%・GELU の 42% は「捨てられる出力」の計算**だった
（AC 窓 16 / DEC 窓 14 のうち中央 8 しか下流に渡らない）。T2（= S9）で **bit 同一のまま** MAC 要素が 42.3% 減った。
...
| 順 | 何 | 出力への影響 | ゲート | **結果（2026-09-03）** |
|---|---|---|---|---|
| **S1** | **テンソル検索を `saan_stream_init` で 1 回に**（解決済みポインタを `saan_stream_impl` に持つ） | **bit 同一** | all-test / QEMU checksum `0x04de91103a0e49f9` 不変 / プロファイルの LOOKUP が 0 回 | ✅ 入った（M-81）。LOOKUP 0 回を `make -C csrc prof --expect-no-lookup` が固定 |
| **S2** | **量子化の除算を逆数の乗算に、`rintf` を FPU の丸め命令に** | ⚠️ **丸め水準で変わる**（`x/s` と `x*(1/s)` は最終ビットが違いうる） | int8-e2e の SNR ゲート（平均 ≥ 27 / 最小 ≥ 25 dB）/ `\|max\|` と Σx² で丸め差の水準 / **新しい checksum を M-番号で記録** | ✅ 入った（M-81）。**丸め水準**（W8A8 e2e 平均 24.24 → 24.21 dB / 最小 21.94 不変）。QEMU checksum は**この 1 文では不変**で、動いたのは S3 |
| **S3** | **GELU の `erff` を多項式近似に**（絶対誤差 1e-7 級。`expf` も呼ばない） | 丸め水準 | 同上 + ホストで `erff` 版との max\|Δ\| を陽性対照つきで測る | ✅ 入った（M-81）。**ここで基準 checksum が動いた**: W8A8+PIE `0x04de91103a0e49f9` → **`0xa69a7ebbb5ccb05f`** / W8A32 `0x78c209af06affc01` → **`0xe4b645c30835d42d`**。ゲートは `make -C csrc erf`（線形補間の陽性対照つき） |
| **S4** | **重みを blob の時点で `[cout][k][align16(cin)]` に並べ替えて 0 埋め**（exporter 変更、blob v2） | **bit 同一**（順序を変えない） | all-test / `export_c_weights.py` の一致ゲート / QEMU checksum | ✅ 入った（M-81）。blob は **v2**（654,032 B）。⚠️ **リリース資産 `saanotts-jp-v3-int8.bin` はまだ v1** で、S4 以降のコアは `SAAN_ERR_VERSION` で拒む |
| **S5a** | PIE ループのロード併合 | **bit 同一**（整数） | pie_probe の bit 一致 / QEMU checksum | ✅ 入った（M-81）。`ee.` 命令 7 本 |
| **S5b** | 重みをレジスタに置いたまま t を回す（weight-stationary） | **bit 同一**（整数） | 同上 | ✅ 入った（M-90）。dot の 95.1% / MAC の 76.3% を覆う。⚠️ **単独の寄与は分離していない**（漢字構成とまとめて焼いた） |
| S6 | token block の出力をトークン単位で持ち越す（H7） | bit 同一（ハロー 12 は保つ） | stream_test G2（一括版と bit 一致） | ✅ 入った（S2 計画の **T3**、M-88）。TOKEN 2,077,284 → **730,908 cyc（−64.8%）** |
| S7 | `SAAN_CHUNK` 8 → 16（窓の再計算率 AC 2.0→1.5 倍、DEC 1.75→1.375 倍、flash 読み出しとヘッダ走査が半分） | bit 同一 | arena（`o1539` は 2 分割で 49 KB のまま）/ 遅延 +93 ms を requirements と照合 | ⏸ **入れていない**（要件を満たしたので起票せず）。遅延が +93 ms 増える側なので、初回レイテンシと引き換えになる |
| S8 | **2 コア**（出力チャネルを 2 分割して並列） | bit 同一（各チャネルの計算は同じ） | 同上 + DRAM（タスク 2 本目のスタック） | ⏸ **入れていない**（同上） |

**この順序表に無かった 5 つが、実際には効いた**（S2 計画。[C-054](../decisions.md) が経緯）:

| | 何 | 効果（実機） |
|---|---|---|
| **T1** | 末尾 pull の早期終了と xRT の定義の修正（全フレームが出そろった後も `step_chunk` が 3 回回っていた） | 表示 0.926 の 13% ぶんはこれ。アンダーラン 1/14 → 0（M-87 / M-88） |
| **T2 = S9** | **捨てる出力を計算しない**（AC 窓 16 / DEC 窓 14 のうち中央 8 しか下流に渡らない。フレーム独立なので中央だけ計算しても bit 同一） | MAC 要素 **−42.3%** / GELU 要素 −42.1% / DW −42.9%（M-88） |
| **T4** | arena の詰め（`cdel` 6 本をリング 1 本に / iSTFT の作業配列を共用 / `h` の in-place 化） | 静的確保 212,992 → **180,224 B**、内部 DRAM の空き **+36,420 B**（M-89） |
| **T5** | GELU のコード生成（`saan_erf_approx` のインライン化 + 事前スケール表 + DRAM 配置） | 118.38 → **73.14 cyc/要素**（M-87 / M-88）。⚠️ **途中で符号ビットの memcpy を入れて 211 cyc/要素に倍増させた**（[C-055](../decisions.md)） |
| **F-1** | D-cache の行を 32 B → 64 B（sdkconfig 1 行） | xRT 0.926 → **0.861**（M-84） |

⚠️ **S2 / S3 は「ホストとターゲットは bit 一致しない。それは正常」（M-62）の水準に収まることを、
`\|max\|` 完全一致 + Σx² 相対差 1e-7 級で示す。** 収まらなければ近似の次数を上げる。
→ **S3 は収まった**（W8A32 は `\|max\|` 9529 が同一・Σx² 相対差 8.7e-9。W8A8 は量子化の境界で 1 値が動き `\|max\|` 9744 → 9627 になるが、
fp32 比 SNR は 24 文とも不変。M-81）。**基準 checksum はここで 1 度だけ更新した。**
⚠️ **S7 / S8 は最後。** 先に S1〜S5 で 1 コアの床を出さないと、何倍になったか分からない。
→ **床を出したら S7 / S8 の前に要件を満たした**（0.446 ≤ 0.5）。**どちらも入れていない。**
...
⚠️ **ESP32（Core2 / Basic）は静的 arena が `dram0_0_seg` に入らない**（208 KB のとき 65,368 B 溢れた。
T4 後は 176 KB だが ESP32 では依然入らない）。`SAAN_ARENA_HEAP=1` で PSRAM から取る。**遅い（未測定）ので速度の測定には使わない。**
⚠️ ~~**S3 以外で `SAAN_ENABLE_PIE=1` を渡すと CMake で止める**~~ → **ESP32-S3 では W8A8+PIE が既定になった**（[D-048](../decisions.md)、2026-09-03）。
S3 以外では自動的に W8A32 のまま。W8A32 で測りたいときは `-DSAAN_ENABLE_PIE=0` を明示する。
⚠️ ~~K トラック（辞書 13.7 MB のパーティション mmap）は PSRAM 有効な板で動かない可能性がある（§0 の報告）。**未対応。**~~
→ **原因が分かって直った**（[M-90](../measurements.md#m-90) §4）。M5 構成は `CONFIG_SPI_FLASH_ROM_IMPL=y` で
`spi_flash_mmap` が **ROM の実装**にリンクされ、IDF はそれに **128 ページ = 8 MB** しか渡さない。辞書は 211 ページなので `esp_partition_mmap` では貼れない
（報告された `ESP_ERR_NO_MEM` の説明になる）。**`esp_mmu_map`（`esp_mm`、ROM 実装と無関係）に切り替えて解決**し、
PSRAM 8 MB と同居して 23.7 MB の連続 vaddr が空いていた。**M5 構成で辞書 + 漢字 + PIE の 1 本のファームが動いている。**

## 7. 未検証・限界

- ~~実機の内訳は無い~~ → **取った**（M-82 §3）。§3.3 の QEMU の割合は実機とは**別物**だった（MAC 54.7% → 63.9%、GELU は消えず 14.0%）。**QEMU は「何が呼ばれているか」の道具で、「どこで時間が消えるか」の道具ではない**。⚠️ さらに [C-055](../decisions.md): **命令数が減っても実機は遅くなりうる**（T5 の符号ビット memcpy で GELU が 2 倍になった）。速度の判断は実機の `-DSAAN_PROFILE=1` の表だけで行う
- ~~1.55× RT を私は再現していない~~ → S1〜S5a 後のコアで **0.926**（M-82）、現行コアで **0.446**（M-90）。**S1 前のコアは焼いていないので 1.554 そのものは今も未再現**（S1 前の W8A32 は 4.877 = M-83 §2 で測った）
- **音は聴いていない**（G32）。checksum が M-82 以来同一なので**波形は 1 bit も変わっていない**が、途切れの有無・過剰強調・誤読の重大さは人が聴くまで分からない
- **発話全体の xRT 0.541〜0.712（M-90、n=4 文）は要件を超えている。** 要件の分母（満チャンク 1 pull か発話全体か）は `../requirements.md` §6.2 に**両方書いてあるだけで未決**（D-049）
...
- 「flash 上限 40 MB/s で 56 ms」はデータシートの理論上限からの算術で、**実測ではない**。⚠️ 実測は [M-85](../measurements.md#m-85) の D 節（**259 cyc / 32 B 行 = dot 時間の 38.5%**）
- **S5b の寄与を分離していない**（漢字構成とまとめて焼いた。M-90 §5）。**S7 / S8 は入れていない**ので、1 コアの床がどこかは分かっていない
