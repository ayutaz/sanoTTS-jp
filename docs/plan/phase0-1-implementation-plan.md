...
✅ B-5       教師品質ベースライン         教師/実人間 = UTMOS 0.758 / SCOREQ 0.820
✅ Phase A   ラベル生成の設計確定         A-1/A-3 確定、A-2 は実測で決着（prosody=zeros）
✅ Phase B   実装完了                    B-e (本番実行) のみ vast.ai 待ち
✅ Phase C   実装完了                    4 段の学習ループがスモークテスト通過
✅ B-4/6/7/8/9/10  検証タスク完走        D-016〜D-025 として凍結（下表）
✅ SCOREQ    論文の主指標を導入           PyPI にあった。教師 2.0488 / 実人間 2.4983
▶  次        本学習 (vast.ai)            手順は ../vastai-runbook.md
...
| リスク | 影響 | 状態 |
|---|---|---|
| **567 K で日本語の品質が届くか** | 成果物の価値に直結 | **⚠️ 現在地: v1（各 5,000 step）で SCOREQ 教師比 0.387 / 目標 1.112 に対し絶対値 0.7645。学習量が全く足りていない（Stage 4 で 0.24 epoch）** |
| C99 コアの実装量 | 参照実装が無い。Phase D の主コスト | Phase D |
| ~~`λ` 群のチューニング~~ | ~~論文に値が無い~~ | ✅ 勾配整合で初期値を決めた（D-021）。`λ_n`/`λ₂` は実行時算出で探索不要 |
| ~~SCOREQ が未導入~~ | ~~論文の主指標が測れない~~ | ✅ PyPI にあった（C-016）。導入済み |
| **UTMOS と SCOREQ が生徒の劣化で食い違う** | 指標の読み方を誤ると判断を誤る | ⚠️ v1 で UTMOS 比 0.700 / SCOREQ 比 0.387。**主指標は SCOREQ**（D-020） |
...
| ID | 判断 | 期限 | 影響範囲 |
|---|---|---|---|
| ~~**C-1**~~ | ~~`λ₂ / λ_n / λ_Δ / λ_s` と `λ_T`~~ → **勾配整合で初期化。`λ_n`/`λ₂` は実行時算出**（D-021） | ✅ | 残る探索は `λ_Δ` / `λ_s` / `λ_T` の 3 つ |
| ~~**C-2**~~ | ~~判別器の構造~~ → **`scales=3, base_channels=16` = 94,755 params**（D-025） | ✅ | ⚠️ 正規化 3 種に測れた差は無い |
| ~~**D2**~~ | ~~`_` PAD の duration の扱い~~ → **特別扱いしない**（D-018） | ✅ | PAD はフレームの 53.76% |
| ~~**D5**~~ | ~~iSTFT のフレーミング~~ → **`center=True` + `length=T*256`**（D-022） | ✅ | 末尾ゼロ埋めも外した |
| ~~**D6**~~ | ~~`yT` に EMA を当てるか~~ → **当てる**（canonical との整合、D-023） | ✅ | — |
| **D4** | アクセント記号トークンのみか、A1/A2/A3 3 スカラー (+128 params) を足すか | Phase 2 の評価後 | `Dα` のアクセント再現性 |
| **D7** | `β`（式7 のノイズ注入強度） | Phase 5 | **聴取で決める。** 集約スコアはむしろ下がる |
| **D8** | 学習ステップ数 / lr スケジュール / バッチサイズ | 本学習 | **論文に記載が無い。** 損失曲線で決める |
