...
| # | タスク | いま着手できるか | 状態 |
|---|---|---|---|
| **P-1** | **PIE カーネル** | ✅ **実装完了**（M-57） | ⚠️ **速度は未測定**。QEMU では測れない。**実機待ち** |
...
| **PIE カーネルの実装** | ✅ **完了**（M-57）。**QEMU で bit 完全一致**を確認。MAC の **77.1%** を覆う |
| 残り 22.9% の PIE 化 | ▶ activation のストライドを 16 の倍数にパディングすれば可能（未着手） |
