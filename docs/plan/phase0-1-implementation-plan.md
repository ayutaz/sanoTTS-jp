...
**品質もメモリも目標に届き、追試 E-1 / E-2 も決着した。残りは 3 本。**
...
| **E-2b** | **`Gγ` が容量律速かの幅スイープ** | ✅ **今できる**（学習 約 2 時間） | E-2 の唯一の残り |
...
### ▶ E-2b: `Gγ` が容量律速かの幅スイープ — **今できる唯一の実験**

decoder gap 0.395 が **(i) 容量不足**なのか **(ii) 学習不足**なのかは切り分いていない。

| 項目 | 内容 |
|---|---|
| やること | `Gγ` の幅 `W` を振って Stage 3 のみ再学習し、gap が幅に応じて縮むか見る |
| コスト | **約 2 時間**（Stage 3 のみ ×4 本、MPS） |
| 判定 | 幅を増やして gap が縮む → 容量律速。縮まない → 別の要因 |

⚠️ **これは「decoder を作り直す」判断の前提**であって、作り直しそのものではない。
⚠️ 幅を変えると **559,008 params と int8 blob サイズが変わる**ので、
**成果物の仕様変更になる**。スイープは調査であって採用ではない。
### 優先度を下げたまま据え置くもの
