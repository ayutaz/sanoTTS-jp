...
- 作成 2026-08-26 / 最終更新 2026-08-27（Phase D-2 ストリーミング化 完了時点）
...
| 項目 | 着手前 | 実測（M-42） |
|---|---:|---:|
| ピーク RAM | 1,258 KB（SRAM の 246%） | **197 KB（38%）** |
...
| # | 条件 | 結果 |
|---|---|---|
| G1 | ピーク RAM < 200 KB | ✅ **197.0 KB**（実用最大 350 ids = D-017 の上限） |
...
#### D-3 ▶ 次: FFT 化 → レイテンシ測定 → int8 カーネル → 実機

**順序が重要**。まとめて変えると切り分けができない。

| # | 作業 | 完了条件 | 状態 |
|---|---|---|---|
| **D-3a** | `irfft_1024` を radix-2 FFT に | **ストリーミング版と bit 一致**しないので、往復 SNR ≥ 120 dB と golden test の Pearson ≥ 0.98 で判定 | ▶ 次 |
| **D-3b** | レイテンシ測定 | ⚠️ **一度も測っていない。** 論文は ESP32-S3 で 0.22× RT。まず手元で「1 秒の音声を作るのに何秒」を測り、ESP32 との比を明示して外挿する | D-3a の後 |
| **D-3c** | int8 カーネル | fp32 版と SCOREQ 教師比が 0.95 倍以上。blob は 624,692 B（M-39） | D-3b の後 |
...
⚠️ **レイテンシの外挿には根拠が要る。** M-18 の「RTX 4090 ×10」のような
係数を推測で置かない。ESP32-S3 の実クロックと SIMD 幅から積む
（M-16 と同じ方法）。**それでも外挿は外挿**と明記する。

### 主要リスク

| リスク | 影響 | 状態 |
|---|---|---|
| **レイテンシが 0.22× RT に届くか** | **残る唯一の成否要因** | ⚠️ **一度も測っていない。** `irfft` が naive DFT (O(N²))。FFT 化が先（D-3a → D-3b） |
| **実機で測れていない** | 解析と実機は違う | ⚠️ ハードウェア待ち（D-3d）。RAM は解析で 197 KB / SRAM 512 KB |
...
| ~~ESP32 の RAM に載らない~~ | ~~プロジェクトの成否に直結~~ | ✅ **197 KB / SRAM 512 KB の 38%**（M-42）。一括版と bit 一致 |
...
| **D10** | int8 カーネルの activation スケール | D-3c | 論文は per-frame。ESP32-S3 の PIE は 16 レーンなので整合が要る |
| **D11** | FFT の実装（radix-2 自前か既存ライブラリか） | D-3a | ⚠️ **bit 一致は要求できない**（積和の順序が変わる）。SNR ≥ 120 dB で判定する |
...
| リスク | 状態 | 潰し方 |
|---|---|---|
| **レイテンシ** | ⚠️ **一度も測っていない** | `irfft` を FFT 化してから（D-3a → D-3b）。naive DFT (O(N²)) では測る意味が無い。⚠️ **外挿には根拠が要る**（M-18 の「×10 と仮定」を繰り返さない） |
| ~~ESP32 の RAM ピーク~~ | ✅ **197 KB / SRAM 512 KB の 38%**（M-42） | 一括版と bit 一致。⚠️ ただし fp32 かつ解析値（実機未測定） |
