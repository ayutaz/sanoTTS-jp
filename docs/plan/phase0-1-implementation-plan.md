...
✅ Phase D-3b レイテンシ初測定           手元 **0.024× RT**。iSTFT 88% → 0.9%（M-43）
✅ Phase D-3c int8 カーネル（C 版）      逆量子化と 100 dB 超で一致。⚠️ **PIE 未使用**
▶  次        Phase D-3c' PIE 最適化      **移植可能 C のままだと ESP32 で 0.93× RT**
   Phase D-3d 実機                      ⚠️ ハードウェア待ち
```

**プロジェクトの主目的（ESP32 で日本語 TTS が動く）に対する現在地**:
品質は目標に届き（M-37）、**メモリも SRAM に載った**（197 KB / 512 KB、M-42）。
**レイテンシも測れた**（手元 0.024× RT、M-43）。
⚠️ **ただし ESP32 への外挿では、現行の移植可能 C（fp32）は 0.93× RT で実時間ぎりぎり。**
論文の 0.22× RT は **fp32 では達成不可能**で int8/int16 の PIE が前提だと検算できた。
**残るのは PIE カーネルと実機。**
...
| **D-3b** | レイテンシ | ✅ 手元 **0.024× RT**。iSTFT 88% → **0.9%**、いまは **decoder が 60%** |
...
| 実装 | 合計 × RT |
|---|---:|
| **移植可能 C / fp32（＝現行 `csrc` をそのまま移植）** | **0.93** ← 実時間ぎりぎり |
