...
▶  今        P-1 W8A8 + PIE カーネル      **fp32 のままでは ESP32 で 2.47× RT = 間に合わない**
                                        ⚠️ **W8A32 なので現行に intrinsic を足しても速くならない**
                                        逆アセンブルで **PIE 命令 0 件**を確認（M-53 / C-032）
   Phase D-3d 実機                      ⚠️ **ESP32-S3 ボード待ち**（これは本物の待ち）
```
...
| **D12** | **PIE カーネルを実機なしで書くか** | D-3c'-3 | ▶ **書く。** ⚠️ **かつて「toolchain が無いので待ち」としていたのは誤りだった**（C-033）。ESP-IDF v5.5 を導入して**コンパイルと逆アセンブルはできる**（M-54）。速度は実機が要るが、**PIE 命令が実際に出ているかは確認できる**。⚠️ ただし本体は W8A8 への書き換えで、intrinsic を足すだけではない（M-53 / C-032） |
...
であり、**PIE（整数 SIMD）では速くならない**。**W8A8 への書き換えが本体**（M-53）。
...
| # | タスク | いま着手できるか | 状態 |
|---|---|---|---|
| **P-1** | **W8A8 + PIE カーネル** | ▶ **着手した**（toolchain 導入済み） | **唯一の未達項目**。⚠️ **想定より大きい**（M-53）。実機は無い |
...
#### 誤り 1: 「toolchain 待ち」ではなかった（M-54 / C-033）
...
#### 誤り 2: 「intrinsic を足すだけ」ではなかった（M-53）

現行の `csrc/saanotts_int8.c:88-91` はこうなっている:

```c
const float wv = (float)qv;                                /* int8 → float に戻す */
for (int t = t0; t < t1; ++t) yo[t] += wv * xi[t + sh];    /* ← float の積和 */
```

**W8A32 である。** 重みだけ int8 で、**積和は fp32**。
**ESP32-S3 の PIE は 8/16 ビット整数の SIMD であって浮動小数の SIMD ではない**
（FPU はスカラのみ）ので、**この関数に intrinsic を足しても速くならない**。

M-43 の外挿表の 10 倍差（f32 asm 142 MMAC/s 対 int8 asm 1,427 MMAC/s）は
「アセンブリ化したか」ではなく「**SIMD が使えるか**」の差である。
...
最小 **23.27 dB**（M-45。24 文中 9 文が 25 dB 未満）。
**golden test を bit 一致では書けなくなる**（許容誤差での比較に変わる）。
