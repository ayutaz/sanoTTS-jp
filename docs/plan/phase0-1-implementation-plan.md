...
▶  次        Phase D-2 ストリーミング化   **今の実装は 1.26 MB。ESP32 の SRAM に載らない**
   Phase D-3 int8 カーネル + 実機
```

**プロジェクトの主目的（ESP32 で日本語 TTS が動く）に対する現在地**:
品質は目標に届いた。**残るのはメモリとレイテンシ**で、そこが Phase D-2 / D-3。
...
**decoder ±16 フレーム / acoustic ±10 フレーム**。チャンク処理＋オーバーラップが要る。

⚠️ **`irfft_1024` が naive DFT（O(N²)）。** 正しさ優先でそうしてあるので、
**このままではレイテンシを測れない**。radix-2 FFT に差し替える。

**完了条件**: ピーク RAM < 200 KB（SRAM に余裕を持って収まる）かつ
ストリーミング版と一括版の出力が **bit 一致**（チャンク境界の取りこぼしが無いこと）。
