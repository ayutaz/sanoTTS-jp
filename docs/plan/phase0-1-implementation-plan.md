...
| **速度** | ✅ **達した**（2026-09-03）。第三者の実機報告 1.554× RT（未再現）→ 自分の実測 0.926（M-82）→ **満チャンク 1 pull の xRT 0.446**（M-90。要件 ≤ 0.5）。アンダーラン 0、鳴らし始め 432 ms。⚠️ **発話全体では 0.541〜0.712 でまだ 0.5 超**（要件の分母が未定）。経緯は [`s2-fast-kanji-m5-plan.md`](s2-fast-kanji-m5-plan.md)（§10 に S-1 の前史） |
