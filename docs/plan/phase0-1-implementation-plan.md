...
✅ D-3d      実機測定                  **CoreS3 で W8A8+PIE 0.926× RT**（M-82 / D-047）、漢字経路 G28〜G31（M-83）
                                        ⚠️ 目標 RTF ≤ 0.5 は未達。続きは s1-speed-implementation-plan.md
▶  今        S1 の続き                  flash 律速の切り分け / GELU 表の DRAM 配置 / S7 / S5b / D-048（M-82 §4）
```
...
| **速度** | ⚠️ **ここだけ未達。** 第三者の実機報告（M5Stack CoreS3、2026-09-02）は **W8A8+PIE 1.554× RT**（間に合っていない。未再現）。積和以外が支配的だった（M-80）ので S1〜S5a で削り、QEMU 命令数比で 1 step −49%（M-81）。**実機での効果は未測定** → [`s1-speed-implementation-plan.md`](s1-speed-implementation-plan.md) |
...
## 10. 残りのタスク（2026-08-31 現在）

**品質もメモリも目標に届き、追試 E-1 / E-2 / E-2b も決着した。残るのは速度だけ。**

⚠️ **この計画は「かな中間表現の生徒モデル」のトラックだけを扱う。**
端末で漢字を扱う **K トラックは別計画**で、**K-0 〜 K-7 まで完了している**
（QEMU で漢字文から合成まで完走。M-76）。
→ [`k1-kanji-implementation-plan.md`](k1-kanji-implementation-plan.md)

**残りは 5 つ（2026-09-02 更新）。5 は S1〜S5a まで入れ、実機で効果を測るのが残り。**

| # | 何 | トラック | ゲート | 実機が要るか |
|---|---|---|---|---|
| **1** | **サイクル実測**（速度） | 両方 | D-3d（かな）/ G28 / G29（K） | ✅ **要る** |
| **2** | **聴取** | 両方 | G32 | ❌ 不要（WAV は用意済み） |
| **3** | 起動から合成まで落ちない / checksum が QEMU と一致 | K | G30 / G31 | ✅ 要る |
| **4** | 出荷ファームで **W8A8 + PIE** を有効にするかの判断 | かな | — | ✅ 実質要る（動機が速度） |
| **5** | **速度の作り直し S1〜S8**（[`../research/s1-m5-cores3-speed.md`](../research/s1-m5-cores3-speed.md) §5 / 計画 [`s1-speed-implementation-plan.md`](s1-speed-implementation-plan.md)）。第三者の実機報告 **1.554× RT** を受けて起票（2026-09-02）。**S1〜S5a は入れた**（M-81 / D-046。QEMU 命令数比 −49%）。残り S5b と実機での効果測定 | かな | all-test / QEMU checksum / 実機プロファイル | S1〜S6 は ❌ 不要（ホスト + QEMU で bit 一致まで確かめられる）。効果の確認は ✅ 要る |
...
**その速度が測れるのは実機だけ**。材料（M-55 / M-57 / M-58）は揃っている。

### ⚠️ ボードが手に入ったら、**2 トラック分をまとめて測る**

同じボードで両方測れる。**別々に取りに行くと 2 回焼き直すことになる。**

| | かなトラック | K トラック |
|---|---|---|
| 焼くもの | app + `model`（8 MB の表で足りる） | app + `model` + **`dict` 13,702,320 B**（**16 MB の表**） |
| 測ること | 定常 xRT / アンダーラン / PIE の効き | G2P レイテンシ / 定常 xRT / 完走 |
| bit 一致の確認 | かな経路 `0x78c209af06affc01` | 同上（**同じ値になるはず**） |
| 聴取 | β の再確認（D-038 は 1 名） | 漢字経路とかな経路の聞き比べ |

⚠️ **ボードは N16R8 を選ぶこと。** 8 MB では K トラックの辞書が入らない。
16 MB のボードなら**かなトラックの構成もそのまま焼ける**（`partitions.csv` は 5.06 MB）。
⚠️ **PSRAM の有無で構成が変わる。** `kj_alloc()` は PSRAM を優先するので、
**bit 一致を主張するなら先に PSRAM 無しで測る**。
...
| 実機でのサイクル実測 | ⚠️ **第三者の報告が 1 件**（CoreS3 / W8A8+PIE 1.554× RT。未再現）。自分の板（スタックチャン）で測るのが A-4 |
...
**P-1 の実装は終わった。⚠️ ただし「速くなった」とは言えなかった** —
第三者の実機報告（2026-09-02）は **1.554× RT** で、M-43 の外挿 0.088× RT より **18 倍遅い**。
原因は積和ではなく量子化 / GELU / テンソル検索 / 重みのコピー（M-80）。**S1〜S5a で削った**が
（M-81。QEMU 命令数比 −49%）、**実機での効果は未測定。**
