...
- 作成 2026-08-26 / 最終更新 2026-09-03（**完了した計画の記録**として畳んだ）
...
> ## ⚠️ この文書は**完了した計画の記録**である（2026-09-03）
>
> **Phase 0 / A / B / C / D と検証タスク B-0〜B-12 / D-4 / E-1 / E-2 / E-2b はすべて決着し、
> §10 に「残り」として挙げていた 5 件も片づいた**（速度は要件 RTF ≤ 0.5 を満たした = M-88 / M-90。残るのは **G32 の聴取**だけ）。
>
> **現在地と、これからやることは [`s2-fast-kanji-m5-plan.md`](s2-fast-kanji-m5-plan.md) を見ること。**
> この文書を残してあるのは、**測定の根拠と「なぜその設計にしたか」がここにしか無い**ため（§1〜§9）。
> **§0 のロードマップと §10 の「残りのタスク」は完了済みなので、当時の記録として読む。**
- 本文書の位置づけ: **§1 以降が本体**（検証タスク B-* と各 Phase の詳細、設計の逆算、評価パイプライン）。
  未確定のものは `⚠️ 未検証`、済んだものは `✅` を付ける。反証された主張は採用しない。

---

## 0. ロードマップ（**当時の記録。全部片づいた**）
...
✅ Phase D-3c'-4 ESP-IDF 雛形            M-46 の時点では未ビルド → **M-56 でビルド成功**（267,968 B）/
...
✅ P-1      W8A8 + PIE カーネル        **実装完了**。QEMU で bit 完全一致（MAC の **99.40%**。M-57 / M-58）
                                        ✅ 速度も実機で測った（M-82 → M-90）。**出荷ファームの既定にした**（D-048）
...
✅ D-3d      実機測定                  **CoreS3 で W8A8+PIE 0.926× RT**（M-82 / D-047）、漢字経路 G28〜G31（M-83）
                                        当時は目標 RTF ≤ 0.5 が未達 → S-1 計画（現 s2-fast-kanji-m5-plan.md §10）へ
✅ S1〜S5a   速度の作り直し（第 1 波）    テンソル検索 / 量子化 / GELU / blob v2 / PIE ループ（M-81。QEMU 命令数比 −49%）
✅ T1〜T5    速度の作り直し（第 2 波）    末尾 pull / S9 / S6 / arena / GELU のコード生成 → **満チャンク xRT 0.497**（M-88）
✅ S5b       weight-stationary の PIE   **0.446**（M-90）。**要件 RTF ≤ 0.5 を満たした**
✅ D-048     W8A8+PIE の既定化          ESP32-S3 では既定にする（W8A32 は xRT 4.28〜4.62 で間に合わない）
✅ K トラック 漢字を M5 で              スタックチャンで漢字・カタカナ・ひらがなを喋った（M-90。`!` も要らない）
▶  今        **G32 の対照つき聴取だけ** ざっとした聴取は済み（M-91 / M-93）。計画は s2-fast-kanji-m5-plan.md
```
...
| **速度** | ✅ **達した**（2026-09-03）。第三者の実機報告 1.554× RT（未再現）→ 自分の実測 0.926（M-82）→ **満チャンク 1 pull の xRT 0.446**（M-90。要件 ≤ 0.5）。アンダーラン 0、鳴らし始め **384 ms**（M-90。かな構成は 432 ms = M-89）。⚠️ **発話全体では 0.541〜0.712 でまだ 0.5 超**（要件の分母が未定）。経緯は [`s2-fast-kanji-m5-plan.md`](s2-fast-kanji-m5-plan.md)（§10 に S-1 の前史） |
| **音** | ⚠️ **誰も聴いていない**（G32）。**これが最後の 1 つ。** |

⚠️ **fp32 では最初から届かなかった。** 実測 η_host=0.364 を転移すると移植可能 C の fp32 は
**2.47× RT で実時間に間に合わない**（η=1 の下限 0.93 だけ見ると「あと少し」と誤読する）。
論文の 0.22× RT も **fp32 では達成不可能**で、**int8 + PIE が必須**だと検算できた。
✅ **実際、要件を満たした構成は W8A8 + PIE である**（D-048）。⚠️ ただし **PIE だけでは足りなかった** —
実機の内訳では積和以外（量子化 / GELU / テンソル検索 / 重みのコピー）が同等以上で（M-80）、
**捨てる出力を計算しない・token を持ち越す・GELU のコード生成を直す**方が効いた（M-88）。
...
[`../decisions.md`](../decisions.md) の **D-014**（⚠️ かつて `phase-a-decisions.md` にあった一次記録は
2026-09-03 に D-014 へ統合し、ファイルは削除した。**A-2 が反証を通らないまま結論ごと入れ替わった経緯**もそこにある）。
...
#### D-3 の結果（a/b/c/c' 完了。**d = 実機も M-82 → M-90 で完了**）
...
| **D-3c'** | **PIE 最適化** | ✅ **完了**（M-57 / M-58）。`ee.vmulas.s8.accx` のアセンブリで **MAC の 99.40%**、QEMU で **bit 完全一致**。✅ **速度も測り**（M-82 → M-90）、**既定にした**（D-048）。⚠️ S5a / S5b でさらに書き直した（s2 計画 §10-3 / §5） |
| **D-3d** | 実機 | ✅ **測った**（M-82 / M-83。ユーザーの CoreS3 = D-047） |
...
| **c'-3** | PIE intrinsic 版のカーネル | **正しさのみ**（C 版と bit 一致 or 高 SNR） | ✅ **速度も測った**（M-82 → M-90。実機 CoreS3） |
| **c'-4** | ESP-IDF プロジェクト雛形 | ✅ CMake 構文 / ホスト stub ビルドで **C コアと bit 一致** / コアにホスト専用 API が無いこと / arena の実測（M-46） | ⚠️ **当時の記述**。`idf.py build` は M-54 で通り、QEMU 完走は M-62。→ ✅ **実 SRAM と実 xRT は測った**（M-82 / M-89 / M-90。arena 静的 180,224 B / used 157,360 B、起動直後の内部 DRAM free 132,039 B、満チャンク xRT 0.446）。⚠️ **I2S の実サンプルレート誤差だけが今も未測定**（ESP32-S3 に APLL が無い。`esp32/README.md` §測り方 11） |
...
> ⚠️ **下の表の「状態」欄は 2026-08-27 当時のもの。** D-3a〜D-3d は全部片づいた
> （FFT = M-43 / レイテンシ = M-43 / int8 = M-45 / **実機 = M-82 → M-90**）。
> 結果は [`s2-fast-kanji-m5-plan.md`](s2-fast-kanji-m5-plan.md) と M-82〜M-90 を見ること。
...
| ~~**ESP32 で実時間に間に合うか**~~ | ~~残る唯一の成否要因~~ | ✅ **満チャンク 1 pull の xRT 0.446**（M-90。W8A8+PIE = D-048。要件 ≤ 0.5）。当時の見立て「fp32 のまま移植すると 2.47× RT、**int8 + PIE が必須**」は正しかった（W8A32 の実機は 4.28〜4.62 = M-83）。⚠️ **発話全体では 0.541〜0.712 でまだ 0.5 超**（分母は未決 = D-049） |
| **PIE の効果を手元で検証できない** | 書いても正しさしか確かめられない | ⚠️ **正しさは QEMU で確認できた**（bit 完全一致。M-56 / M-57）。**速度は QEMU では測れない**（サイクル精度でない）→ ✅ **実機で測った**（M-82 / M-90）。⚠️ **QEMU の命令数比は実機の比率と別物だった**（C-055） |
| ~~**実機で測れていない**~~ | ~~解析と実機は違う~~ | ✅ **CoreS3 で測った**（M-82 → M-90 / D-047）。arena は実機で静的 180,224 B / used 157,360 B、起動直後の内部 DRAM free 132,039 B（M-89 / M-90）。⚠️ 解析の 196.9 KB は**当時の値**（fp32 一括版の高水位。M-42） |
...
| ~~**D12**~~ | ~~PIE カーネルを実機なしで書くか~~ → **書いた**（M-57 / M-58） | ✅ | ⚠️ **かつて「toolchain が無いので待ち」としていたのは誤り**（C-033）。⚠️ **「本体は W8A8 への書き換え」も誤り**（C-034。W8A8 は既に実装済みだった）。本体は **intrinsic / アセンブリ**で、GCC は `-O2` で PIE へ自動ベクトル化しない（M-53）。QEMU で **bit 完全一致**、MAC の **99.40%** を被覆。✅ **速度も測った**（M-82 → M-90） |
...
## 10. 残りのタスク — ✅ **5 つとも片づいた**（2026-09-03。**残るのは G32 の聴取だけ**）

**品質もメモリも目標に届き、追試 E-1 / E-2 / E-2b も決着し、最後まで残っていた速度も 2026-09-03 に要件を満たした。**

⚠️ **この計画は「かな中間表現の生徒モデル」のトラックだけを扱う。**
端末で漢字を扱う **K トラックは別計画**で、**K-0 〜 K-8 まで完了している**
（**スタックチャン（M5 CoreS3）で漢字を喋った**。M-90）。
→ [`k1-kanji-implementation-plan.md`](k1-kanji-implementation-plan.md)

**続きの計画は [`s2-fast-kanji-m5-plan.md`](s2-fast-kanji-m5-plan.md)**（S-1 計画は同文書 §10 に前史として畳んだ）。

| # | 何 | 状態 |
|---|---|---|
| **1** | **サイクル実測**（速度） | ✅ **測った**。かな = M-82（0.926）→ M-88（0.497）→ **M-90（0.446）**。K = M-83（G28 / G29） |
| **2** | **聴取** | ⚠️ **これだけ残っている**（G32）。**ボードは要らない**（WAV は `reports/k8_listen/` に 12 組。M-78） |
| **3** | 起動から合成まで落ちない / checksum が QEMU と一致 | ✅ **通った**（G30 / G31 = M-83。M5 構成でも M-90） |
| **4** | 出荷ファームで **W8A8 + PIE** を有効にするかの判断 | ✅ **決めた = D-048**（ESP32-S3 では既定。W8A32 は xRT 4.28〜4.62 で間に合わない） |
| **5** | **速度の作り直し** | ✅ **S1〜S5a**（M-81 / D-046）→ **T1〜T5**（M-87 / M-88）→ **T4**（M-89）→ **S5b**（M-90）。**満チャンク xRT 0.446 で要件 ≤ 0.5 を満たした**。⚠️ **S7 / S8 は未着手**（要らなかった） |

⚠️ **速度で 2 回同じ形の間違いをした**（C-054 / C-055）: **QEMU の割合も命令数も実機の速度を予測しない**。
S1〜S5a の「QEMU 命令数比 −49%」は実機の −49% ではなかったし、命令数が 70 → 59 に減った変更で **GELU が 2 倍遅くなった**（M-87）。
**速度の判断は実機の `-DSAAN_PROFILE=1` の表で、マージ直後に前の版と 1 行ずつ並べて行う。**
...
⚠️ **4 は「今すぐ決められる」ように見えて決められなかった。** 有効にする動機は速度なのに、
**その速度が測れるのは実機だけ**だったため。材料（M-55 / M-57 / M-58）は揃っていた。
→ 板が来て測れるようになり、**D-048 として決着した**。

### ⚠️ ボードが手に入ったら、**2 トラック分をまとめて測る** — ✅ **そうした**（D-047 / M-82 / M-83）

同じボードで両方測れる。**別々に取りに行くと 2 回焼き直すことになる。**
⚠️ **実際には 2 回どころではなかった** — 速度の変更は入れるたびに焼き直しが要るので、
2026-09-02〜03 だけで M-82 / M-84 / M-86 / M-87 / M-88 / M-89 / M-90 の 7 セッションになった。

| | かなトラック | K トラック |
|---|---|---|
| 焼くもの | app + `model`（8 MB の表で足りる） | app + `model` + **`dict` 13,702,320 B**（**16 MB の表**） |
| 測ること | 定常 xRT / アンダーラン / PIE の効き | G2P レイテンシ / 定常 xRT / 完走 |
| bit 一致の確認 | かな経路 `0x78c209af06affc01`（⚠️ **S3 = erf 近似より前の値**。現行は W8A8+PIE `0xa69a7ebbb5ccb05f` / W8A32 `0xe4b645c30835d42d`） | 同上（**実際に同じ値になった**。M-83 / M-90） |
...
✅ **実機は CoreS3（16 MB flash / PSRAM 8 MB Quad）だった**（D-047）。
⚠️ **PSRAM の有無で構成が変わる。** `kj_alloc()` は PSRAM を優先するので、
**bit 一致を主張するなら先に PSRAM 無しで測る**。
→ ✅ **PSRAM 有効でも checksum は変わらなかった**（M-90。作業領域の置き場所は変わるが出力は同じ）。
⚠️ ただし **PSRAM 有効の M5 構成では辞書の mmap が別問題を出した** — `CONFIG_SPI_FLASH_ROM_IMPL=y` の
ROM 実装は 128 ページ（8 MB）しか貼れず、辞書 211 ページが入らない。`esp_mmu_map` で解決（M-90 §4）。
...
→ ✅ **実機のコンソール（USB Serial/JTAG）で検証した**（M-83 / M-90。CoreS3 の `かな>` に行を打ち込んで合成した）。
⚠️ **UART0 では未検証**で、**v0.2.0 の配布イメージは UART0 入力**のため native USB だけの板には入力が届かない（M-83 §1）。
速度も M-82 → M-90 で実測済み（満チャンク xRT 0.446）。
...
| # | タスク | いま着手できるか | 状態 |
|---|---|---|---|
| **P-1** | **PIE カーネル** | ✅ **実装完了**（M-57 / M-58）＋ **QEMU で全経路 bit 一致**（M-62） | ✅ **実機で測った**（M-82 → M-90）。**出荷ファームの既定にした**（D-048）。⚠️ **PIE だけでは足りず**、積和以外を削って初めて要件に届いた（M-88） |
...
| ~~P-3~~ | ~~端末でかなを自由入力する経路~~ | ✅ **実装完了**（M-63 / D-040）＋ **QEMU の UART で実測** | ✅ **実機のコンソール（USB Serial/JTAG）で検証した**（M-83 / M-90）。⚠️ **UART0 では未検証**（v0.2.0 の配布イメージは UART0 入力で、native USB の板には届かない = M-83 §1）。手順は `esp32/TESTING.md` |
| ~~P-4~~ | ~~外の人が実際に動かせる状態にする~~ | ✅ **完了**（M-64 〜 M-67 / D-041） | piper-plus も教師も無しで合成可。**焼くだけの firmware** を v0.1.1 で配布。⚠️ 手順書の欠陥 2 件を修正（M-66） |

### P-1: PIE (SIMD) カーネル — **着手した。ただし想定より大きい**（✅ **決着済み**。以下は当時の記録）

**当時の唯一の未達項目。** fp32 のまま移植すると **2.47× RT** で実時間に間に合わない（M-43）。
✅ **決着**: W8A8 + PIE を既定にし（D-048）、そのうえで積和以外を削って **満チャンク xRT 0.446**（M-90）。
...
⚠️ **それでも実機のサイクル数は測れない**（当時は ESP32-S3 ボードが無かった）。
できるのは **コンパイル** と **逆アセンブルでの PIE 命令の確認**、命令数の計数まで。
→ ✅ **2026-09-02 に板が来て測れるようになった**（D-047）。⚠️ **そして「命令数」は速度の証拠にならないと分かった**（C-055 / M-87）。
...
| **出荷ファームでの有効化** | ✅ **既定にした**（**D-048**。ESP32-S3 ではフラグ無しで W8A8 + PIE が有効） |
| 実機でのサイクル実測 | ✅ **自分で測った**（M-82 で 0.926 → M-90 で **0.446**。CoreS3 = D-047）。⚠️ **第三者の 1.554 そのものは今も未再現**（S1 前の値） |
...
**P-1 の実装は終わった。⚠️ ただし当時は「速くなった」とは言えなかった** —
第三者の実機報告（2026-09-02）は **1.554× RT** で、M-43 の外挿 0.088× RT より **18 倍遅い**。
原因は積和ではなく量子化 / GELU / テンソル検索 / 重みのコピー（M-80）。**S1〜S5a で削った**
（M-81。QEMU 命令数比 −49%）。
→ ✅ **実機で測ったら −40%**（1.554 → **0.926**。M-82）。そこから T1〜T5 / 64 B 行で **0.497**（M-88）、
S5b + 漢字構成で **0.446**（M-90）。⚠️ **QEMU の命令数比は実機の比率を予測しなかった**（C-055）。

### ✅ 出荷ファームで有効化するかの判断 — **決着した**（**D-048**。以下は当時の材料）

**結論: ESP32-S3 では W8A8 + PIE を既定にする**（フラグ無しのビルドで有効）。
決め手は速度で、**W8A32 は実機で xRT 4.28〜4.62** と実時間に遠く届かない（M-83）のに対し、
W8A8 + PIE は **0.446**（M-90）。**知覚差は無い**（M-55）ので払う代償が無い。
⚠️ arena も収まった（M-89。静的 180,224 B に対し used 157,360 B）。
...
PIE を入れる段で 1 に切り替える。~~ → **D-048 で切り替えた**（ESP32-S3 では既定 1）。
**SNR ゲートは落ちる**（平均 23.24 dB）が、
