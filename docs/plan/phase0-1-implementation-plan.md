...
✅ Phase 5   β スイープ                 β=0 と 2 が候補。⚠️ **聴取待ち**（M-40）
...
| 軸 | 状態 |
|---|---|
| 品質 | ✅ SCOREQ 教師比 **0.611**（論文の英語比 0.5427 超、M-37）。⚠️ n=24 / 聴取は未実施 |
| メモリ | ✅ **196.9 KB / SRAM 512 KB の 38%**（stack 込み、M-42 / M-43） |
| **速度** | ⚠️ **ここだけ未達。** 手元 0.023× RT だが、**fp32 のまま移植すると 2.47× RT** |
...
| **D-3c'** | **PIE 最適化** | ▶ **次。** esp-dsp 級の intrinsic/asm が要る |
| **D-3d** | 実機 | ⚠️ ハードウェア待ち |
...
| リスク | 影響 | 状態 |
|---|---|---|
| **集約指標が金属的アーティファクトを検出できるか** | ⚠️ **未測定（DNSMOS を測っていない）** | 上流いわく「metallic は SCOREQ で**高**得点・DNSMOS で低得点」。`speechmos` で DNSMOS を足す（追試 E-1） |
| **decoder をゼロから学習していること** | ⚠️ **未検証。上流の主張と噛み合わない** | 上流は「from-scratch な sub-400k decoder は死んだクラス」。うちは 331,308 params を from-scratch で教師比 0.611（追試 E-2） |
| **ESP32 で実時間に間に合うか** | **残る唯一の成否要因** | ⚠️ 手元 0.023× RT だが、**fp32 のまま移植すると 2.47× RT**（実測 η_host=0.364 を転移）。**int8 + PIE が必須**（D-3c'） |
| **PIE の効果を手元で検証できない** | 書いても正しさしか確かめられない | ⚠️ Apple の SIMD は fp32 向きで int8 が速くならない。**実機待ちにする判断もある**（D12） |
| **実機で測れていない** | 解析と実機は違う | ⚠️ ハードウェア待ち（D-3d）。RAM は解析で 196.9 KB / SRAM 512 KB |
| **int8 カーネルが未実装** | blob は 625 KB だが C は fp32 で読んでいる | D-3c |
| **UTMOS と SCOREQ が生徒の劣化で食い違う** | 指標の読み方を誤ると判断を誤る | ⚠️ v1 で UTMOS 比 0.700 / SCOREQ 比 0.387 と乖離した。**主指標は SCOREQ**（D-020） |
| **`β` が聴取で決まっていない** | 式7 を入れるか外すか | ⚠️ 候補は β=0 と 2（M-40）。**指標では決めない**。聴取セットは用意済み |
| **アクセント型の再現性が未測定** | 日本語 TTS の核 | ⚠️ ミニマルペア（橋/箸/端）を評価セットに入れていない。**入れないと検出できない** |
...
### Phase 5: 摩擦音ノイズ注入 `β` の決定（式7）— ⚠️ 聴取待ち（M-40）
...
| ID | 判断 | 期限 | 状態 |
|---|---|---|---|
| **D7** | `β`（式7 のノイズ注入強度） | Phase 5 | ⚠️ **聴取待ち。** 候補は β=0 と 2（M-40。sweep は 0/2/4/6/8 を回した）。`reports/listening_beta/` に 40 試行の A/B セットがあるが **1 名分**。**指標では決めない**。⚠️ **上流（英語）は β=6.0 を採用**している（`../upstream-sanotts.md`）。うちの J 指標では 6 は候補に残らなかった。**言語差か、指標の違いか、切り分けていない** |
| **D12** | **PIE カーネルを実機なしで書くか** | D-3c'-3 | ⚠️ **実質「待ち」で確定。** 速度が測れないだけでなく、**この環境に xtensa toolchain が無いのでコンパイルすら通せない**（`idf.py` 不在 / `IDF_PATH` 未設定 / `~/.espressif` 無し）。書いても「構文が通ることすら未確認のアセンブリ」にしかならない |
| ~~**D4**~~ | ~~アクセント記号トークンのみか、A1/A2/A3 を足すか~~ → **記号だけで足りる**（D-030） | ✅ | ミニマルペア 15 群 32 語で符号一致 **35/36**（M-44）。⚠️ **聴取はしていない** |
| **D13** | **DNSMOS を評価指標に加えるか** | 追試 E-1（別レーン進行中） | ⏳ **M-49 に相乗りで測った（別経路）。** 実人間の天井を先に取り、6 レーンを同じ発話集合で測定。**教師/人間 0.989**（SCOREQ 0.820 / UTMOS 0.758 と大きく違う）。⚠️ 上流の言う「SCOREQ で高・DNSMOS で低」のパターンは**出ていない**。⚠️ `speechmos` は UTMOS の torch.hub checkout と同名で衝突するので**別プロセス**で測る |
...
| ~~アクセント型の再現性~~ → **決着（M-44 / D-030）** | ✅ 符号一致 **35/36** | ⚠️ **聴取していない**。2 型の下降核は n=13〜16 でしか測れていない |
...
## 10. 残りのタスク（2026-08-28 現在）

**品質もメモリも目標に届いた。残りは 4 本だけ。** うち 2 本は待ち、2 本は今できる追試。

| # | タスク | いま着手できるか | 何が分かるか |
|---|---|---|---|
| **P-1** | **PIE (SIMD) カーネル** | ❌ **toolchain 待ち** | ESP32 で実時間に間に合うか（**唯一の未達項目**） |
| **P-2** | **`β` の聴取決定** | ⚠️ **人が要る** | 式7 が日本語で要るのか |
| **E-1** | **DNSMOS を測る** | ✅ **完了（M-49 / M-50）** | 金属的のパターンは**出ていない**。教師/人間 OVRL 0.979。⚠️ **DNSMOS が反応しない劣化が実在する**（SNR 10.5 dB のクリップ・位相破壊）|
| **E-2** | **decoder の教師初期化** | ⚠️ **設計判断が先** | 論文どおりの `Gγ` が正しいのか |

### P-1: PIE (SIMD) カーネル — **待ち**

int8 カーネルは書けたが**移植可能 C** で、ESP32-S3 の PIE を使うには intrinsic か
アセンブリが要る。⚠️ **この環境に xtensa toolchain が無く、コンパイルすら通せない**
（`idf.py` 不在 / `IDF_PATH` 未設定 / `~/.espressif` 無し）。

**書いても「構文が通ることすら未確認のアセンブリ」にしかならない**ので、
D12 は実質「待ち」で確定した。着手条件は **ESP-IDF v5.x の導入**（約 2〜3 GB）か**実機の入手**。

⚠️ **fp32 のまま移植すると 2.47× RT で間に合わない**（M-43）。
上流は同じ ESP32-S3 で **int8 + PIE の 0.22× RT を実測**と申告しており
（`../upstream-sanotts.md`）、**int8 + PIE が必須**という結論は補強された。

### P-2: `β`（式7）の聴取決定 — **人が要る**

候補は **β=0 と 2**（M-40。sweep は 0/2/4/6/8 を回した）。
A/B セットは `reports/listening_beta/` に 40 試行あるが **1 名分**。日本語母語話者 10 名以上が理想。

⚠️ **指標では決めない**（論文も同様。集約 SCOREQ はむしろ下がる）。
⚠️ **上流（英語）は β=6.0 を採用している。** うちの J 指標では 6 は候補に残らなかった。
**言語差なのか指標の違いなのかは切り分けていない。**
⚠️ この生徒は **β=0 で既に教師と一致**しており、式7 自体が要らない可能性がある。

### E-1: DNSMOS を評価に足す — **今できる**

**なぜやるか**: 上流は
*"a metallic artifact scores high on [SCOREQ/UTMOS] and low on DNSMOS"* と書いている。
うちの主指標は SCOREQ synthetic/nr + UTMOS（D-020）で、**どちらも自然さの予測器**。
**金属的な尾があっても現在の指標構成では原理的に見えない。**

これは B-6 で摩擦音の欠陥を見つけたときと**同じ構図**（集約スコアが盲目で、
分解能を上げたプローブが見つけた）。

| 項目 | 内容 |
|---|---|
| 導入 | `uv add speechmos`（PyPI 0.0.1.1。DNSMOS P.835 を含む）。⚠️ `pip` は使わない |
| 測る対象 | **教師 / 生徒 / 実人間**の 3 レーン。D-013 に従い**教師比と人間比の両方**で報告 |
| 併記 | SCOREQ・UTMOS と**同じ発話集合**で測り、3 指標を並べる |
| ラッパ | `src/saanotts_jp/scoreq_metric.py` に倣って `src/saanotts_jp/` に置く |
| 合否 | ⚠️ **決めない。** DNSMOS も日本語で較正されていない（UTMOS / SCOREQ と同じ問題、D-013） |

#### 結果 — **完了（M-49 で相乗り測定 → M-50 で独立に測り直し + 対照実験）**

| 指標 | 教師 | 実人間 | 生徒 | 教師/人間 |
|---|---:|---:|---:|---:|
| DNSMOS OVRL | 2.7299 | 2.7881 | 2.1088 | **0.979** |
| SCOREQ | 2.0488 | 2.4983 | 1.2063 | 0.820 |
| UTMOS | 1.7479 | 2.3047 | 1.3585 | 0.758 |

**上流の言う「metallic は SCOREQ で高・DNSMOS で低」のパターンは出ていない。**
生徒は 4 スコアすべてで教師より低く、SCOREQ と同じ順序で下がる。
⚠️ これは**事前登録どおり**（生徒には既に過平滑が測られており、それだけで SIG は下がる）。
**「生徒の DNSMOS が低い」は金属的の証拠にならない。**

E-1 で新たに分かった、**指標そのものの限界**（M-50、対照実験）:

- ⚠️ **SNR 10.5 dB のハードクリップに 4 スコアとも反応しない**（むしろ上がる）。
  陽性対照 G6 は **4 系統中 SIG 2/4・OVRL 3/4** しか単調にならず、**FAIL**
- ⚠️ **位相破壊（Griffin-Lim, SNR −2.7 dB）にほぼ反応しない** —
  同 SNR の白色雑音より OVRL で **1.33〜1.53 ポイント甘い**。
  **「DNSMOS なら金属的を捕まえられる」という素朴な期待は成り立たない**
- ✅ **唯一 SIG だけがフレームレート 86.13 Hz の AM に選択的**（雑音より厳しい）。
  「金属的」がフレーム境界の周期性を指すなら、**見るべきは OVRL ではなく SIG**
- ⚠️ **`p808_mos` は前処理に弱い**（リサンプラで 0.066 / 自己連結 vs ゼロ埋めで 0.22 動く）。
  生徒差 0.27 と同オーダーなので**切り分けられていない**
- ✅ M-49 の人間天井は**独立に再現できた**（torchaudio 経路で \|Δ\| < 4e-5）

⚠️ **合否は決めなかった**（D-013）。**併記プローブとして採用**（D-034）。

### E-2: decoder の教師初期化 — **まず設計判断**

**なぜ引っかかるか**: 上流は
*"Teacher-init the decoder. It survives channel-pruning; from-scratch sub-400k decoders
are a dead class."* と断言している。
**うちの `Gγ` は 331,308 params の sub-400k で、しかもゼロから学習して
SCOREQ 教師比 0.611（論文の英語 0.5427 超）を出している。** 噛み合わない。

⚠️ **そのままでは実行できない。トポロジが違う。**

| | 教師 `MBiSTFTGenerator` | 生徒 `Gγ`（論文 Table I の逆算） |
|---|---|---|
| 構造 | `conv_pre` → `ConvTranspose1d` ×2 → `ResBlock2` → PQMF 4 サブバンド合成 | 深さ方向分離 conv ×5 + **rank-12 FiLM** + 分解出力ヘッド |
| 時間方向 | **アップサンプルする**（upsample_rates 4,4） | **しない**（フレームごとに 1539ch のスペクトルを直接出す） |
| 幅 | `upsample_initial_channel = 256` | `W=76 / E=304` |

**チャネル切り出しで初期化するには、`Gγ` を「教師を細くしたコピー」にする必要がある**
— つまり**論文 Table I の 331,308 params を捨てる**ことになる。

⚠️ **「切り出しができない」は「形が合わない」という意味ではない。**
実測すると **28 本の conv 重みのうち 27 本は形状として切り出せる**が、
合致先は定数 FiLM（`cond` / `cond_layers`。教師では発話ごとに定数の 512 次元 g を受ける）か
**別フレームレートの ResBlock** で、**意味が対応しない**。
`hout (1539, 48, 1)` だけは教師側に候補がゼロ。
再現: `uv run python reports/r3_slice_matrix.py`（教師 dec の実効 params 2,108,208）。

したがって E-2 は「重みをコピーする作業」ではなく、**次の 3 択を切り分ける調査**:

| 仮説 | 判定（M-49 / 2026-08-28） |
|---|---|
| **(a) 論文の `Gγ` 逆算が間違っている** | ⚠️ **決着しない。** (a1)（算術）は padding と iSTFT 支援表を宣言して測り直すと、**合計 679,832 B は align-64 なら到達可能**で決まらない（C-027）。(a2)（それが品質を律速するか）は**未測定**。⚠️ **iSTFT 表現が天井ではない**ことだけは確定（`istft(stft(yT))` が教師比 1.0033） |
| **(b) 上流の主張が上流の構成に固有** | **部分的に支持。** 論文の「sub-400k は死んだクラス」の根拠は **357k の R8（波形 transposed-conv / 522 MMAC/s）**の測定で、**331k iSTFT decoder に教師潜在を通したセルは論文に無い**。うちがそのセルを埋めて教師比 **0.802**（R8 の 0.684 と 1.0M teacher-init の 0.883 の間）。⚠️ 言語・教師・レシピが違うので決定的ではない |
| **(c) 日本語では from-scratch でも足りる** | **支持されない。ただし「decoder が主因」でもない。** 鎖分解では decoder 0.395 > acoustic 0.283 > duration 0.052（すべて有意）。**論文流の置換定義では acoustic 0.508 > decoder 0.395 で順序が逆**。⚠️ **かな CER の劣化は decoder の段だけ**（+0.039） |

**40 次元 c-line 自体のコストは 0.024 しかない**（教師 decoder に線形 lift 経由で
`Eρ(zT)` を通した T レーン）。したがって鎖分解の decoder gap は `Gγ` に帰属する。

⚠️ **「金属的な尾が出たら (a) の証拠」は撤回した**（C-026）。
尾が出ても「うちの decoder が不十分」であって「うちの逆算が間違い」ではない。
実際 DNSMOS では尾のパターン自体が出ていない（E-1 / M-49）。

**残る 1 手**: `Gγ` が**容量律速か**を測る幅スイープ。
Stage 3 だけで W ∈ {76（2 seed の対照）, 96, 128} × 20,000 step、**約 2 時間**。
⚠️ **これは「教師初期化 vs from-scratch」の実験ではない**（初期化はトポロジ差で
定義できない）。測れるのは容量律速かどうかだけ。⚠️ **診断専用。デプロイ候補を作らない。**
⚠️ 合否条件: W=76 の対照が M-37 の Stage 3 val SNR（20,000 step で 8.91 dB、以降 8.0〜8.4）
を再現し、**seed 間の差 < 幅の効果**であること。満たさなければ幅の比較を報告しない。

⚠️ **上流のソースコードは読まない**（GPL-3.0 / `../upstream-sanotts.md`）。
公開ドキュメントの記述と、うち自身の実測だけで切り分ける。
