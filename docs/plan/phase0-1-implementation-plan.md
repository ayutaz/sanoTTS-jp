...
✅ E-1      DNSMOS                     生徒/教師 0.7725。**上流の主張は再現せず**（M-50 / D-034）
...
▶  今できる   E-2b `Gγ` の幅スイープ       decoder gap が**容量律速か学習不足か**（約 2 時間）
...
| 軸 | 状態 |
|---|---|
| 品質 | ✅ SCOREQ 教師比 **0.611**（論文の英語比 0.5427 超、M-37）/ DNSMOS 教師比 **0.7725**（M-50）。⚠️ n=24 / **聴取は未実施** |
...
**品質もメモリも目標に届き、追試 E-1 / E-2 / E-2b も決着した。残りは 3 本。**
うち 2 本は外部待ち、1 本だけ今できる。
...
| # | タスク | いま着手できるか | 状態 |
|---|---|---|---|
| **P-1** | **PIE (SIMD) カーネル** | ❌ **toolchain 待ち** | **唯一の未達項目**。fp32 のままでは ESP32 で 2.47× RT |
| **P-2** | **`β` の聴取決定** | ⚠️ **人が要る** | 候補 β=0 と 2。⚠️ 上流（英語）は 6.0 |
| ~~E-2b~~ | ~~`Gγ` の幅スイープ~~ | ✅ **決着**（M-52） | **容量律速ではない。学習律速。** ⚠️ W=56 に落とせるかだけ保留 |
| **E-2c** | **W=56 が無料かの確認**（40k のノイズ床 `w76b_40k`） | ✅ **今できる**（40 分） | 速度 P-1 に直結 |
...
### ▶ E-2c: W=56 が無料かの確認 — **今できる（40 分）**

W=56 は params **−34%**（331,308 → 219,828）で 40k の Δ が **+0.0632**
[+0.0501, +0.0760]。20k で測った床 0.0812 より小さいが、**CI は 0 を含まない**。

⚠️ **40k のノイズ床を測っていない。** 40k の Δ を 20k の床と比べるのは単位違いで、
**学習を延ばすと run 間ばらつきは縮む可能性が高い**（縮めば +0.0632 は有意な劣化）。

| 項目 | 内容 |
|---|---|
| やること | `w76b_40k`（W=76 / 40k / seed 違い）を 1 本回して **40k の床**を出す |
| コスト | **約 40 分** |
| 判定 | +0.0632 が 40k の床以下 → **W=56 は無料** → P-1 に直結 / 床超え → W=76 据え置き |

**なぜ効くか**: decoder は推論時間の **59.9%**（M-43）で、`pw1`/`pw2` は `W²` で効く。
W=56 なら `pw1+pw2` が 232,940 → 126,840（**−46%**）。**唯一の未達である速度に直接効く。**
