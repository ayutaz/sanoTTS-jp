...
✅ B-0〜B-11 検証タスク完走             D-016〜D-025 として凍結（下表）
...
**B-0 〜 B-11 はすべて決着した（2026-08-27）。**

| 状態 | タスク |
|---|---|
| ✅ 解決済み | **B-0 〜 B-11 の全部**。B-1 / B-2 は Phase A の設計判断で構造的に消え、残りは実測で決着した |
...
| **D13** | **DNSMOS を評価指標に加えるか** | 追試 E-1 | ⚠️ **未測定。** 上流は「金属的アーティファクトは SCOREQ で**高**得点・DNSMOS で低得点」と書く。現在の指標構成（D-020）では**原理的に見えない欠陥がありうる**。`speechmos` (PyPI 0.0.1.1) で測れる |
| **D14** | **decoder を教師で初期化するか** | 追試 E-2 | ⚠️ **未検証で、そのままでは実行できない。** 上流は「from-scratch な sub-400k decoder は死んだクラス」と断言するが、**うちの `Gγ` は教師とトポロジが違うのでチャネル切り出しができない**（下記 E-2） |
...
| **E-1** | **DNSMOS を測る** | ✅ **今できる** | 現在の指標が見落としている欠陥があるか |
...
- **DNSMOS が日本語で較正されているか**は未確認。**まず実人間の日本語音声で天井を測る**
  （UTMOS で天井を測らずに絶対値を英語と比べて判断を誤った前例がある。C-012）
- DNSMOS は内部で **16 kHz** にリサンプルする可能性が高い。
  そうなら**摩擦音プローブ（2–8 kHz）の代わりにはならない**。両方が要る
- **「SCOREQ と DNSMOS が食い違った」ときにどちらを採るかを先に決めない。**
  食い違い自体が情報（上流もそう書いている）
...
| 仮説 | 検証の仕方 |
|---|---|
| **(a) 論文の `Gγ` 逆算が間違っている** | int8 blob2 が論文比 **−9.7%**（語彙と無関係、M-39）という既知の食い違いと同根か調べる。§6 の逆算をやり直す |
| **(b) 上流の主張が上流の構成に固有** | 上流の decoder は ~1.0M（quality tier）。**sub-400k で「死んだクラス」と言っているのは 745k tier の話ではない**可能性。上流の公開文書だけで切り分けられるか確認する |
| **(c) 日本語では from-scratch でも足りる** | 教師比 0.611 は既に論文の英語比 0.5427 を超えている。**上限を測っていないだけ**かもしれない |

⚠️ **(a) が当たりなら Gγ の再設計になり、学習をやり直すことになる。**
先に **E-1（DNSMOS）を済ませる**のが順序として良い —
