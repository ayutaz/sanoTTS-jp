# saanoTTS-jp Phase 0 / Phase 1 実装計画

- 作成日: 2026-08-26
- 対象: `arXiv:2608.21378` の蒸留レシピの日本語適用（PoC・非配布）
- 上位ドキュメント: [`../research/saanotts-jp-feasibility.md`](../research/saanotts-jp-feasibility.md) / [`../../CLAUDE.md`](../../CLAUDE.md)
- 本文書の位置づけ: **5 チームの調査 + それぞれへの反証検証を通したあとの、実行可能な作業計画。**
  反証された主張は採用しない。未確定のものは `⚠️ 未検証` を付け、着手前に潰すタスクを §2 に置く。
...
### B-5 教師音声の品質ベースライン ⚠️ 未検証

**採用しない**: 「教師 wav が SCOREQ 2.06 / UTMOS 1.62 で論文の 4.68 / 4.42 から乖離しているので、SCOREQ < 3.5 なら Phase 0 に戻る」。
**反証**: 公開 canonical ONNX（SHA-256 `5289e9b6…` 一致確認済み）で同じ 8 文を合成すると **SCOREQ 2.341 / UTMOS 1.989**（決定的推論）。ckpt 教師（2.142 / 2.021）とほぼ同じ。**低い絶対値は推論条件の不具合ではなく、SCOREQ/UTMOS × この話者 × 日本語の性質。** この閾値ゲートは無条件発火して存在しないバグ探しを始めさせる。

**代わりに置くゲート**:
1. **日本語の既知良品リファレンスを測る**（つくよみちゃんコーパスの原収録音声 1〜3 本を SCOREQ/UTMOS に通す）。教師比の分母が健全かはこれで初めて判断できる。
2. **対照は論文の絶対値ではなく公開 canonical ONNX**。教師 ckpt の SCOREQ が公開 ONNX の ±0.3 以内なら健全とみなす。

**完了条件**: `reports/b5_teacher_baseline.json` に `{natural_ref, ckpt_teacher, public_onnx}` の 3 系列が入り、`|ckpt - onnx| < 0.3` が成立。
...
### B-11 `.venv` の stale install ⚠️（対策の一部が反証済み）
...
| ID | 判断 | 期限 | 影響範囲 |
|---|---|---|---|
| **D1** | **フロントエンド: `MultilingualPhonemizer` か言語ピン留めか** | **Phase 1 着手前（最優先）** | pool の 5.36% のラベル。評価セットの供給元。英数字混在軸の設計。並行作業の `gen_teacher_labels.py` |
| **D0** | ターゲット tier: 567 K (c-line) か 1.4 M (z-line) か | Phase 2 着手前 | 全モデル設計。推奨は 1.4 M を先行 |
...
| **SCOREQ の日本語での単調性** | ⚠️ 未検証 | 日本語 MOS 付きコーパス（JVS-MuSiC 等）で順位相関を一度確認 |
