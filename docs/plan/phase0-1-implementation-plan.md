...
- 作成 2026-08-26 / 最終更新 2026-08-27（Phase D の C99 コア完了時点）
...
✅ Phase D-2 ストリーミング化            **1,258 KB → 197 KB。一括版と bit 一致**（M-42）
▶  次        Phase D-3 FFT 化 + int8      **レイテンシはまだ一度も測っていない**
```

**プロジェクトの主目的（ESP32 で日本語 TTS が動く）に対する現在地**:
品質は目標に届き（M-37）、**メモリも SRAM に載った**（197 KB / 512 KB、M-42）。
**残るのはレイテンシだけ** — `irfft` が naive DFT なので一度も測っていない（Phase D-3）。
...
| # | 条件 | 結果 |
|---|---|---|
| G1 | ピーク < 200 KB | ✅ **197.0 KB**（実用最大 350 ids） |
...
**現在の実装は 1.26 MB 使い、ESP32-S3 の SRAM 512 KB の 246%。載らない。**
M-16 の 96 KB は I2S 逐次出力の前提で、いまは発話全体をバッファしている。

| 項目 | 現在 | ストリーミング後 |
|---|---:|---:|
| mag + cos + sin | 637 KB | **6 KB**（1 フレーム分） |
| iSTFT の acc + wsq | 218 KB | **10 KB**（リングバッファ） |
| pcm | 106 KB | **0**（I2S に流す） |

⚠️ **フレーム単位には割れない。** 時間方向のカーネルがあるため受容野は
**decoder ±16 / acoustic frame ±20 フレーム**（実測。**旧記述の acoustic ±10 は
`c1` だけ数えた誤り**）。

**方式は D-029 で確定: ステート保持型ストリーミング（chunk=8）。**
各 conv 層が左コンテキストを保持してチャンク間で繋ぐので**再計算ゼロ**。
ハロー再計算方式は同じメモリで計算が 10 倍になるので採らない。

| 方式 | ピーク | 計算量 |
|---|---:|---:|
| 一括（現状） | 1,258 KB | 1.0x |
| ハロー再計算 chunk=8 | 192 KB | 10.0x |
| **ステート保持 chunk=8** | **97 KB** | **1.0x** |

⚠️ 代償は**レイテンシ 36 フレーム = 0.42 秒**（受容野ぶんの遅延）。

⚠️ **`irfft_1024` が naive DFT（O(N²)）。** 正しさ優先でそうしてあるので、
**このままではレイテンシを測れない**。radix-2 FFT に差し替える。

**完了条件（D-029）**:

| # | 条件 |
|---|---|
| G1 | ピーク RAM < 200 KB（arena の高水位で測る） |
| G2 | 一括版と **bit 完全一致**（`memcmp`）。⚠️ **SNR では不可** — 1 フレーム落としても 20 dB 出る |
| G3 | 発話長に対して RAM が **O(1)**（10 / 100 / 1000 フレームで高水位が変わらない） |
| G4 | 既存の golden test が通り続ける |

**int8 カーネルと FFT 化はここでやらない**（同時に変えると切り分けができない）。

#### D-3 未着手: int8 カーネル + 実機

blob は 624,692 B に落ちている（M-39）が、**C コアは fp32 で読んでいる**。
int8 のまま演算するカーネルが要る。⚠️ 実機測定はハードウェア待ち。

### 主要リスク

| リスク | 影響 | 状態 |
|---|---|---|
| **ESP32 の RAM に載らない** | **プロジェクトの成否に直結** | ⚠️ **現在地: C コアが 1.26 MB / SRAM 512 KB の 246%。** ストリーミング化で 6+10 KB に落ちる見込みだがチャンク処理が要る（Phase D-2） |
| **レイテンシが測れていない** | 0.22× RT に届くか未知 | ⚠️ `irfft` が naive DFT (O(N²))。**FFT 化するまで測る意味が無い**（Phase D-2） |
| ~~567 K で日本語の品質が届くか~~ | ~~成果物の価値に直結~~ | ✅ v2 で SCOREQ 教師比 **0.611**、論文の英語比 0.5427 を上回った（M-37）。⚠️ n=24 / 聴取は未実施 |
| ~~C99 コアの実装量~~ | ~~参照実装が無い~~ | ✅ 476 行で golden test 通過（M-41） |
...
| **int8 カーネルが未実装** | blob は 625 KB だが C は fp32 で読んでいる | Phase D-3 |
...
| ID | 判断 | 期限 | 状態 |
|---|---|---|---|
| **D7** | `β`（式7 のノイズ注入強度） | Phase 5 | ⚠️ **聴取待ち。** 候補は β=0 と 2（M-40）。`reports/listening_beta/` に 40 試行の A/B セットがある。**指標では決めない** |
| **D9** | ストリーミングのチャンク長 | Phase D-2 | 受容野は decoder ±16 / acoustic ±10 フレーム。チャンク 32 + オーバーラップ 16 が素直だが**未検証** |
| **D10** | int8 カーネルの activation スケール | Phase D-3 | 論文は per-frame。ESP32-S3 の PIE は 16 レーンなので整合が要る |
| **D4** | アクセント記号トークンのみか、A1/A2/A3 3 スカラー (+128 params) を足すか | 未定 | 現状のアクセント再現性を測っていない。⚠️ **ミニマルペア（橋/箸/端）の評価が未実施** |
...
| リスク | 状態 | 潰し方 |
|---|---|---|
| **ESP32 の RAM ピーク** | ⚠️ **C コアが 1.26 MB。SRAM 512 KB の 246%** | ストリーミング化（Phase D-2）。完了条件はピーク < 200 KB |
| **レイテンシ** | ⚠️ **一度も測っていない** | `irfft` を FFT 化してから。naive DFT (O(N²)) では測る意味が無い |
| **教師の事前学習テキスト (497,519 発話 / MOE-Speech) と pool の重複** | ⚠️ 未検査 | `ayousanz/moe-speech-20speakers-ljspeech` の metadata を引いて照合。⚠️ **B-10 で検査したのは FT テキストだけ** |
| **アクセント型の再現性** | ⚠️ **未測定** | ミニマルペア（橋/箸/端、雨/飴）を評価セットに入れる。**入れないと検出できない**（D4 の判断材料） |
