...
> ⚠️ **音は誰も聴いていない。** ⚠️ **発話全体の RTF は 0.541〜0.712 でまだ 0.5 を超える**（要件の分母が未定 = §7 #1）。
...
| 鳴らし始めまで | 719 ms | **432〜434 ms** | 要件なし（§7 #1） | M-82 → M-88 / M-89 |
...
| # | 何 | 状態 | 材料 |
|---|---|---|---|
| 1 | **RTF ≤ 0.5 の分母**を「満チャンク step / チャンク音声長」で確定するか。初回 pull と鳴らし始めまでは別要件にするか | ⚠️ **未決（D-049 は空き番）**。**満チャンクの定義では 0.446 で達成、発話全体では 0.541〜0.712 で未達**（M-90）。ファームは両方をログに出す | requirements §6.2 に遅延の数値要件が無い。鳴らし始めまで 432〜434 ms（M-88 / M-89） |
...
- ⚠️ **音は誰も聴いていない**（G32）。**これが最後の 1 つ。** checksum が M-82 から同一なので**波形は 1 bit も変わっていない**が、
