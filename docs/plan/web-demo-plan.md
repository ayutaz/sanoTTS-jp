...
✅ **G-W7 は実際に欠陥を捕まえた**（[C-080](../decisions.md#c-080)。2026-09-10）: [C-073](../decisions.md#c-073) で §3.1 に **LibriTTS-R / CML-TTS / AISHELL-3** を足したとき、**`web/index.html` の写しを置き去りにした**。**GitHub Pages に置くことも再配布**なので、あのページは帰属義務のある 3 素材を欠いたまま配られていた。⚠️ **手元で回しているゲートには入っていない**（emcc と node が要る）ので、**PR の CI が初めて回した。**（⚠️ **本数は書かない** — 増えるたびに古くなる）
...
（**G-A1** = 3 か所の一致 / **G-A2** = 同梱した全文が記載どおり。陽性対照 7 件。
...
| **ブラウザでの実測** | ✅ **Chrome 152 で測った**（M-95。PCM は node と bit 一致 / 短文 0.008〜0.019 ×RT）。⚠️ **これはゲートではなく手作業**。⚠️ **モバイルと Safari は未測定**。node の値はブラウザの値ではない（C-055） |
| **聴取** | ✅ **両レーンとも聴いてもらった**（M-96。「問題なかった」/ 途切れ無し）。⚠️ **1 名・対照なし・盲検なし**。⚠️ **実機の G32（対照つき）は別物で、まだ残っている** |
...
- **モバイルで測っていない**
- ~~**ブラウザの音を誰も聴いていない**~~ → ✅ **両レーンとも聴いてもらった**（M-96）。
  ⚠️ **1 名・対照なし・盲検なし**で、「問題なかった」は**欠陥が無いことの証明ではない**。
  ⚠️ **実機の G32（`reports/k8_listen/` の 12 組 / `reports/d4_accent/`）は別物で、まだ残っている**
