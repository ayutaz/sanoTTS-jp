...
| # | 内容 | 必要な入力 | 期限 |
|---|---|---|---|
| 1 | **B-1: G2P 経路の選択**（言語ピン留め / 除外 / フォールバック）。⚠️ 入力仕様が中間表現に変わったので、**そもそも問題が消えている可能性がある**。ホスト側変換器がどの経路を通るかを先に確認すること | `reports/b1_routing.json` | ラベル一括生成より前 |
| 2 | 教師品質が低かった場合の期待値再設定 | B-5 の再測定（正しい音素化経路・十分な発話長で） | Phase 1 と並行 |
| 3 | 567 K の日本語品質が実用に足るかの判定基準 | 1.4 M との差分、日本語 CER | Phase 6 |
| 4 | 中間表現で表現できない 3.6% の扱い（アクセント記号がモーラ内部に来るケース） | 原因の切り分け | Phase 1 より前 |

**決着済み**: B-0 の続行判断（D-009 〜 D-011）、ターゲット tier（D-007）、
実行環境（D-012）、ESP32 メモリ（M-16）。
