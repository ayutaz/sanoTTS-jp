...
| # | 内容 | 必要な入力 | 期限 |
|---|---|---|---|
| 1 | **`λ₂ / λ_n / λ_Δ / λ_s`（式3）と `λ_T`（式2）の値** | 論文に記載が無い。学習で探索する | 本学習の前 |
| 2 | **判別器の構造** | 論文は「一次差分に対する判別器」としか書いていない。現状は MSD 相当の推測 | 本学習の前 |
| 3 | **`β`（式7）** | **聴取で決める。** 集約スコアはむしろ下がる（論文 4.09 → 3.92） | Phase 5 |
| 4 | 567 K の日本語品質が実用に足るかの判定基準 | 1.4 M との差分、日本語 CER | Phase 6 |
| 5 | SCOREQ の導入 | 論文の主指標だが pip パッケージが見つからない | Phase C までに |
