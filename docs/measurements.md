...
```bash
./csrc/k7_test <k6 の 438,750 ベクタ> --dump-ids /tmp/ids.tsv
...
<a id="m-113"></a>
## M-113. **v4（CC0/PD のみ）の品質** — SCOREQ と CER は同等、⚠️ **アクセントだけ落ちた**（31/36。原因は未分離）

[D-056](decisions.md#d-056) の Task 7。学習は v3 と同じスケジュール（[M-111](#m-111)）、
パックは [M-112](#m-112) の `data/pack_cc0`（14,426 発話）。
...
<a id="m-114"></a>
## M-114. **seed 対照** — アクセントの劣化は大部分が run 間のばらつきだった（⚠️ **テキストの影響を否定はできない**）

[M-113](#m-113) で v4 のアクセントが 31/36 に落ちた。**「JSUT を外したせい」か
...
```bash
# v3 と seed 以外を完全に一致させる（スケジュールは M-111）
...
⚠️ 教師ゲートは生徒依存なので（[C-075](decisions.md#c-075)）各 run の分母が違う
...
⚠️ M-113 の時点では「別の要因を示唆する」と書いたが、**その要因はテキストではなかった。**
...
<a id="m-115"></a>
## M-115. **C-075 を直した** — 教師ゲートが生徒に依存しなくなり、3 run で分母がそろった

[C-075](decisions.md#c-075) で「教師ゲートが生徒依存」と記録したが**直していなかった**。
...
（[M-114](#m-114) の結論と同じ。[D-057](decisions.md#d-057) の判断は変わらない）。
