...
⚠️⚠️ **再現コマンドは副作用を持つ。走らせる前に読むこと**（M-99 で実際に壊した）:
...
make -C csrc label_ids_test && ./csrc/label_ids_test <vec>   # G26b ⚠️ かつて label-ids_test / k7_test と書いていた（M-100）
```
...
C-057（「音素の 0.32%」は n=298 の値）を受けて、**同じ量を n 無しで書いている箇所**を機械的に探した:

```bash
uv run python -c "…（0.32% / 0.55% / 0.22% を含む行のうち、前後 1 行に n= が無いものを列挙）"
```

⚠️ **公開向けの 4 箇所が n 無しだった**（どれも「0.63%（n=1,495）」に直し、C-057 を指した）:
...
（基準 438,750 / int16 は n=298 で 0.32%、**n=1,495 では 0.63%**。C-058）。
...
### 3. ⚠️ **M-97 の絶対値は n=298 で 2 倍楽観的だった**（→ C-058）
...
**1494 / 1495** で同じ。**既定の 298 文では見えなかっただけ**（C-058 と同じ形 =
**n を書かずに「完全一致」と言っていた**）。
...
| **M-94** | ✅ | 精度は全部一致。⚠️ **サイズ 6 箇所がずれる** → **M-99 の整数式の値に直した**（§6） |
| **M-71** | ✅ | §3 で解決 |
| **M-68** | ⚠️ | **2 つ値が出ない**（§5） |
