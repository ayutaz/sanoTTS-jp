...
## M-112. **ブラウザデモの「ids 350 超えは拒否」ゲートが 6 日間空虚だった** — 形態素上限が先に発火していた / CI は辞書を落としていないので原理的に気づけない / ⚠️ **ついでに [D-052](decisions.md#d-052) の包絡が破れていることが分かった**（自己実測 / M4 Max）

**きっかけ**: 無関係な作業（GitHub Sponsors の導線）で `scripts/check_web_gates.sh` を回したら
NG 3 件で落ちた。変更を stash して `main` で回し、**同じ 3 件が同じ本文で出る**ことを確認した
= 既存の壊れ。
...
⚠️ **この NG 本文自体が嘘だった。** `rc=-11 < 0` は**拒否された**という意味なのに
「通った」と書いてある。判定が `rc < 0 && msg に 'ids が'` の 1 本で、外れたときの
文面を 1 つしか用意していなかったため。**嘘の診断を読んで原因を探すことになった。**

### 1. 直接の原因 — テストベクタが古い（実測）

ゲートの入力は `'今日は良い天気ですね。'.repeat(9)` だった。上限は 2 つある:

| | 上限 | どこで | いつ入ったか |
|---|---|---|---|
| (1) | 形態素 **44**（`SAAN_KANJI_MAX_INPUT_TOK`） | `saan_kanji.c:153`。**NJD に入る前** | `55c4329` 2026-09-10（[D-052](decisions.md#d-052) / [M-98](#m-98)） |
| (2) | ids **350**（`SAAN_MAX_IDS`） | `saan_web.c:550`。G2P の**後** | 以前から |

**(1) が先に発火するので (2) は 1 度も走っていなかった。** ゲートを書いたのは
`8b64c15` 2026-09-04 01:50 JST で、**(1) はその 6 日後**。`git merge-base --is-ancestor` で
先祖関係も確認した。

⚠️ **この文では、何回繰り返しても (2) には届かない**（構造的に測れない）:

```bash
# 棒は scratchpad に書いて実行した（リポジトリには置かない）。ABI は
# scripts/check_web_gates.sh の __synth と同じ。辞書を渡して W8A32 レーンで測る。
EMCC="$HOME/emsdk/upstream/emscripten/emcc" bash web/build.sh
```
...
| 入力 | 形態素 | ids | 結果 |
|---|---:|---:|---|
| `今日は良い天気ですね。`×6 | 42 | 308 | 通る |
| `今日は良い天気ですね。`×7 | 49 | — | **(1) 形態素上限** |
| `今日は良い天気ですね。`×9（ゲートの入力） | 63 | — | **(1) 形態素上限** |
| `コンピューターグラフィックス`×8 | 8 | 325 | 通る |
| **`コンピューターグラフィックス`×9** | **9** | **365** | **(2) ids 上限** ← 新しいベクタ |

この文は 7 形態素で 50 ids しか作らない（7.1 ids/形態素）ので、**44 形態素の枠内では
最大 308 ids にしかならない**。カタカナ語は 1 形態素あたりの mora が多く、(1) に当たらずに (2) へ届く。

⚠️ **ids の値は辞書で変わる**（44,000 entries だと同じ入力が 343 / 387）。
**ゲートは値を合否にせず「どちらの上限で拒まれたか」だけを見る。**

### 2. なぜ 6 日間 CI が緑だったか — 辞書を落としていない（実測）

辞書依存の検査は `if (haveDict)` の中にあり、CI は辞書を置いていないので**丸ごと飛ぶ**。
`ci.yml` にはその理由が書いてあった:

> 辞書 `csrc/k1_dict.bin`（13,702,320 B）が **git にもリリースにも無い**

⚠️ **後半は事実ではない。** リリースに在る:

```bash
GH_TOKEN="$(gh auth token)" gh release view v0.3.0 \
  --json assets -q '.assets[] | select(.name|test("dict")) | "\(.name)\t\(.size)"'
# k1-dict-438750.bin	13702320
# NOTICE-dictionary.txt	5865
GH_TOKEN="$(gh auth token)" gh release download v0.3.0 -R ayutaz/sanoTTS-jp \
  --pattern 'k1-dict-438750.bin' --dir /tmp --clobber
shasum -a 256 /tmp/k1-dict-438750.bin csrc/k1_dict.bin
# f162c922074d76817298b34d8a8fd35f7d195f38540303485a76c956b5d84877  （2 行とも同じ）
```

**手元の辞書と bit 一致する。** しかも **`pages.yml` は同じ commit `8b64c15` から
同じタグの同じ資産を落としている**（`.github/workflows/pages.yml:111`）。
つまり「在るのに落としていない」だけだった。→ [C-072](decisions.md#c-072)

v0.3.0 は 2026-09-03 07:53 UTC 公開で、ゲートを書いた `8b64c15`（09-04 01:50 JST）**より前**。
**この注記は書かれた瞬間から誤りだった。**

### 3. 直したあとのゲート（陽性対照つき）

`scripts/check_web_gates.sh` の G-W6 を 1 本 → **3 本**にした。

```bash
bash scripts/check_web_gates.sh        # exit 0 / NG 0 件
```

```
OK  W8A32: ids 上限 350 で拒否（ids が 365 個で上限 350 を超えた。**…）
OK  W8A32: 1 語短いと通る（325 ids ≤ 350）— 上の拒否は長さが理由
OK  W8A32: 形態素上限 44 で拒否（D-052。漢字 G2P 失敗: 長すぎる（形態素 44 個まで…）
```

⚠️ **「1 語短いと通る」が要る。** 拒否だけ並べたゲートは「全部拒む」実装で満点になるうえ、
**上のベクタが (1) に当たっていないことの証拠**にもなる（当たっているなら 1 語短いこれも当たる）。

**陽性対照は 3 本とも注入して落ちるのを見た**（1 回の実行で 3 種 × 2 レーン = 6 件の NG）:

| 注入 | 出た NG |
|---|---|
| A: `IDS_OVER` を昔のベクタに戻す | `**拒否はされたが ids 上限ではない** rc=-11 … **ids 上限は測れていない**` |
| B: `IDS_UNDER` も拒否される入力にする | `ids 上限の 1 つ手前が通らない rc=-11 … 境目を踏めていない` |
| C: `TOK_OVER` を 7 形態素の文にする | `63 形態素の入力が形態素上限で拒まれない rc=0` |

⚠️ **A の文面が「通った」から「拒否はされたが ids 上限ではない」に変わったのが本題。**
旧版はこの状況で嘘をついていた。

CI の `web` job が辞書を落とすようにした。step をそのまま真似して実測:

```bash
GH_TOKEN="$(gh auth token)" gh release download v0.3.0 -R ayutaz/sanoTTS-jp \
  --pattern 'k1-dict-438750.bin' --dir /tmp/cisim --clobber
mv /tmp/cisim/k1-dict-438750.bin /tmp/cisim/k1_dict.bin
test "$(wc -c < /tmp/cisim/k1_dict.bin)" -eq 13702320 && echo OK
```

### 4. ⚠️ 副産物 — [D-052](decisions.md#d-052) の包絡が破れている（**未解決**）

D-052 は「形態素数で縛れば Open JTalk の一時ヒープが縛れる」という論法で、
2 つの実測を繋いでいる:

```
OJ ヒープ = 197.6 B × ids + 924 B     （QEMU / n=6 / R²=0.99905。M-98）
ids ≤ 8 × 形態素 + 54                 （held-out 2,328 文で例外なし。M-98）
```

⚠️ **後者が held-out の外で破れる。** カタカナ複合語は 1 形態素あたりの ids が桁違いに多い。

再現（**ホストで実測**。リポジトリの `csrc/oj_heap_probe.h` をそのまま当てる）:

```bash
# openjtalk の .c だけ -include oj_heap_probe.h で建て、saan_kanji_to_ids を直接呼ぶ。
# n_tokens は形態素上限の検査より前に書かれるので、拒否された入力でも形態素数が取れる。
# （棒は scratchpad。csrc/Makefile の oj_heap_test と同じ当て方）
```

| 入力 | 形態素 | ids | 包絡 8t+54 | **OJ ピーク実測** | |
|---|---:|---:|---:|---:|---|
| `今日は良い天気ですね。` | 7 | 53 | 110 | 9,837 | |
| `今日は良い天気ですね。`×6 | 42 | 308 | 390 | 57,122 | |
| `コンピューターグラフィックス`×9 | 9 | 365 | 126 | 66,709 | ⚠️ 包絡 +239 |
| `ソーシャルネットワーキングサービス`×8 | **16** | **453** | 182 | **81,212** | ⚠️ 包絡 +271 |
| `国際連合教育科学文化機関`×7 | **42** | **495** | 390 | **99,146** | ⚠️ **`SAAN_KANJI_OJ_MAX_BYTES` 81,412 B を超える** |

`SAAN_KANJI_OJ_MAX_BYTES` = `198 × (8×44 + 54) + 1024` = **81,412 B** が
`_Static_assert` で 80 KB 予算と突き合わされている値。
**形態素上限が通す入力が、その値を 1.22 倍超えた。**

⚠️ **これが実機で問題になるとは、この測定だけでは言えない**:

- **ホストの「要求バイト数」であって ESP32 のヒープではない**（`oj_heap_probe.h` が明記）。
  ポインタ幅もアロケータの丸めも違う。**PSRAM 無しの実機でも QEMU でも測っていない**
- M-98 の式は**QEMU 実測**なので、上の絶対値と直接は比べられない
- ⚠️ **ただし相対の主張は測定で閉じている**: **同じ 42 形態素**の 2 文が
  **57,122 B と 99,146 B**（1.74 倍）で、**形態素数はヒープを縛れていない**
- 影響するのは **PSRAM の無い板だけ**（ATOMS3 など。[M-109](#m-109)）。
  PSRAM がある板（CoreS3 = 実測に使っている板）では PSRAM に落ちる

**打ち手は決めていない**（ids で縛る / 形態素上限を下げる / 実機で測る、のどれか）。
**この PR では直していない。**

### ⚠️ この測定で言えないこと

- **実機では 1 つも測っていない。** ゲートも包絡の破れも、node（wasm）とホストの C だけ
- **包絡が破れる入力が実文に出るかは測っていない。** 上の 3 つは**作った入力**で、
  held-out 2,328 文では [M-98](#m-98) のとおり例外なく包絡が成り立っていた
- **CI で辞書を落とす変更が実際に緑になるかは、この時点では未確認**（PR を出して見る）
- **`ids 上限 350` が正しい値かは見ていない。** 見たのは「その検査に到達するか」だけ
