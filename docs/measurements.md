...
TTS に必要なのは**読み・アクセント型・アクセント結合規則・最小限の品詞**だけなので、
エントリ枝刈りと feature 削減は独立に効く:

| 語数 | 素の概算 (131 B/entry) | feature を TTS 用に削減した場合 |
|---:|---:|---:|
| 10,000 | 1.2 MB | 〜0.6 MB |
| 30,000 | 3.7 MB | 〜1.8 MB |
| 60,000 | 7.5 MB | 〜3.7 MB |
| 100,000 | 12.5 MB | 〜6.3 MB |

⚠️ **これは平均バイト数からの線形概算であって実測ではない。darts trie は語数に線形には
縮まない。** B-0 で実際に枝刈り辞書をビルドしてサイズと実文カバー率を測ること。

参考: ESP32-S3 (16 MB flash) はアプリ + IDF を引いて 12 MB 前後残る。

---

## M-9. コーパス（ワークフロー調査による。未再検証）

> 以下は 2026-08-26 のワークフロー調査エージェントの報告値で、**私自身は再実測していない**。
> Phase 1 で使う前に再確認すること。
