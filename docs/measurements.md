...
|---|---|
| [M-1](#m-1) | 環境 |
| [M-2](#m-2) | 教師 checkpoint |
| [M-3](#m-3) | 教師への入力 |
| [M-4](#m-4) | 音素化 |
| [M-5](#m-5) | 発話速度 |
| [M-6](#m-6) | 決定的推論とラベル |
| [M-7](#m-7) | 公開 ONNX との関係 |
| [M-8](#m-8) | OpenJTalk 辞書のフットプリント |
| [M-9](#m-9) | コーパス |
| [M-10](#m-10) | 教師音声の品質ベースライン |
| [M-11](#m-11) | C 実装のみの G2P と教師経路の乖離 |
| [M-12](#m-12) | ひらがな入力に限定した場合 |
| [M-13](#m-13) | アクセントはアーキテクチャで対応済み |
| [M-14](#m-14) | かな中間表現の変換器 |
| [M-15](#m-15) | 実行環境と学習コストの見積もり |
| [M-16](#m-16) | ESP32-S3 のメモリ収支 |
| [M-17](#m-17) | 生徒モデルの層構成 |
| [M-18](#m-18) | 学習 1 ステップのコストと vast.ai 費用 |
| [M-19](#m-19) | A-1 の不一致は経路 (b) の誤りではない |
| [M-20](#m-20) | Phase B: ラベル生成経路の確定 |
| [M-21](#m-21) | B-d: デバイス間のラベル一致 |
| [M-22](#m-22) | Phase C: 学習ループのスモークテスト |
| [M-23](#m-23) | B-4: 長さ分布と符号化の関係式 |
| [M-24](#m-24) | B-7: 日本語の length scale `s_v` |
| [M-25](#m-25) | B-8 / D2: `_` PAD はフレームの過半を占める |
| [M-26](#m-26) | B-9: デプロイ語彙は 57 |
| [M-27](#m-27) | B-6: スペクトル平坦度プローブの窓設計と教師ベースライン |
| [M-28](#m-28) | B-10: 教師の学習テキストとの重複 |
| [M-29](#m-29) | SCOREQ を導入し、教師と実人間のベースラインを測った |
| [M-30](#m-30) | C-1: 論文に値が無い λ を勾配ノルムで決めた |
| [M-31](#m-31) | C-2: 一次差分判別器の凍結 |
| [M-32](#m-32) | D5: iSTFT のフレーミング規約 |
| [M-33](#m-33) | D6: 波形ラベル `yT` に EMA を当てるか |
| [M-34](#m-34) | Phase C: 全修正を入れ直したあとのスモークテスト |
| [M-35](#m-35) | 本番ラベルパックの生成 |
| [M-36](#m-36) | 本学習 v1 |
| [M-37](#m-37) | 本学習 v2 |
| [M-38](#m-38) | CER |
| [M-39](#m-39) | Phase 6: int8 量子化と blob サイズ |
| [M-40](#m-40) | Phase 5: `β` |
| [M-41](#m-41) | Phase D: C99 推論コアが参照実装と一致した |
| [M-42](#m-42) | Phase D-2: ストリーミング化で ESP32 の SRAM に載った |
| [M-43](#m-43) | Phase D-3a/b: FFT 化とレイテンシの初測定 |
| [M-44](#m-44) | D-4: アクセント型ミニマルペアの再現性 |
| [M-45](#m-45) | Phase D-3c'-1/2: int8 経路の end-to-end 統合 |
| [M-46](#m-46) | Phase D-3c'-4: ESP-IDF 雛形と arena の実測 |
| [M-47](#m-47) | B-12: 教師の事前学習テキスト (MOE-Speech) と評価コーパスの重複 |
| [M-48](#m-48) | 敵対的検証で見つかった「空虚に通るゲート」と silent failure |
| [M-49](#m-49) | 追試 E-2: レーンラダーで品質ギャップを分解した |
| [M-50](#m-50) | 追試 E-1: DNSMOS P.835 を 4 レーンで測った |
| [M-51](#m-51) | 端末側 G2P の C99 実装 |
| [M-52](#m-52) | 追試 E-2b: `Gγ` の幅スイープ |
| [M-53](#m-53) | P-1 の前提が違っていた |
| [M-54](#m-54) | ESP-IDF は導入可能だった |
| [M-55](#m-55) | W8A8 は知覚的には無料だった |
| [M-56](#m-56) | PIE は QEMU で検証できる |
| [M-57](#m-57) | PIE カーネルを実装し、QEMU で bit 完全一致を確認した |
| [M-58](#m-58) | PIE の被覆を 69.0% → 99.40% に広げた |
| [M-59](#m-59) | 成果物を v2 → v3 |
| [M-60](#m-60) | 聴取 |
| [M-61](#m-61) | リリース前の検算で v3 の品質値 3 件が誤っていた |
| [M-62](#m-62) | 出荷ファームを ESP-IDF v5.5 でビルドし、QEMU で完走させた |
| [M-63](#m-63) | 端末でかなの自由入力を受けられるようにし、QEMU で通した |
| [M-64](#m-64) | `--intermediate` は教師経路とバイト単位で一致する |
| [M-65](#m-65) | piper-plus も教師も無い環境で音が出ることを、新規 clone で確かめた |
| [M-66](#m-66) | `esp32/TESTING.md` の手順を、新規 clone + リリース資産で 1 行ずつなぞった |
| [M-67](#m-67) | v0.1.1 で配布する firmware イメージを作り、QEMU で起動まで確かめた |
| [M-68](#m-68) | K トラック |
| [M-69](#m-69) | K-4b: NJD チェーンの C 統合とホスト一致 |
| [M-70](#m-70) | 端末に載せられない後処理 3 段の影響 |
| [M-71](#m-71) | K-5: 1 文あたりのピーク RAM と、ラベルバッファの縮小 |
| [M-72](#m-72) | 接続行列を 1 バイトに落とす取引 |
| [M-73](#m-73) | 辞書の頻度カーブと、落ちる語の正体 |
| [M-74](#m-74) | K-6: 端末の全段をホストと突き合わせた |
| [M-75](#m-75) | 未知語のフォールバック |
| [M-76](#m-76) | K-7: ESP32-S3 へのビルドと QEMU での通し |
| [M-77](#m-77) | 動作点を決めるための実測 |
| [M-78](#m-78) | 辞書の枝刈りは音を汚さない。変えるのは読みだけ |
| [M-79](#m-79) | ⚠️ 出荷構成では DRAM が 19,304 B 足りなかった |
| [M-80](#m-80) | 段別プロファイラを入れ、1 step の内訳をホストと QEMU で取った |
| [M-81](#m-81) | S1〜S3 を入れた |
| [M-82](#m-82) | 実機で初めて自分で測った |
| [M-83](#m-83) | K-8 実機 |
| [M-84](#m-84) | D-cache の行を 32 B → 64 B にしたら xRT 0.926 → 0.861 |
| [M-85](#m-85) | マイクロベンチ |
| [M-86](#m-86) | 漢字ビルド + W8A8+PIE |
| [M-87](#m-87) | T1 + T5 を実機に載せたら GELU が 118 → 211 cyc/要素に倍増していた |
| [M-88](#m-88) | 要件 RTF ≤ 0.5 を達成した |
| [M-89](#m-89) | T4 |
| [M-90](#m-90) | スタックチャン |
| [M-91](#m-91) | G32 聴取 |
| [M-92](#m-92) | ホストの `--text` |
| [M-93](#m-93) | v0.3.0 の配布イメージを焼いた実機で、新しい文 |
| [M-94](#m-94) | C99 コアと漢字経路が WebAssembly でそのまま動いた |
| [M-95](#m-95) | 本物の Chrome でブラウザ実測 |
| [M-96](#m-96) | ブラウザの音を人が聴いた |
| [M-97](#m-97) | 8 MB flash に漢字辞書を載せられるか |
| [M-98](#m-98) | PSRAM 無しの内部 DRAM |
| [M-99](#m-99) | ③ に進む前の 2 つの前提を測った |
| [M-100](#m-100) | 辞書リーダの入力検査を入れた |
| [M-101](#m-101) | 走っていなかったゲートが 1 本、n を増やすと落ちるゲートが 1 本 |
| [M-102](#m-102) | M 番号の再現コマンドを機械的に走らせた |
| [M-103](#m-103) | 未解明だった 4 件を追った |
| [M-104](#m-104) | 8 MB flash の漢字構成を QEMU で通した |
| [M-105](#m-105) | 実機 |
| [M-106](#m-106) | 2 MB flash に漢字を載せられるか |
| [M-107](#m-107) | `charr` が未知語ノード生成を殺していた |
| [M-108](#m-108) | `rec5` |
| [M-109](#m-109) | ⚠️ 第三者が 5 板で試した |
| [M-110](#m-110) | ✅ この PR のコードで出荷構成 |
| [M-111](#m-111) | Dependabot アラート 5 件への対応を実測で決めた |
| [M-112](#m-112) | 差し替え候補の教師 2 件を実測した |
| [M-113](#m-113) | CC0/PD のみに絞った蒸留テキストの統計 |
| [M-114](#m-114) | v3 の学習スケジュールを ckpt から復元した |
| [M-115](#m-115) | CC0/PD のみのラベルパック `data/pack_cc0` を生成した |
| [M-116](#m-116) | v4 |
| [M-117](#m-117) | seed 対照 |
| [M-118](#m-118) | C-076 を直した |
| [M-119](#m-119) | Task 8: v4 の blob と golden を書き出し、C99 コアの全ゲートを通した |
| [M-120](#m-120) | v4 の firmware をビルドし、QEMU で起動から合成まで通した |
| [M-121](#m-121) | v4 で 3 構成すべてビルドした |
| [M-122](#m-122) | 出荷用の firmware を作り直した |
| [M-123](#m-123) | v4 を実機 |
| [M-124](#m-124) | 出荷イメージ 3 本をマージ後のツリーで作り直し、実機で漢字を喋らせた |
| [M-125](#m-125) | v4 は同じ入力で 3 通りの発話長になる |
| [M-126](#m-126) | 出荷イメージ 10 本すべてに v4 と正しい辞書が入っていることを抽出照合した |
| [M-127](#m-127) | `？` + `〜`(U+301C) が端末で疑問にならない件を直した |
| [M-128](#m-128) | 出荷イメージ 10 本を D-062 込みで作り直した |
| [M-129](#m-129) | D-062 を QEMU の firmware 経路で確認した |
| [M-130](#m-130) | D-062 込みの出荷イメージを実機 |
| [M-131](#m-131) | 辞書 13.7 MB の完全性検査のコストを実機で測った |
| [M-132](#m-132) | v4 の W8A32 の checksum を測った |
...
詳細は `research/b0-g2p-footprint.md`。
...
**⚠️ 全項目の詳細と再現は `research/k1-kanji-katakana-ondevice.md`。**
...
（1 チャンク 144 ms）と報告した（`research/s1-m5-cores3-speed.md`。
...
直し方の順序と受け入れ条件は `research/s1-m5-cores3-speed.md` §5。
...
**2026-09-03。W トラック（`plan/web-demo-plan.md`）の着手前調査。**
...
`docs/research/k1-kanji-katakana-ondevice.md` §6-2 / §6-3 の
...
- **Core2**（**ESP32 無印**）: ⚠️ **これは別のチップ**。Xtensa LX6 / **PIE が無い** /
  ...
  ⚠️ **うちは ESP32 無印を 1 度も測っていない**（`docs/requirements.md` §2.1 は S3 が対象）
...
（Task 3 / Task 4。⚠️ **設計文書は 2026-09-12 に消した** — 判定の一次ソースは
[`../src/saanotts_jp/corpus_license.py`](../src/saanotts_jp/corpus_license.py)、
決定は [D-054](decisions.md#d-054)〜[D-057](decisions.md#d-057)）
...
## M-135. **v4 の音を初めて対照つきで聴いた** — held-out 18/24 文で「問題なし」（⚠️ **盲検でない / n=1**）

**2026-09-12。** 残タスク 1（G32）で止まっていた「**v4 の音は誰も 1 秒も聴いていない**」を崩した。
⚠️ **私は音を聞けない。** 私がやったのは `afplay` で鳴らすことだけで、**判定はユーザー**である。

再現:

```bash
# 生徒 → ピン → 教師 → ピン を 1 文ずつ（教師が対照）
ls reports/eval_v4_full/student/*.wav | head -18 | while read s; do
    uid=$(basename "$s" .wav)
    afplay "$s"
    afplay /System/Library/Sounds/Tink.aiff
    afplay "reports/eval_v4_full/teacher/$uid.wav"
    afplay /System/Library/Sounds/Tink.aiff
done
```
...
| | |
|---|---|
| 素材 | `reports/eval_v4_full/` の **held-out 24 文**（学習に使っていない）。**v4 の生徒**と、**同じ文の教師** |
| 順序 | **生徒 → 教師** を 1 文ずつ。**教師が直後に来る = 対照つき** |
| 実際に聴いた数 | **18 / 24 文**（19〜24 は聴く前に打ち切り） |
| 聴いた uid | `BASIC5000_0083 / 0123 / 0377 / 1619 / 2314 / 2652 / 4093 / 4197 / 4739`、`ONOMATOPEE300_064`、`ROHAN4600_0850 / 1788 / 2252 / 3325 / 3629 / 4164`、`sentence_collector_00225 / 02992` |
| 判定 | 1 文目「**問題なかったです**」／ 18 文を通して「**ok ですね**」 |

### 2. ⚠️ これで言えること・言えないこと

✅ **言えること**: **v4 の生徒の音は、教師を直後に置いて比べても破綻していない。**
これまでの聴取（[M-91](#m-91) / [M-93](#m-93) / [M-96](#m-96)）は**すべて v3 で、しかも対照なし**だった。
**v4 に対照がついたのはこれが初めて。**
...
| | |
|---|---|
| **盲検でない** | 生徒が**必ず先**で、どちらがどちらか**伝えたうえで**鳴らした |
| **n=1** | 聴者は 1 名。[C-037](decisions.md#c-037) のとおり、この 1 名は v2↔v3 の差を 7 試行で聴き分けられなかった |
| **6 文は聴いていない** | 19〜24 文目 |
| **摩擦音を個別に見ていない** | 論文が集約スコアで見逃した欠陥（sibilant が whistly）は、**そこを聴くと決めて聴かないと出ない** |
| **アクセントの過剰強調は未聴取** | `magnitude_ratio` **1.193**（[M-59](#m-59)）。素材は `reports/d4_accent_v4/` に 64 本あるが**鳴らしていない** |
| **枝刈りの誤読は未聴取** | `reports/k8_listen/` は**手元に無い**（`k8_audio_gap.py --wav-dir` で作るところから） |
| **実機のスピーカーではない** | これはホストで合成した wav。**途切れ・音量・実サンプルレートの誤差は別**（[M-90](#m-90) §5） |
...
### 3. CI で実際に効いたか（[PR #29](https://github.com/ayutaz/sanoTTS-jp/pull/29) の run）

⚠️ **「緑になった」は辞書が効いた証拠ではない。** 辞書なしでも緑だった（それが問題だった）。
**CI 自身のログを読んで、検査が増えたことを数えた**:

```bash
gh run view <run-id> --log | grep 'wasm ゲート G-W1' | grep -c '  OK  '
```

| run | 辞書 | **web job の OK 件数** |
|---|---|---:|
| `34701609411`（main / f767b90） | なし | **63** |
| `34702092717`（PR #29） | **あり** | **67** |

**+4** で、手元の 75 → 79 と**同じ差**。CI のログに実際に出た行:

```
      辞書あり: csrc/k1_dict.bin（13702320 B）→ 漢字 == かな も回す
  OK  W8A32: 漢字文 → route=辞書 / 53 ids / 27648 sample / PCM が かな経路と bit 一致
  OK  W8A32: 長すぎる漢字文 は拒否（効いた上限: 形態素 44 / …）
  --  ⚠️ **350 ids の枝は覆えていない** — 形態素 44 の上限が先に発火する（M-98）
  OK  W8A8: 漢字文 → route=辞書 / 53 ids / 27136 sample / PCM が かな経路と bit 一致
  OK  W8A8: 長すぎる漢字文 は拒否（効いた上限: 形態素 44 / …）
```

⚠️ **CI の絶対数（63 / 67）は手元（75 / 79）と違う。** 手元にしか無い入力
（`ids_heldout.bin` など）で回る検査があるため。**比べてよいのは同じ環境どうしの差だけ。**
...
<a id="m-140"></a>
## M-140. ⚠️ **既定の RAM を減らしたが、arena の下限を読み間違えた**（訂正 = [C-100](decisions.md#c-100) / 正 = [M-142](#m-142)）— arena 176 → 136 KB。**PCM は bit 一致**、定常 xRT 0.448 → 0.483（要件内）（自己実測 / M5 CoreS3）

> ⚠️⚠️ **この節の構成（arena 136 KB）は 350 ids で init が失敗する。**
> 下限を `saan_stream_arena_used(350)` = 134,432 B と読んだのが誤りで、
> **本当の下限は init の途中のピーク 149,824 B**（W8A8）だった。
> **訂正は [C-100](decisions.md#c-100)、正しい構成と数字は [M-142](#m-142)**（arena 148 KB / **−28,840 B**）。
> ⚠️ **§1 の削減量（a/b/c）と §3 の bit 一致は有効。** 無効なのは
> **§2 の表の arena 139,264 B と −41,168 B**、および §4 の起動時チェックの式である。

**2026-09-17。ユーザーの CoreS3（[D-047](decisions.md#d-047)）。**
[M-139](#m-139) は「減らせる手段が 2 つある」で止めたが、**要望は「減らすこと」**だったので
**既定を変えた**（[D-066](decisions.md#d-066) を書き直した）。生ログ `reports/m140_ram/`。

### 1. 何を削ったか（3 つ。**どれも PCM を 1 bit も変えない**）

| # | 何 | 根拠 | 減った量 |
|---|---|---|---:|
| a | **MEM-5 粒度 4**（`o1539` 49,248 → 24,624 B） | [M-139](#m-139) §6 の実機スイープ | **−24,624** |
| b | **漢字の素性表 96 → 44 本**（30,720 → 14,080 B） | `saan_kanji_to_ids` が `nt > 44` を**素性を作る前に**弾くので `nf ≤ 44` がコードで保証されている | **−16,640** |
| c | **K-7 のトークン表 640 → 256 本**（10,240 → 4,096 B） | [M-79](#m-79) が 256 で held-out 298/298 一致・**128 で 273/298** を陽性対照つきで測ってある | **−6,144** |

b と c で**漢字経路の作業領域 144,640 → 121,856 B**（−22,784）。
a で**合成側が 350 ids で 159,056 → 134,432 B**。
**両方の下限が下がったので `SAAN_ARENA_BYTES` を 180,224 → 139,264 B（136 KB）**にした。

⚠️ **b は「測って減らした」のではなく「コードが保証している不要分」。**
`nt = jdict_analyze(...)` → `if (nt > SAAN_KANJI_MAX_INPUT_TOK) return` → 素性ループ、
という順なので 96 本目は**一度も書かれない**。
⚠️ **NJD 段の `s_k4`（524 B × 96 = 50,304 B）は減らせない** — `njd_set_digit` がノードを
増やすので入力の形態素数では縛れない（[M-98](#m-98) の `SAAN_KANJI_MAX_TOK` の注記と同じ）。

### 2. 実機（既定ビルド。**フラグを 1 つも渡していない**）

```bash
cd esp32/boards/m5unified
idf.py -B /tmp/ram/bdef -DSDKCONFIG=/tmp/ram/bdef/sdkconfig \
    -DSDKCONFIG_DEFAULTS="sdkconfig.defaults;sdkconfig.cores3" -DSAAN_KANJI=1 \
    -DSAAN_MODEL_BLOB=… -DSAAN_DICT_BLOB=… build
idf.py -B /tmp/ram/bdef -p /dev/cu.usbmodem2101 -b 921600 app-flash
```

| | 変更前（v1.1.0） | **変更後（既定）** | 差 |
|---|---:|---:|---:|
| 静的 DIRAM | 260,855 B（76.33%） | **219,687 B（64.28%）** | **−41,168** |
| 　`.bss` | 201,144 B | **159,976 B** | −41,168 |
| **起動直後の内部 DRAM free** | 131,799 B | **172,967 B** | **+41,168** |
| **最大ブロック** | 86,016 B | **126,976 B** | **+40,960** |
| 1 発話後の低水位 | 127,015 B | **168,183 B** | +41,168 |
| `SAAN_ARENA_BYTES` | 180,224 B | **139,264 B** | −40,960 |
| 漢字経路の作業領域 | 144,640 B | **121,856 B** | −22,784 |
| Viterbi に渡る | 84,736 B | 66,560 B（予算 49,152 以上） | −18,176 |
| **定常 xRT** | 0.448 | **0.483 / 0.483 / 0.484** | +7.8% |
| 鳴らし始め | 385 ms | **372〜377 ms** | −8〜13 |
| アンダーラン | 0 / 55 | **0 / 103 チャンク** | 変わらず |

⚠️ **−41,168 は −40,960（arena）+ 208**。後者は `.bss` の `s_feat` ポインタ表
（96 × 4 → 44 × 4 B）。
...
| 文 | ids | 変更前 | 変更後 |
|---|---:|---|---|
| `今日は良い天気ですね。` | 53 | `0x390bf4b2aef8f2ec` | **同一** |
| `吾輩は猫である。名前はまだ無い。` | 84 | `0xd6f6fd9d4750719e` | **同一** |
| `明日は雨が降るかもしれませんから、傘を持って出かけましょう。` | 140 | `0xefb0dfc6d93d8845` | **同一** |
| `私は昨日の夕方に近所の公園で…帰りました。` | 224 | `0x1d6943fa6fc06c5d` | **同一** |

ホスト側は [M-139](#m-139) §6 のスイープ（長さを振った 50 本 × 2 レーン・陽性対照 3 件）。

### 4. 足した安全弁 2 つ（**起動時に落とす**）

`SAAN_ARENA_BYTES` と `SAAN_MEM_HEAD_PF` を**別々に**触ると、
「短い文は喋れるが長い文だけ `SAAN_ERR_ARENA`」「漢字だけ喋らない」という
**原因の分かりにくい形**になる。`tts_task` の先頭と辞書 mmap の後で落とすようにした:

```
I (1044) saanotts: arena は最長入力（350 ids）に足りる: 要 134432 B / 確保 139264 B（余り 4832 B）
I (1519) saanotts: 漢字経路の作業領域 121856 B（最低限）/ Viterbi に渡る 66560 B （arena 139264 B のうち）
```

⚠️ **漢字側の静的検査（`check_esp32_template.sh` §10）は当てにしない。**
あれは **ホストの `sizeof`** で `SAAN_KANJI_WORKBYTES` を評価するので
**ターゲットより小さく出る**（今回 119,808 対 **121,856** = 2,048 B 甘い）。
**スクリプト自身がそう注記している**が、注記は検査ではない。ターゲットの実値で
見るのは上の起動時チェックだけ。

### 5. 既定をどこに置いたか

**`csrc/saanotts_stream.c` の `#ifndef SAAN_MEM_HEAD_PF` 1 行**に置いた。
⚠️ **CMake 側に置かない** — ESP-IDF / wasm / Arduino の 3 経路がこの 1 行を見るので、
CMake に既定を置くと **wasm と Arduino だけ別の値**になる（[C-099](decisions.md#c-099) と同じ形）。
`SANOTTS_ARENA_BYTES`（Arduino）と `arena_stress.c` / `Makefile --g1-kb` も 136 KB に揃えた。

### 6. ⚠️ ここが新しい下限

**合成側 134,432 B（350 ids）が効いている。** 漢字経路は 121,856 B まで下がったので、
**もう壁ではない。** さらに減らすなら:

| 案 | 減る量 | 状態 |
|---|---:|---|
| MEM-5 粒度 2 / 1 | 12,304 / 18,464 B | ⚠️ **xRT が 0.553 / 0.700 で要件を割る** |
| パイプ段のハローだけ保持（`ac`/`dblk`/`tok`） | 算術で 14,048 B | ⚠️ **未実装・未測定** |
| NJD の `s_k4` 524 B × 96 = 50,304 B | ? | ⚠️ **ノード数の実測がまだ**（ここが漢字側の最大項） |
| `ola`/`olw`/`obuf`/`win` | 28,672 B | ⚠️ 未調査 |

### 7. ⚠️ 測っていないもの

- **音は聴いていない**（PCM は 4 文とも bit 一致なので波形は同じ）
- **350 ids ちょうどの文を実機で流していない**（最長は 224 ids）。余り 4,832 B は
  `saan_stream_arena_used(350)` との差で、**起動時チェックが毎回これを出す**
- **`make -C csrc stream` / `int8-e2e`** はこの作業ツリーでは回せない（第三者コーパス不在）
- **wasm と Arduino のビルドは手元で回していない**（emcc / PlatformIO とも CI 側）
- **PSRAM の無い板**での再測定（`SAAN_ARENA_HEAP` とは独立に効くはずだが未確認）

---

<a id="m-141"></a>
## M-141. **これ以上どこまで下げられるか** — 合成側は **−14,048〜20,192 B の余地が算術で見える**（未実装）。⚠️ **漢字側の最大項 50,304 B は下げられない**（`njd_set_digit` が本当にノードを増やす）（自己実測 / M5 CoreS3）

**2026-09-17。** [M-140](#m-140) の後、**まだ下げられるか**を調べた。
arena は 139,264 B、測った下限は **134,432 B**（合成・350 ids）で、
**4,832 B は私が残した余裕**である。その先を 2 方向から見た。

### 1. 合成側 — パイプ段の窓は**ハローだけ持てばよい**（算術。**未実装・未測定**）

各段は `[C][2·pad + CH]` の窓を持つ。**step ごとに持ち越すのは 2·pad 列だけ**で、
中央 CH 列は「その step の作業用」である。ハローを段ごとに持ち、
**作業窓を 1 本共用**すれば次のように詰まる:

| 群 | 現在 | ハロー + 群ごとに共用 1 本 | ハロー + **全群で共用 1 本** |
|---|---:|---:|---:|
| `dblk[0..4]`（pad 3 / W 14） | 21,280 B | 5×1,824 + 4,256 = 13,376 | 9,120 |
| `ac[0..4]`（pad 4 / W 16） | 15,360 B | 5×1,536 + 3,072 = 10,752 | 7,680 |
| `tok[0..2]`（pad 4 / W 16） | 9,216 B | 3×1,536 + 3,072 = 7,680 | 4,608 |
| 共用の作業窓 | — | （上に含む） | 4,256 |
| **合計** | **45,856 B** | **31,808 B（−14,048）** | **25,664 B（−20,192）** |

⚠️ **速度は測っていない。** 代償は step ごとの memcpy（ハローの出し入れ）で、
`dblk` なら 2×6 列×76ch×4 B×5 段 = 18,240 B/step。既に PIPE 段が 1 step の
約 3%（350,645 / 11,659,500 cyc。[M-89](#m-89)）なので**安いはず**だが、
[C-055](decisions.md#c-055)（ホストの見積りで速度を語らない）どおり**実機で測るまで書かない**。
⚠️ **全群共用は「3 つのパイプが同時に生きていないこと」に依存する。** 未確認。

### 2. 漢字側 — ❌ **`s_k4` 50,304 B は下げられない**

`s_k4`（`accent_node_t` 524 B × `SAAN_KANJI_MAX_TOK` 96）は漢字経路の最大項。
**寸法の根拠を測ったことが無かった**ので、高水位計を入れて実機で測った
（`saan_kanji_hwm()`。出荷構成に入れてある = 数十 B の `.bss` と 1 発話あたり数百 strlen）。

```
I (74727) saanotts: 高水位 4 発話: 形態素 26 / 素性 26 / **NJD ノード 26/96** / ラベル 95/512
                    / 最長 pos 9 ctype 24 cform 15 orig 12 pron 21 read 21（枠 64/128）
```

**9 文（数字・電話番号・助数詞・長い片仮名・和数字・長文）での最大**:
...
**3.7 倍〜7 倍の余裕が見える。が、下げてはいけない。**

⚠️ **9 文はすべて私が作った文で、母集団ではない**（[C-004](decisions.md#c-004) / [C-017](decisions.md#c-017) の形）。
⚠️ **決め手は測定ではなくソースだった。** ヘッダの「`njd_set_digit` がノードを増やす」は
**本当で**、`csrc/openjtalk/njd_set_digit.c` の
`convert_digit_sequence_for_numerical_reading()` が**桁ごとに `NJDNode_insert`** する
（290〜335 行）。**私の 9 文はその経路を 1 度も踏んでいない** —
`1234567890円` は端末の枝刈り辞書では形態素 2 / ids 11 になり、桁読みに展開されなかった。

**つまり「9 文で 26/96 だった」は「96 が過大」の証拠にならない。**
下げるには (a) 桁列の展開量を数詞表から上界で押さえる (b) 辞書 438,750 件の
`orig`/`pron`/`read` の最長を走査する、の**両方**が要る。**やっていない。**

### 3. 構造 — **arena は 2 つの max なので、片方だけ下げても意味がない**

| | 現在 | §1 を入れたら |
|---|---:|---:|
| 合成（350 ids） | **134,432 B ← これが効いている** | 120,384 B |
| 漢字経路 | 121,856 B | **121,856 B ← こちらが効く** |
| → arena（余裕込み） | 139,264 B | **122,880 B（120 KB）** |

✅ **§1 だけで arena は 139,264 → 122,880 B = さらに −16,384 B（−11.8%）** になる。
そこで**漢字側が壁になる**ので、それ以上は §2 が要る。

### 4. 採らないもの

| 案 | 理由 |
|---|---|
| MEM-5 粒度 2 / 1 | **xRT 0.553 / 0.700 で要件を割る**（[M-139](#m-139) §6） |
| `win`（4,096 B）を flash へ | ⚠️ 今はターゲット自身の `cosf` で作っている。ホストで焼いた表は **1 ulp 違って PCM が変わりうる** |
| `SAAN_CHUNK` を下げる | §1 の方が安く、[M-139](#m-139) §8b のとおり速度も未測定 |
| 余裕 4,832 B を削る | **350 ids ちょうどの文を実機で流していない**（最長 224 ids）。削る前に流すこと |

### 5. ⚠️ 測っていないもの

- **§1 は 1 行も書いていない。** 数字は配列の形からの算術で、速度も bit 一致も未測定
- **§2 の 2 つの走査**（数詞表の上界 / 辞書の最長フィールド）
- 高水位は **9 文・すべて自作**。held-out で回すには第三者コーパスが要る
- `obuf` 12,288 B / `ola`+`olw` 12,288 B / `w_e` 9,728 B は**内訳を見ただけで、削れるかは調べていない**

---

<a id="m-142"></a>
## M-142. **MEM-6（パイプ段はハローだけ持つ）を実装した** — **速くなった**（xRT 0.483 → 0.474）が **arena は 1 バイトも減らない**。⚠️ 下限は duration の一時領域で、**M-140 の 136 KB は 350 ids で落ちる**（自己実測 / M5 CoreS3）

**2026-09-17。** [M-141](#m-141) §1 の案（パイプ段の窓はハローだけ持てばよい）を実装し、実機で測った。
**結果は 2 つとも予想と違った。**

### 1. 実装 — 持ち越すのは `W − CH` 列だけ

各段の窓 `[C][W]`（W = 2·pad + CH）のうち、**step をまたいで生き残るのは末尾 `W − CH = 2·pad` 列だけ**
（`pipe_push` は左へ CH シフトして末尾に CH 列足すので、次の窓の先頭 2·pad 列 = 今の窓の末尾 2·pad 列）。
中央の CH 列は作業用なので **全段で 1 本の窓を共用できる**（`-DSAAN_MEM_PIPE_HALO=1`。**既定 1**）。

| 群 | 変更前 | 変更後 |
|---|---:|---:|
| `ac[0..4]`（pad 4 / W 16） | 15,360 B | 7,680 B |
| `dinp`（pad 1 / W 10） | 1,600 B | 320 B |
| `dblk[0..4]`（pad 3 / W 14） | 21,280 B | 9,120 B |
| `tok[0..2]`（pad 4 / W 16） | 9,216 B | 4,608 B |
| 共用の作業窓 | — | 4,256 B |
| **合計** | **47,456 B** | **25,984 B（−21,472）** |

**実機の `arena used` は 132,064 → 110,592 B**（53 ids）= 算術どおり −21,472。

### 2. ⭐ 予想外 1 — **遅くなるどころか速くなった**

| | MEM-6 前 | **MEM-6 後** |
|---|---:|---:|
| 定常 xRT | 0.483 / 0.483 / 0.484 | **0.474 / 0.474 / 0.474 / 0.475** |
| 初回 pull | 228.06 ms | **222.90 ms** |
| 鳴らし始め | 372〜377 ms | **364〜374 ms** |
| アンダーラン | 0 / 103 | **0 / 141 チャンク** |
| PCM（3 文） | 基準 | **bit 一致** |

**ハローの出し入れで memcpy が増える（`dblk` で 18,240 B/step）のに速くなった。**
⚠️ **機構は未検証。** 作業窓が 47,456 → 4,256 B になって D-cache（64 KB / 行 64 B）に
収まるのが効いているように見えるが、**段別プロファイラで確かめていない**。
（`memmove` → `memcpy` に変わったことも効きうる。**どちらが効いたかは分けていない**。）

### 3. ⚠️ 予想外 2 — **arena は 1 バイトも減らない**

`make -C csrc arena` を 120 KB で回したら落ちた:

```
OK  クラッシュ 0 件 / 通った最大 n_ids = 300 / 最初に clean fail した n_ids = 350
NG! init: arena が足りない
```

**arena の下限は `saan_stream_arena_used()` ではなく init の途中のピーク**だった。
`saan_run_duration` が `h` / `t1` / `t2` の 3 本 `[SAAN_DUR_W][n_ids]` を取り、
`a->used = mark` で**返す**ので `used` には出ない（詳細と訂正は [C-100](decisions.md#c-100)）:

| 350 ids のとき | W8A32 | **W8A8（実機）** |
|---|---:|---:|
| `log_d` + `d_hat` | 2,816 B | 2,816 B |
| duration の h / t1 / t2 | 134,400 B | 134,400 B |
| W8A8 の act scratch | 0 | **12,608 B** |
| **= init のピーク** | **137,216 B** | **149,824 B** |
| streaming の `arena_used` | 114,128 B | 114,128 B |
| **arena の下限 = max** | **137,216 B** | **149,824 B** |

**16 B 刻みで実測した「init が通る最小 arena」と完全一致した**（`reports/m142_ram/minarena.c`）:

```
W8A32  350 ids で init が通る最小 arena = 137216 B / arena_peak(350) = 137216 B  → OK（安全側）
W8A8   350 ids で init が通る最小 arena = 149824 B / arena_peak(350) = 149824 B  → OK（安全側）
```

→ **streaming のバッファを 21,472 B 削っても、下限は duration が決めているので arena は動かない。**

### 4. 正しい既定 — arena **148 KB**（実機。フラグなし）

| | v1.1.0 | **この構成** | 差 |
|---|---:|---:|---:|
| 静的 DIRAM | 260,855 B（76.33%） | **232,015 B（67.89%）** | **−28,840** |
| 　`.bss` | 201,144 B | **172,304 B** | −28,840 |
| **起動直後の内部 DRAM free** | 131,799 B | **160,639 B** | **+28,840（+21.9%）** |
| **最大ブロック** | 86,016 B | **114,688 B** | **+28,672（+33.3%）** |
| `SAAN_ARENA_BYTES` | 180,224 B | **151,552 B** | −28,672 |
| **定常 xRT** | 0.448 | **0.474 / 0.474 / 0.474 / 0.475** | +5.8% |
| 鳴らし始め | 385 ms | **364〜374 ms** | −11〜21 |
| アンダーラン | 0 / 55 | **0 / 141 チャンク** | 変わらず |
| PCM（3 文） | 基準 | **bit 一致** | — |
...
### 5. bit 一致の証拠（**陽性対照 3 種**）

ホストで長さを振った 50 本 × 2 レーン（W8A32 / W8A8）が総合ハッシュ一致。
**ハローの扱いを壊すと必ず値が変わる**ことを 3 通りで確認した:

| # | わざと壊した所 | 総合ハッシュ（W8A32） |
|---|---|---|
| — | 正 | `0xf0a3904845f16bdf` |
| D / E | ハローを更新しない（間違った列を保存 / 保存しない） | `0xa22a738c7ba838a6` |
| F | ハローを 1 列ずらして保存 | `0x104c4d4cfeb579ff` |
| G | `src` を作業窓に写さない | `0x406f3fdb2398a713` |

⚠️ **D と E は同じ値になる** — どちらも「ハローがゼロのまま」に帰着するので**実質 1 種類**。
**対照が同じ値なら、それは別の対照ではない。**

### 6. 次にどこを削るか — **duration の一時領域（下限の 98%）**

| 350 ids の下限の内訳 | B | 割合 |
|---|---:|---:|
| **duration の h / t1 / t2** | **134,400** | **89.7%** |
| W8A8 の act scratch（duration の conv の分） | 12,608 | 8.4% |
| `log_d` + `d_hat` | 2,816 | 1.9% |

**duration net の受容野は ±12 トークン**（c1 k=5 + c2 k=5 の 3 段）で、
**token パイプ（`TOK_HALO` = 3·`TOK_PAD` = 12）とまったく同じ**。
つまり **同じ手口でチャンク化できる**。K=64 なら 3×[32][2·12+64] = 33,792 B（**−100,608 B**）。
そうすると下限が **漢字経路の 121,856 B** に移り、**MEM-5 と MEM-6 で削った分がやっと効く**
（arena ≈ 122,880 B = v1.1.0 から **−57,344 B**）。
⚠️ **未実装。** duration は n_frames を確定させるために全 ids を通す必要があるが、
**チャンクごとに `log_d` へ書き出せば足りる**（ストリーミングではなく分割処理）。

### 7. ⚠️ 測っていないもの

- **音は聴いていない**（PCM は 3 文とも bit 一致）
- **速くなった機構**（D-cache か memmove→memcpy か）を分けていない
- **350 ids ちょうどの文を実機で流していない**（最長 224 ids）。下限はホストの実測と
...
| `olw` の周期性（窓が固定 / hop 固定なので定常部は周期 256） | −5,888 | ⚠️ **加算順を合わせれば**不変 | ⚠️ **素直に表を作ると変わる** — 実装は i の降順で累積するので、昇順で作ると **256 位相のうち 91 で最終ビットが違う**（[M-144](#m-144) で実測。`reports/m144_mem7/olw.log`）。発話の端の running sum も別に要るので得は −5,888 B より小さい。**入れないと決めた**（[D-067](decisions.md#d-067)） |
...
## M-145. **MEM-8 — `g_chunk` 8,192 B は「一度も使われていない引数」の裏に隠れていた**。⚠️ **`g_ids` の分は後で取り消した**（[C-108](decisions.md#c-108)）。静的 DIRAM は ⚠️ **ホストのビルド値**（自己実測 / ホスト + M5 ビルド）

> ⚠️⚠️ **この節の数字は 2 か所が古い。正は [M-147](#m-147)（実機実測）。**
> 1. **`g_ids` を 351 本にする案は取り消した**（[C-108](decisions.md#c-108)）— 実機で
>    **174 B を超える行が全部拒否される**ことが分かった。**−2,704 B は戻した。**
>    したがって MEM-8 の削減は **−10,896 B ではなく −8,192 B**（`g_chunk` の分だけ）。
> 2. **DIRAM 183,979 B は かな構成のビルド値**で、しかも `g_ids` を詰めた状態のもの。
>    **出荷の漢字構成を実機で測ると 211,535 B**（v1.1.0 の 232,015 から **−20,480**）。
> ⚠️ **§1〜§2 の「見つけ方」と §7 の `o1539` の結論は有効**（取り消したのは数字だけ）。
...
| `obuf` | 12,288 | `SAAN_OBUF_HOPS` = 2·`CH` − (`SAAN_LATENCY` mod `CH`) = 12 hop。⚠️ **`CH` を下げれば減る** — 下記 |
...
⚠️ **`SAAN_CHUNK` を 8 → 4 にすると arena は ~12 KB 減る。** 止めているのは
`SAAN_CHUNK` そのものではなく **`saan_t4_istft_fits_in_w_e`**
（`typedef char …[(E * CH >= ISTFT_END) ? 1 : -1];` = iSTFT の `re`/`im`/`frm` 2,056 float が
`w_e` = `E`×`CH` に収まること）で、**これは iSTFT の作業領域を別に取れば外せる**
（`w_e` 9,728 → 4,864 / iSTFT 8,224 を別確保 = +3,360 B。差し引き ~−12 KB）。
❌ **やらない。** `CH` は **S5b の weight-stationary カーネルが重み行を使い回す列数**そのもので、
半分にすると**重みの読み直しが 2 倍**になる（S5b はまさにそれを減らして MAC の 95.1% を
覆った = [M-90](#m-90)）。**step 数も 2 倍になる。** ⚠️ **速度代償は実機でしか測れない。**
⚠️ **「静的検査が止めるから無理」と書きかけたが、それは理由として間違い**だった —
検査が言っているのは「`w_e` の共用が破れる」だけで、**下げられないのは速度のため**である。
...
## M-147. ⭐ **MEM-7 / MEM-8 を実機で測った** — **PCM は 53 ids でも 303 ids でも変更前と bit 一致**。静的 DIRAM **232,015 → 211,535 B**。⚠️ **要件 xRT ≤ 0.5 は 250 ids あたりで破れる（変更前からそう）**。⚠️ **`g_ids` の削減は受け付ける入力を狭めていたので戻した**（[C-108](decisions.md#c-108)）（自己実測 / M5 CoreS3・漢字構成）

実機が USB に戻ったので [M-144](#m-144) / [M-145](#m-145) を測った。**測れたのは出荷と同じ漢字構成**である。

### 0. まず辞書を実機から回収した（これが無いと何も測れなかった）

`csrc/k1_dict.bin` は生成物で、再生成は第三者コーパス（`data/splits/corpus_train.tsv`）を
要求する。**焼いてある板から読み戻した**:

```bash
esptool.py --chip esp32s3 --port /dev/cu.usbmodem2101 --baud 921600 \
    --before default_reset --after no_reset read_flash 0x8000 0xc00 /tmp/pt.bin
gen_esp32part.py /tmp/pt.bin        # → dict,data,65,0x2d0000,13504K
esptool.py ... read_flash 0x2d0000 13702320 csrc/k1_dict.bin
```

⚠️ **読み戻しが正しいかを自分で焼き直して確かめてはいけない**（自分の SHA を焼くので
一致して当たり前になる）。**今 載っている app に焼かれた値と比べた** — その app は
**元の辞書から**ビルドされている:

```bash
esptool.py ... read_flash 0x10000 0x2c0000 /tmp/app_orig.bin
strings -a /tmp/app_orig.bin | grep -oE "^[0-9a-f]{64}$"
shasum -a 256 csrc/k1_dict.bin
```

```
f162c922074d76817298b34d8a8fd35f7d195f38540303485a76c956b5d84877   ← app に焼かれた値
f162c922074d76817298b34d8a8fd35f7d195f38540303485a76c956b5d84877   csrc/k1_dict.bin
```

**一致。** 自己整合も「矛盾なし」（`check_dict_blob.py`）、中身は
**entries 438,750 / 見出し語 355,768 / 行列 1377×1377（生 int16）** = D-044 の動作点そのもの。
⚠️ **3 つめの裏づけ**: 焼き直した後、実機の G34（`saan_dict.c`）が起動時に
`辞書の SHA-256 一致（13702320 B / 471 ms）: f162c922074d7681…` を出した。

### 1. ⚠️ **回帰を 1 つ見つけて戻した**（[C-108](decisions.md#c-108)）

[M-145](#m-145) で `SAAN_G2P_IDS_CAP` を 1,027 → **`SAAN_MAX_IDS + 1` = 351** に詰め、
D-068 に「**失敗の種類が変わるだけ**」と書いた。**間違いだった。**

`speak_line` の事前検査は **G2P を走らせる前**に入力バイト数だけで弾く:

```c
if (saan_g2p_capacity(nbytes) > SAAN_G2P_IDS_CAP)   /* = 2·nbytes + 3 */
```

上限 351 だと **`nbytes > 174` の行が全部拒否**される。実機で:

```
E (20276) saanotts: 入力 264 B は長すぎる（ids バッファ 351 分）
```

⚠️ **その 264 B の行の実 ids は 303**（上限 350 の内側）。**通るべき入力が通らなくなっていた。**
**戻した**（+2,704 B）。⚠️ **これは短い文だけ試していたら永久に見つからない。**

### 2. ⭐ PCM は変更前と bit 一致（**同じ実機で、同じ文を、両方のファームで**）

変更前（`79dd7f2`）のファームを worktree からビルドして焼き、同じ行を打った。

| 入力 | sample | 変更前 | **MEM-7 + MEM-8** |
|---|---:|---|---|
| 53 ids（錨の 1 文） | 27,136 | `0x390bf4b2aef8f2ec` | **`0x390bf4b2aef8f2ec`** |
| **303 ids**（264 B） | 137,216 | `0xf1668c69d55406bf` | **`0xf1668c69d55406bf`** |

⚠️ **303 ids は MEM-7 の窓を 3 枚またぐ**（`SAAN_DUR_K` = 128）。**窓の継ぎ目が波形に出ないことを
実機で示したのはこれが初めて。** かな経路でも漢字経路でも同じ値が出る（`0x390bf…`）。

### 3. 静的 DIRAM と起動時の空き

| | M-142（arena 151,552） | **M-147** | 差 |
|---|---:|---:|---:|
| **静的 DIRAM** | 232,015（67.89%） | **211,535（61.9%）** | **−20,480** |
| 起動直後の内部 DRAM free | 160,639 | **181,119** | **+20,480** |
| 最大ブロック | 114,688 | **131,072** | **+16,384** |
| arena | 151,552 | 139,264 | −12,288 |
| `g_chunk` | 8,192 | **0**（MEM-8） | −8,192 |
| `g_ids` | 4,108 | 4,108（C-108 で戻した） | 0 |

起動時チェックの実機出力（**ターゲットの `arena_peak(350)` = 115,440 B** は
[M-145](#m-145) §6 の算術と完全一致）:

```
I (1044) saanotts: arena 139264 B を .bss に静的確保 (0x3fc98530) / G2P の ids 4108 B
I (1044) saanotts: arena は最長入力（350 ids）に足りる: 要 115440 B / 確保 139264 B（余り 23824 B）
```

**[C-101](decisions.md#c-101) の手当（Viterbi の起動時チェック）も動いた**:

```
I (1044) saanotts: 漢字 Viterbi: 66560 B 渡る / 鍵 1024 B に要 65600 B（余り 960 B・cap 2080 node）
```

cap 2,080 ≥ 2,050 なので **`KEY_MAX` の鍵を全部受けられる = 受け付ける入力は変わっていない。**

### 4. ⭐ `saan_stream_arena_peak(n)` が実測と **6 点すべてで完全一致**（[C-102](decisions.md#c-102) の確認）

発話後の高水位を出すようにしたので、実機で直接突き合わせられる:

| n_ids | 実測 `a.peak` | `arena_peak(n)` の予測 | init ms |
|---:|---:|---:|---:|
| 53 | 113,072 | **113,072** | 25.25 |
| 103 | 113,456 | **113,456** | 33.71 |
| 153 | 113,872 | **113,872** | 47.26 |
| 203 | 114,256 | **114,256** | 56.01 |
| 253 | 114,672 | **114,672** | 64.47 |
| 303 | 115,056 | **115,056** | 78.02 |

⚠️ **変更前の同じ 303 ids は `peak 129,696`** だった（init 時点の値。duration の一時領域込み）
→ MEM-7 で **−14,640 B**。

### 5. 速度 — **MEM-7 の代償は init に +13.27 ms（303 ids）。定常はむしろ速い**

| 303 ids | 変更前 | **MEM-7 + MEM-8** |
|---|---:|---:|
| init | 64.75 ms | **78.02 ms**（**+13.27 / +20.5%**） |
| 定常 xRT（中央値） | 0.526 | **0.523** |
| 定常 xRT（平均） | 0.502 | **0.499** |
| アンダーラン | 0 / 67 | **0 / 67** |
| 鳴らし始めまで | 370 ms | **369 ms** |

**init が遅くなったのが MEM-7 の再計算**（窓のハロー 12 を重ねて計算する分）。
⚠️ **鳴らし始めは変わらない** — そこは 4 チャンクのプリロール（222 ms の初回 pull）が律速で、
init は全体の 2 割しかない。**定常はむしろ速い**（MEM-8 が pull ごとの 8,192 B の memcpy を消した）。

### 6. ⚠️⚠️ **要件 xRT ≤ 0.5 は 250 ids あたりで破れる。変更前からそうだった**

**誰も 224 ids より長い文を実機で流していなかった**（[M-142](#m-142) §7 に書いたとおり）。
流したら**要件を超えた**:

| n_ids | 定常 xRT（中央値） | 平均 | アンダーラン |
|---:|---:|---:|---|
| 53 | 0.473 | 0.487 | 0 / 14 |
| 103 | 0.474 | 0.491 | 0 / 24 |
| 153 | 0.474 | 0.497 | 0 / 35 |
| 203 | **0.498** | 0.498 | 0 / 46 |
| 253 | **0.522** ❌ | 0.499 | 0 / 57 |
| 303 | **0.523** ❌ | 0.499 | 0 / 67 |

⚠️ **MEM-7 のせいではない** — 変更前のファームは同じ 303 ids で **0.526**（こちらの方が遅い）。

**中身を分解した。** 満チャンク pull の時間は**二峰分布**で、**モードの値は長さによらず一定**:

| n_ids | 低モード | 高モード | **高モード率** | 中央値 | 平均 |
|---:|---|---|---:|---:|---:|
| 53 | 8 本 / 43.55 ms | 4 本 / 48.50 ms | 33% | 43.99 | 45.20 |
| 103 | 13 本 / 43.30 | 10 本 / 48.59 | 43% | 44.02 | 45.60 |
| 153 | 17 本 / 43.89 | 16 本 / 48.61 | 48% | 44.05 | 46.18 |
| 203 | 22 本 / 43.93 | 22 本 / 48.63 | **50%** | **48.50** | 46.28 |
| 253 | 26 本 / 43.89 | 29 本 / 48.63 | 53% | 48.52 | 46.39 |
| 303 | 31 本 / 43.73 | 35 本 / 48.65 | 53% | 48.56 | 46.34 |

**1 チャンクの計算量は長さで増えていない**（2 つのモードは 43.8 ms と 48.6 ms で一定）。
増えるのは**高モードの割合**で、**50% を越えた瞬間に中央値が下のモードから上のモードへ跳ぶ**。
**平均は 45.20 → 46.34 ms（+2.5%）しか動かず、0.5 を一度も越えない。**

⚠️ **つまり「定常 xRT = 中央値」という統計が、二峰分布の 50% 境界で跳んでいる。**
実際の仕事は 2.5% しか増えていない。⚠️ **それでも要件は中央値で書いてある**（[D-049](decisions.md#d-049)）ので、
**文字どおりには 250 ids 以上で破れている。** ⚠️ **アンダーランは全長で 0** で、
**音が途切れたわけではない。**

⚠️ **二峰の原因は特定していない。** 候補は token パイプの進行（`tok_pipe_advance` が
チャンクによって走ったり走らなかったりする）だが、**測っていない**
（`-DSAAN_PROFILE=1` で焼き直せば分かる）。

### 7. ⚠️ 測っていないもの

- ⚠️ **350 ids ちょうどはまだ流していない**（303 が最長。308 B の行は **353 ids** になり
  設計上限で正しく拒否された = `353 ids は上限 350 を超える`）
- ⚠️ **音は聴いていない**（PCM は bit 一致なので v1.1.0 と同じ音のはず、だが聴取は別）
- ⚠️ **二峰の原因**（§6）
- ⚠️ **かな構成（`SAAN_KANJI=0`）は焼いていない**。測ったのは漢字構成だけ
- ⚠️ **Arduino ライブラリは実機で走らせていない**（残タスク #18）

---

## M-148. **#21 の原因を確定した** — 高い方のチャンクは **`tok_pipe_advance` が走った回**（+4.6〜4.9 ms）。⚠️ **中央値が跳ぶのは統計の性質で、仕事は 2.5% しか増えていない**。⚠️ **均しても 0.499 が天井**（自己実測 / M5 CoreS3・漢字構成）

[M-147](#m-147) §6 で「満チャンク pull が二峰（43.8 / 48.6 ms）で、長いほど高モード比が上がる」
ところまでは分かったが、**原因は特定していなかった**。確定させた。

### 1. 仮説: 高モード = token パイプが進んだチャンク

`make_hf_body` は、そのチャンクが必要とするトークンが出そろうまで進める:

```c
while (im->tok_pushed * TOK_K - TOK_HALO <= i1) SAAN_TRY(tok_pipe_advance(st));
```

`TOK_K` = `SAAN_CHUNK` = 8 / `TOK_HALO` = 12。**進む回はその分だけ余計に時間がかかる**はず。

### 2. ⭐ 裏づけ 1 — コードから H の個数を**予測**して当てた

`i1` の最大は `n_ids − 1` なので **全 advance 回数 = ⌊(n_ids − 1 + 12)/8⌋ + 1**。
初回 pull は `ofill ≥ CH` まで回る（遅延 38 フレーム ≒ 6 step）ので、そこで数回消費する。

```bash
uv run --no-project python - <<'PY'   # reports/m147_device/ の H/L 列から数える
PY
```

| n_ids | 全 advance | 初回 pull で | **予測 H** | **実測 H** | |
|---:|---:|---:|---:|---:|---|
| 53 | 9 | 5 | **4** | **4** | ✅ |
| 303 | 40 | 5 | **35** | **35** | ✅ |
...
303 ids の並び（H = 48.6 ms / L = 43.8 ms、初回 pull を除く 66 回）:

```
HHLHLHLHLHHLHHLHLHLH LHHLHLHLHHLHHLHLHLHL HHLHHLHLHLHLHHLHHLLH LLLLLL
```

**末尾 6 回が全部 L。** 全トークンを押し終えた後は advance が起きないので、当然そうなる。
lag-2 の一致率 0.66 = **ほぼ交互**。

### 4. ⭐ 裏づけ 3 — プロファイラの TOKEN 段（`-DSAAN_PROFILE=1`）
...
cd esp32/boards/m5unified && idf.py -B <build> \
    -DSDKCONFIG_DEFAULTS="sdkconfig.defaults;sdkconfig.cores3" \
    -DSAAN_KANJI=1 -DSAAN_PROFILE=1 -DSAAN_DICT_BLOB=<abs>/csrc/k1_dict.bin build flash
# reports/m147_device/dev_prof.log
```

```
n_ids  53: step_chunk 18 回 / TOKEN 0.50 回/step  → advance 合計  9 回（予測 9）
n_ids 303: step_chunk 72 回 / TOKEN 0.56 回/step  → advance 合計 40 回（予測 40）
```

**1 回のコスト**: 53 ids で 1,101,368 cyc = **4.59 ms** / 303 ids で 1,121,132 cyc = **4.67 ms**。
⚠️ プロファイラ無しのビルドで実測した **H − L = 48.65 − 43.73 = 4.92 ms** と整合
（プロファイラは細かい区間を過大に出すので他の段も膨らむ）。

### 5. ⚠️ **だから何が言えるか** — 3 つとも重要

1. **高モード率は「文の長さ」ではなく「1 トークンあたりのフレーム数」で決まる。**
   観測できる比 = (全 advance − 初回 pull の分) / (チャンク数 − 1) ≈ **(n_ids + 12) / n_frames**。
   ⚠️ **密な発話（フレーム/トークンが小さい）なら短い文でも高モードが増える。**
   M-147 の試験文は繰り返すほど密になった（2.00 → 1.77 フレーム/トークン）だけである。
2. **中央値が跳ぶのは統計の性質。** モードの値は長さによらず一定で、
   **平均は 45.20 → 46.34 ms（+2.5%）しか動かない。** 高モード率が 50% を越えた瞬間に
   中央値が下のモードから上のモードへ移るだけである。
3. ⚠️⚠️ **均しても要件に余裕は生まれない。** `TOKEN` は **STEP の 5.2〜5.6%** しかなく、
   **平均 xRT は 0.499**。advance をチャンクに分散すれば中央値は平均に寄って
   **0.499 ≤ 0.5 で「満たす」ことにはなるが、余裕は 0.2%** である。
   **本当に効くのは token ブロックそのものを速くすること**で、
   仮に TOKEN が 0 になっても **0.499 × (1 − 0.056) = 0.471**。

### 6. ⚠️ 測っていないもの

- ⚠️ **token ブロックが最適化の余地を持つかは見ていない。** 1 トークン 137,670 cyc で、
  素の MAC 数（3 ブロック × 2 conv × 48×48×5 = 69,120 MAC）から M-89 の 2.8 cyc/MAC で
  見積もると 193K cyc なので、**すでに PIE が効いている側**である。**削れる根拠は無い。**
- ⚠️ **advance を分散する案（`TOK_K` を 8 から下げる）は試していない。**
  `TOK_K` は `pipe_push` が CH 単位なので **CH に固定**されており、
