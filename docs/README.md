...
| 2 | [`measurements.md`](measurements.md) | **実測値の一次ソース** M-1〜M-114（⚠️ **M-97〜M-108 は未マージの別ブランチが使用中**）。全数値に再現コマンド付き | 実測のたび |
...
| **10** | **アクセント指標の教師ゲートが生徒依存**（[C-075](decisions.md#c-075)）。どう直すのが正しいか未決 | 両方 | 判断 | 当面は「共通ペアで測り直す」運用 |
...
│   ├── measurements.md                    実測値の一次ソース M-1〜M-114
