...
| 1 | [`decisions.md`](decisions.md) | 意思決定の記録 D-001〜D-053（⚠️ **D-049 は欠番** = RTF の分母用に予約）と**訂正履歴 C-001〜C-072** | 決定のたび |
| 2 | [`measurements.md`](measurements.md) | **実測値の一次ソース** M-1〜M-112。全数値に再現コマンド付き | 実測のたび |
...
│   ├── decisions.md                       決定記録 D-001〜D-053（D-049 は欠番）+ 訂正履歴 C-001〜C-072
