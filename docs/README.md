...
| 1 | [`decisions.md`](decisions.md) | 意思決定の記録 D-001〜D-053（⚠️ **D-049 は欠番** = RTF の分母用に予約 / ⚠️ **D-051・D-052 は当面の欠番** = PR #14 が使用中で、実機確認待ちのためマージしていない）と**訂正履歴 C-001〜C-058** | 決定のたび |
| 2 | [`measurements.md`](measurements.md) | **実測値の一次ソース** M-1〜M-105（⚠️ **M-97〜M-104 は当面の欠番** = PR #14 が使用中で、実機確認待ちのためマージしていない）。全数値に再現コマンド付き | 実測のたび |
...
│   ├── decisions.md                       決定記録 D-001〜D-053（D-049 は欠番 / D-051・D-052 は PR #14 が使用中の当面の欠番）+ 訂正履歴 C-001〜C-058
│   ├── measurements.md                    実測値の一次ソース M-1〜M-105（M-97〜M-104 は PR #14 が使用中の当面の欠番）
