...
[M-147](measurements.md#m-147)）。**RAM を実機で詰めた** — 静的 DIRAM **232,015 → 211,535 B（−8.8%）**、
...
| 1 | [`decisions.md`](decisions.md) | 意思決定の記録 D-001〜D-068（✅ **D-049 の欠番は 2026-09-11 に埋めた** = RTF の分母）と**訂正履歴 C-001〜C-109** | 決定のたび |
...
[完了] **v1.2.0 リリース**    ✅ **2026-09-19 に出した**（資産 30 本のうち **14 本を差し替え**。[M-149](measurements.md#m-149)）。
                            **RAM を実機で 20,480 B 減らした**（MEM-7 / MEM-8）。**音は v1.1.0 と bit 一致**（53 / 303 ids）。
...
| ~~20~~ | ~~⭐ MEM-7 / MEM-8 の実機確認~~ → ✅ **測った**（2026-09-18。[M-147](measurements.md#m-147)）。**PCM は 53 ids でも 303 ids でも変更前と bit 一致**（303 ids は MEM-7 の窓を 3 枚またぐ）。静的 DIRAM **232,015 → 211,535 B**、起動直後の空き **181,119 B**。`arena_peak(n)` が実測と **6 点すべて一致**（C-102 の確認）。⚠️ **回帰を 1 つ見つけて戻した**（[C-108](decisions.md#c-108)）。⚠️ **要件超過を発見** → #21 | 両方 | ✅ | [`reports/m147_device/`](../reports/m147_device/) |
...
1. **音として通用するか** — ⚠️ **誰も聴いていない**（G32）。SCOREQ も DNSMOS も
   **誤読を罰せず**（M-78）、DNSMOS は**アクセント誤りに無反応**（M-50）。
   ここだけは指標では埋まらない
2. ~~ESP32 で実時間に間に合うか~~ — ✅ **満チャンク 1 pull の xRT 0.474**（[M-142](measurements.md#m-142)。CoreS3 / W8A8+PIE。⚠️ M-90 の 0.446 / v1.1.0 の 0.448 から RAM を詰めた分だけ遅い）。
...
├── docs/
│   ├── README.md                          このファイル
│   ├── decisions.md                       決定記録 D-001〜D-068（**D-049 も埋まった**）+ 訂正履歴 C-001〜C-109
