...
| 2 | [`measurements.md`](measurements.md) | **実測値の一次ソース** M-1〜M-131。全数値に再現コマンド付き | 実測のたび |
...
## 残っているタスク（**2026-09-11 更新。この表は 4 件**: 聴取 1 / リリース 1 / CI のタグ 1 / 辞書の検査 1。⚠️ **プロジェクト全体では 5 件** — 項番 11 は [`../CLAUDE.md`](../CLAUDE.md) にしかない）
...
| **17** | **辞書の SHA-256 検査を入れる**（[D-063](decisions.md#d-063)）。不一致なら**かな専用で続く**。⚠️ **陽性対照が要る** | **K** | 作業（12 の後） | 起動が 1.05 → 1.5 s（[M-131](measurements.md#m-131)） |
...
│   ├── measurements.md                    実測値の一次ソース M-1〜M-131
