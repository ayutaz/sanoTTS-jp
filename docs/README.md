...
| 2 | [`measurements.md`](measurements.md) | **実測値の一次ソース** M-1〜M-77。全数値に再現コマンド付き | 実測のたび |
...
| # | 何 | トラック | ゲート |
|---|---|---|---|
| **1** | **実機でのサイクル実測** | 両方 | G28 / G29（K）+ D-3d（かな） |
| **2** | **聴取** | 両方 | G32 |
...
[完了] K-7 ESP32 / QEMU     **漢字文から合成まで完走**（M-77）。app 359,584 B / 2 MB、
...
│   ├── measurements.md                    実測値の一次ソース M-1〜M-77
