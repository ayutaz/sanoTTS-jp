...
⚠️ **2026-09-11 追記: v4 がある**（JSUT を外した重み。D-057。**次のタグ v1.0.0 で配る**）。
**v4 は SCOREQ 1.2552 / 教師比 0.6361**（M-116。v3 との対応差 **−0.0164**
[−0.0671, +0.0361] = **検出できない**）。
⚠️ **v4 の DNSMOS は測っていない。** 「v4 の DNSMOS」を書こうとしたら**まず測ること。**
⚠️ **どの版の値かを必ず添える。** この表には v2 / v3 / v4 が混在しうる。
...
⚠️ **v4 は 0.1670（差 +0.0319）= v3 と実質同一**（M-116）。
