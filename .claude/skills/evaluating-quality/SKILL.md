...
| 状態 | 内容 |
|---|---|
| DNSMOS | ⚠️ **未測定**（追試 E-1、`docs/plan/phase0-1-implementation-plan.md` §10） |
| 測る手段 | `speechmos` (PyPI 0.0.1.1) にある。⚠️ `uv add` で入れる |

**測るときに必ず守ること**:

- **まず実人間の日本語音声で天井を測る。** DNSMOS も日本語で較正されていない。
  UTMOS で天井を測らずに絶対値を英語と比べて判断を誤った前例がある（C-012）
- **教師比と人間比の両方**で報告する（D-013）
- **SCOREQ / UTMOS と同じ発話集合**で測って 3 指標を並べる
- DNSMOS も内部で **16 kHz** にリサンプルする可能性が高い。
  そうなら**平坦度プローブ（2–8 kHz）の代わりにはならない**。両方が要る
