...
→ **2026-09-02 に実機で測った**（M-82 以降）: M5Stack CoreS3 の W8A8+PIE で
満チャンク 1 pull の xRT **0.446**（M-90）、W8A32 は 4.28〜4.62（M-83）。
⚠️ **QEMU の命令数も、ホストのマイクロベンチも、実機の速度を予測しない**（C-055）。
速度の主張は `idf.py -DSAAN_PROFILE=1` の実機の表だけを根拠にすること。
