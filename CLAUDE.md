...
| **判断** | 8 | 辞書 blob を端末で検査するか | ✅ **コストは測った**（SHA-256 **471 ms** = いちばん速い。M-131）。**残るのは「入れるか」と「失敗したら止めるか」の判断だけ** |
...
| **8** | **辞書 blob の完全性を端末で検査するか**（M-100 §8 の 1・2。いま端末は blob 長も SHA-256 も見ていない） | 判断 | ✅ **コストを測った**（[M-131](docs/measurements.md#m-131)）: ⚠️ **SHA-256 がいちばん速い 471 ms**（CRC32 680 / 読むだけ 802）。**起動時の 1 回だけ**で **D-049 の ③ には乗らない**（起動が約 1.05 → 1.5 s）。✅ **端末の辞書が無傷であることも実証**（digest が凍結ファイルと一致）。⚠️ **決めるのは「入れるか」と「失敗したらどうするか」** |
