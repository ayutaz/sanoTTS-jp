...
|---|---:|---:|---:|
| SCOREQ synthetic/nr | **1.3049** | **0.6613** | 0.5427 |
| **DNSMOS OVRL** | **2.2892** | **0.8385** | — |
| **かな CER** | **0.1386** | 教師 0.1351（**差 +0.0034 = 区別できない**） | — |
| アクセント符号一致 | 37/37 = 1.000 | — | — |

**2026-08-29 に成果物を v2 → v3 に差し替えた**（M-59 / D-037）。
**Stage 3 の step 数を 40k → 80k にしただけで 5 指標すべてが改善**した
（旧 v2: SCOREQ 比 0.6113 / DNSMOS 比 0.7725 / かな CER 0.1776）。
...
1. ~~**【次】PIE カーネル。**~~ ✅ **実装完了**（M-57）。
   ...
   **QEMU が PIE を実装しているので、実機なしで正しさを検証できる**（M-56）。
   ...
   自動ベクトル化は期待できない（M-53）。
   ⚠️ **W8A8 は精度を約 2.3 dB 落とす**（`decoder.inp` 単層。end-to-end は未測定）。
...
**残りは 3 本**（同 §10）: **P-1** PIE カーネル（toolchain 待ち）/ **P-2** β の聴取（人が要る）/
**E-2 の残り** = `Gγ` が容量律速かの幅スイープ（約 2 時間の学習）。
**E-1 は決着した**（M-50 / D-034）。
