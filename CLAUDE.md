...
✅ **ただし「8 MB は無理」ではない**（2026-09-03 に測った。M-97 / **M-99**）。枠は 7,143,424 B で、
**エントリ数を落とせば入る。代償は読みの精度**（⚠️ **数値は n=1,495**。M-99 §4）:

| 案 | 新規に書く C | entries | イメージ | 音素の誤り |
|---|---|---:|---:|---:|
| **現行 16 MB** | — | 438,750 | 13,702,320 | **0.63%** |
| **① 行列 affine uint8 ← ✅ 実装した** | **書いた** | **228,000** | **7,123,088**（実ビルド） | **1.01%** |
...
✅ **① は書いて QEMU で通した**（M-104 / C-065。2026-09-04）。⚠️ **②〜④ の C リーダは無い**（サイズは形式からの算術）。

```bash
# 8 MB の辞書（接続行列を行ごとアフィン uint8 = セクション `matrixa` に）
...
⚠️ **速度を 1 つも測っていない** — アフィンの逆量子化が Viterbi を何倍遅くするかは未測定。
⚠️ **実機に焼いていない**（QEMU まで）。**音も聴いていない。**
