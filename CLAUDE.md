...
**どれも実機で動き、速度の要件も満たした。**
⚠️ **残りは 3 件で、内訳は「聴取 1 / 実機 1 / 作業 1」**（下の「いま止まっているもの」）。
✅ **判断は全部片づき、v1.0.0 も出した**（2026-09-12。資産 28 本 / 140 MB）。
**実装として書くものは残っていない。**
...
| テスト | `scripts/make_release_body.py --self-test` | **リリースページ用の本文**を 1 つの md から作る（相対リンク → 絶対 URL / **`<!-- release-body: 内部の手順 -->` の区間を外す**）。⚠️ **内部の手順が公開ページに出かけた** — 本文 501 行のうち **81 行**が「タグの打ち方」と「タグを打った後にやること」だった。⚠️ **印は対で要る**（片方だけだと**黙って何も外れない**）ので、開きと閉じの数が合わなければ落とす。**陽性対照 5 件** + CI が**実物のノート 6 本**を変換して内部の手順が残っていないことを確かめる |
...
### いま止まっているもの（**3 件**。⚠️ **依存はもう無い** — どれから着手してもよい）

| 誰が | # | 何 | 止まっている理由 |
|---|---|---|---|
| **人** | **1** | **対照つきの聴取**（G32） | ⚠️ **私は音を聞けない。** ボードは要らない（`saanotts-jp-v4-samples.zip` を再生するだけ） |
| **人** | 11 | 4 MB / 2 MB の**実機の数字** | 板が無い。第三者の報告に checksum / xRT / UR が無い |
| **作業** | **17** | **辞書の SHA-256 検査を入れる**（D-063） | ✅ **もう着手できる**（v1.0.0 を出したので）。⚠️ **陽性対照が要る** |

✅ **12（リリース）と 16（CI / Pages）は 2026-09-12 に済んだ**（[M-133](docs/measurements.md#m-133)）。
**残る 3 件に順序の依存は無い。**
...
| **17** | **辞書の SHA-256 検査を入れる**（[D-063](docs/decisions.md#d-063)）。ビルド時に `file(SHA256 ...)` で digest を焼き、起動時に `d->blob_len` バイトと比べる。不一致なら `saan_dict_open()` が `false` → **かな専用で続く**（B2 = 既存の挙動）。⚠️ **陽性対照が要る**（1 バイト書き換えた辞書で漢字経路が無効になること）。⚠️ **`main/CMakeLists.txt` は 2 つある** / **`REQUIRES` を `if()` にすると届かない** | 作業（**12 の後**） | ⚠️ **いま入れると staged の資産とソースが対応しなくなる**。起動が 1.05 → 1.5 s（M-131）。⚠️ 小容量版のコストは未測定 |

<details>
<summary><b>決着したもの 13 件</b>（⚠️ <b>番号は付け替えない</b> — 他のファイルからの参照が黙って壊れる。C-064 / C-078 / C-085）</summary>

| # | 何 | 種類 | ゲート |
|---|---|---|---|
| ~~12~~ | ~~v1.0.0 をリリースする~~ → ✅ **出した**（2026-09-12。[M-133](docs/measurements.md#m-133)）。資産 **28 本 / 140,291,218 B**。**上げる前に表と照合 27/27** → **上げた後に落とし直して 27/27** → **名前の実在 22/22（`v1.0.0` と `latest` の両方）**。⚠️ **残り 9 本の firmware は焼いていない**（中身は M-126 / M-128 で照合済み）。⚠️ **音は誰も 1 秒も聴いていない** | ✅ | — |
| ~~16~~ | ~~CI / Pages が `v0.3.0`（= v3）のタグ固定~~ → ✅ **向け直した**（[M-133](docs/measurements.md#m-133) §3）。`ci.yml` の 2 job / `pages.yml` の `RELEASE_TAG` / **寸法のアサート 12 行** / `check_web_gates.sh` の案内文 / **読者向け 58 行 / 9 ファイル**。✅ **v4 で初めて 2 本の golden の大きさが分かれた**（fp32 786,912 / int8 794,240。**v3 はどちらも 779,584 でアサートが取り違えを止められなかった**）。⚠️ **一括置換がまた歴史的記述を壊した**（C-064 の 4 度目。`2d2b8543` は v3 の値なのに v4 の主張に化けた） | ✅ | — |
...
| ~~8~~ | ~~辞書 blob の完全性を端末で検査するか~~ → ✅ **決めた**（[D-063](docs/decisions.md#d-063)。2026-09-11）: **firmware に焼いた SHA-256 で検査**（A2）/ **不一致なら漢字経路だけ無効化**（B2。**既に実装済みの挙動**）/ **v1.0.0 の後に入れる**（C2）。⚠️ **コストは実測 471 ms で SHA-256 がいちばん速い**（M-131）。⚠️ **実装は未着手** — 項番 17 | ✅ | — |
