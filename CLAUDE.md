...
<details>
<summary><b>決着したもの 14 件</b>（⚠️ <b>番号は付け替えない</b> — 他のファイルからの参照が黙って壊れる。C-064 / C-078 / C-085）</summary>

| # | 何 | 種類 | ゲート |
|---|---|---|---|
| ~~17~~ | ~~辞書の SHA-256 検査を入れる~~ → ✅ **入れた**（2026-09-12。[M-134](docs/measurements.md#m-134)）。ビルド時に `file(SHA256)` で焼き、起動時に `d->blob_len` バイトと照合。**不一致なら漢字経路だけ無効 → かな専用で続く**。ゲートは **G34** `bash scripts/check_dict_integrity.sh`（**1 ビット壊すと漢字経路だけが無効**）。コスト **+540 ms**（起動時の 1 回）/ app **+4,560 B**。⚠️ **予告していなかった 3 つ目の罠を踏んだ** — include が `#if SAAN_DICT_MMU` の中にあり、**DevKit ではコンパイルされない**（M-131 のプローブも同じ場所にあった）。⚠️ **v1.0.0 には入っていない**（次のリリースから） | ✅ | **G34** |
