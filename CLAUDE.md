...
| **10** | **小容量版を配布するか**（M-105 / M-106。⚠️ **読みが 0.63% → 1.01%（8 MB）/ 1.94%（4 MB）/ 3.86%（2 MB 枠）に落ちる**。板ごとに表と辞書が別なので本数が増える） | 判断 | 配る / 「ソースからどうぞ」のまま。⚠️ **辞書単体を並べると取り違えても止まらない**（残タスク 8 が未解決で、端末は blob の身元を見ていない）。⚠️ **4 MB / 2 MB は実機で 1 度も動かしていない** |
...
⚠️ **4 MB / 2 MB とも実機では測っていない**（4 MB のチップを持っていない。M-106 §12）。速度も音も未。
...
⚠️ **4 MB / 2 MB とも実機では測っていない**（速度も音も未。M-106 §12）。
