...
| **作業** | **17** | **辞書の SHA-256 検査を入れる**（D-063） | ⚠️ **12 の後**（いま入れると staged の資産とソースが対応しなくなる） |
| **作業** | 16 | CI / Pages を v1.0.0 に向ける | ⚠️ **12 の後でないとできない**（タグが無いと CI が落ちる） |

**依存は 12 → {16, 17}。** ⚠️ **1 と 11 は 12 と独立**（聴かずに出すこともできるし、その逆もできる）。
⚠️ **17 を 12 より先にやると、staged の資産とソースが対応しなくなる**（[M-128](docs/measurements.md#m-128) で 1 度閉じた穴を開け直す）。
...
<details>
<summary><b>決着したもの 11 件</b>（⚠️ <b>番号は付け替えない</b> — 他のファイルからの参照が黙って壊れる。C-064 / C-078 / C-085）</summary>
