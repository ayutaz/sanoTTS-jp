...
## 残っているタスク（2026-09-12 更新。**対照つきの聴取が 1 つ、判断が 4 つ、実機待ちが 1 つ**）

⚠️ **この件数は 2026-09-12 に数え直した。** 直前まで「判断が 4 つ」と書いてあったが実体は 3 で、[M-112](docs/measurements.md#m-112) の 12 を足して初めて 4 になった（C-042 と同じ形）。
...
| **12** | ⚠️ **[D-052](docs/decisions.md#d-052) の包絡 `ids ≤ 8×形態素 + 54` が破れる**（[M-112](docs/measurements.md#m-112) §4）。形態素上限 44 は「ids を縛れば OJ ヒープが縛れる」という論法だが、カタカナ複合語は **16 形態素で 453 ids**（包絡は 182）。**同じ 42 形態素の 2 文でホストのヒープ実測が 57,122 B と 99,146 B = 1.74 倍**で、**形態素数はヒープを縛れていない**。⚠️ 影響するのは **PSRAM 無しの板だけ**（ATOMS3 など。CoreS3 は PSRAM に落ちる）。⚠️ **実機でも QEMU でも測っていない** | 判断 | (a) ids で縛る（NJD の前に ids を見積もる） (b) 形態素上限を下げる (c) 実機で測ってから決める。⚠️ **held-out 2,328 文では包絡は成り立っている**（M-98）ので、実文で踏むかは未測定 |
