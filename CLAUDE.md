...
| **10** | **小容量版を配布するか**（M-105 / M-106。⚠️ **読みが 0.63% → 1.01%（8 MB）/ 1.64%（4 MB）/ 3.27%（2 MB 枠）に落ちる**。板ごとに表と辞書が別なので本数が増える） | 判断 | 配る / 「ソースからどうぞ」のまま。⚠️ **辞書単体を並べると取り違えても止まらない**（残タスク 8 が未解決で、端末は blob の身元を見ていない）。⚠️ **4 MB / 2 MB は実機で 1 度も動かしていない** |
...
大きい板の上で「枠に収まるか」を確かめただけ）: 44,000 entries / 977,456 B / **音素の誤り 3.27%**。
...
| **4 MB** | **3,006,656** | **135,000** | **1.64%** | ✅ QEMU（M-106 §11） |
| **2 MB 枠** | **977,456** | **44,000** | **3.27%** | ✅ QEMU（M-106 §13） |
