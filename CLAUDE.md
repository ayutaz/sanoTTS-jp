...
**ターゲット tier の議論は決着済み（567 K / ESP32）。** 残るのは次の 3 つ。

1. **【最優先・プロジェクトの成否を決める】オンデバイス日本語 G2P のフットプリント。**
   枝刈り辞書を実際にビルドし、**サイズ・未知語率・アクセント精度**を実測する。
   これが数 MB に収まらなければ、ESP32 で日本語を喋らせること自体が成立しない。
   蒸留の実装より先にこの spike をやること — 蒸留が全部うまくいっても G2P が載らなければ無駄になる。
   評価は「実文で何 % の文が正しく読めるか」で行う（辞書サイズ単体では判断できない）。
2. **567 K で日本語が実用に足るか。** 論文の英語実績は SCOREQ 2.54 / WER 14.8%。
   日本語 CER は未知。モーラ等時性は英語の強勢拍リズムより duration 予測が容易なはずで
   有利に働く可能性がある一方、摩擦音・無声化母音の多さは不利に働く。
   **1.4 M を先に作って上限を測り、567 K との差分で判断する。**
3. **教師音声の品質ベースラインが未確定** — 短尺 8 文の暫定計測で SCOREQ 2.06 / UTMOS 1.62 と、
