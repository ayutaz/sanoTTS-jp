...
| 7 | **辞書 blob の完全性を端末で検査するか**（M-100 §8 の 1・2。いま端末は blob 長も SHA-256 も見ていない） | 判断 | 版を上げる / firmware に SHA-256 を焼く / CRC。⚠️ **13.7 MB を起動時に舐めるコストが未測定** |
...
✅ **ただし「8 MB は無理」ではない**（2026-09-03 に測った。M-94 / **M-96**）。枠は 7,143,424 B で、
**エントリ数を落とせば入る。代償は読みの精度**（⚠️ **数値は n=1,495**。M-99 §4）:

| 案 | 新規に書く C | entries | イメージ | 音素の誤り |
|---|---|---:|---:|---:|
| **現行 16 MB** | — | 438,750 | 13,702,320 | **0.63%** |
| ① 行列 affine uint8 | リーダ 1 本 | 228,000 | 7,115,943 | **1.01%** |
...
⚠️ **「音素の 0.32%」は n=298 の値で、n=1,495 では 0.63%**（C-057）。**編集距離の比を書くときは n を必ず添える。**
...
⚠️ **C リーダは 1 本も書いていない。** サイズは形式からの算術。**方針は未決。**
