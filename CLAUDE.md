...
（**RAM を実機で 20,480 B 減らした**。音・重み・辞書は `v1.1.0` と bit 一致で、⚠️ **14 本を差し替え / 15 本は `v1.0.0` から SHA-256 が変わっていない** = [M-149](docs/measurements.md#m-149)）
...
**⚠️ 残っているのは 4 件で、どれも人が要る**（⭐ だった #20 = MEM-7 の実機確認は ✅ **2026-09-18 に測った** = [M-147](docs/measurements.md#m-147)）:
...
4. ⚠️⚠️ **定常 xRT の要件（≤ 0.5）が 250 ids あたりで破れている**（[M-147](docs/measurements.md#m-147)。253 ids で中央値 0.522）。⚠️ **v1.1.0 も同じ**なので新しい退行ではない。⚠️ **平均は越えず、アンダーランは 0**。
   ✅ **原因は確定した**（[M-148](docs/measurements.md#m-148)。高い方のチャンクは `tok_pipe_advance` が走った回）。⚠️ **均しても余裕は生まれない**（その処理は STEP の 5.2〜5.6% で、平均が既に 0.499）。**残るのは [D-049](docs/decisions.md#d-049) の統計量を変えるかどうかで、人の判断**（詳細は下の残タスク表 #21）
...
**どれも実機で動き、`v1.2.0` として配っている**（重み・辞書は `v1.0.0` と bit 同一）。⚠️ **速度の要件は 250 ids あたりで破れる**（[M-147](docs/measurements.md#m-147) / [M-148](docs/measurements.md#m-148)）。
⚠️ **G32 は閉じていない**（残タスク 1）。✅ **2026-09-12 に初めて対照つきで聴いた**（[M-135](docs/measurements.md#m-135)。held-out **18/24 文**を生徒 → 教師の順で。「問題なし」）。⚠️ **盲検でない / n=1 / 摩擦音とアクセントは個別に未確認**なので **G32 は閉じていない。**
...
✅ **分母は 2026-09-11 に決めた**（[D-049](docs/decisions.md#d-049)）— **定常の分母で ≤ 0.5** + **アンダーラン 0** + **鳴らし始めまで ≤ 0.8 s**（**v4 の実機で約 407〜433 ms** = 余裕 46〜49%。⚠️ 約 479 ms は **v3** の値）。⚠️ **発話全体は参考値**（≤ 0.5 に必要な音声長が 3.43〜4.40 秒 = 合否が文の長さで決まる）。⚠️ **G2P の「1% 未満」は廃止**（分母が動く）。
⚠️ **音は聴いていない**（G32）。
...
⚠️ **配布しているのは `runs/v4/stage4.pt`**（= リリース `v1.0.0` の
`saanotts-jp-v4-stage4.pt` と **SHA-256 が bit 一致**: `be5bf3f586cc6de7…`）。
**`runs/v3` は Stage 3 = 80,000 step の旧成果物**（D-037）で、
⚠️ **手元にはもう `.pt` が無い**（`log.jsonl` と `summary.json` だけ）。
**v3 で測り直すならリリース資産から落とすこと。**
`runs/v2` は M-49 など過去の測定の再現用。**混同しないこと。**
⚠️ **`runs/` は git 管理外**なので、どのゲートもこの食い違いを見ない。
...
# ★ G34: 辞書の SHA-256 検査（D-063 / M-134）。⚠️ **辞書の作り方か saan_dict.c の照合を
#   触ったら回すこと**。ビルド → QEMU 2 回（正しい辞書 / **1 ビット壊した辞書**）
...
`uv run python scripts/d4_accent_pairs.py --ckpt runs/v4/stage4.pt`（18 秒）。⚠️ **`runs/v3/stage4.pt` は手元に無い**（リリース資産から落とす）。
...
| テスト | `make -C csrc jdict-hard` | **辞書リーダの入力検査**（M-100）。⚠️ **返り値だけでは越境が見えない**ので **ASan ビルドを同居**させ、`JDICT_TEST_WEAK` で検査を外した**陽性対照が 27/27 leak** することを要求する（⚠️ **ケースを足すたびに増える**。M-100 の 19 → M-107 で 26 → M-108 で **27**。⚠️ **数え方で 33 とも 35 とも出るので、実行時の `通してしまった壊れ方: N / N` を正とする**）。`all-test` と CI に入っている |
...
## 残っているタスク（2026-09-19 更新。**残り 4 件。どれも人が要る** — ⚠️ **#21 は原因まで確定していて、打つ手が無いことも測ってある**）
...
blob / golden / **firmware 10 本**も作り、**実機（M5 CoreS3）で漢字を喋らせた**（M-119〜M-124。xRT 0.448 / アンダーラン 0 / 漢字==かな bit 一致）。⚠️ **聴取は 2026-09-12 が初めて**（[M-135](docs/measurements.md#m-135)。18/24 文・対照つき・**盲検でない / n=1**）。
✅ **`v1.0.0` として配った**（2026-09-12。資産 28 本。M-133）。
✅ **いま配っているのは `v1.2.0`**（2026-09-19。資産 30 本のうち **14 本を差し替え**。RAM を実機で 20,480 B 減らし、**PCM は `v1.1.0` と bit 一致**。[M-149](docs/measurements.md#m-149)）。
...
| 誰が | # | 何 | 止まっている理由 |
|---|---|---|---|
| **人 or 私** | **21** | ⚠️⚠️ **定常 xRT の要件（≤ 0.5）が 250 ids あたりで破れている**（[M-147](docs/measurements.md#m-147)） | **253 ids で中央値 0.522 / 303 ids で 0.523**。⚠️ **v1.1.0 も同じ**（0.526）なので新しい退行ではないが、[D-049](docs/decisions.md#d-049) の要件を文字どおりには満たしていない。⚠️ **平均は 0.499 で越えず、アンダーランは全長で 0**（音は途切れていない）。満チャンク pull が **43.8 / 48.6 ms の二峰**で、長いほど高モード比が上がり（33→53%）**50% で中央値だけ跳ぶ**。✅ **原因は確定**（[M-148](docs/measurements.md#m-148)）— 高い方は **`tok_pipe_advance` が走ったチャンク**（+4.6〜4.9 ms。3 通りで裏づけ）。⚠️ **高モード率は長さではなく「1 トークンあたりのフレーム数」で決まる**ので**密な発話なら短文でも起きる**。⚠️⚠️ **均しても余裕は生まれない**（TOKEN は STEP の 5.2〜5.6%・平均 xRT は 0.499）。打つ手: (a) token ブロックを速くする（⚠️ **既に PIE が効いており削れる根拠は無い**）/ (b) 要件の統計を中央値→平均に改める（**人が決める**）/ (c) 密な長文は分割を案内する |
...
⚠️ **決着した 15 件の表は [`docs/README.md`](docs/README.md#残っているタスク2026-09-12-更新残り-2-件どちらも人が要る) に集約した。**
**ここには置かない** — 2026-09-12 まで同じ表が両方にあり、**どちらも別々に欠けていた**
（こちらは項番 14 が、あちらは項番 11 と 6〜10 が無かった）。**同じ表を 2 か所に持たない。**
...
⚠️ **11 は板が要る**（4 MB / 2 MB は QEMU までしか行っていない。M-106 §12）。

### 2 MB / 4 MB flash と「ほぼ全てのマイコン」（M-106。2026-09-06）

⚠️ **ESP32-S3 に 2 MB flash の品番は無い**（WROOM-1 は N4 / N8 / N16）。**4 MB が下限。**

**予算を接続行列から語彙へ移すと、同じ約 966 KB で音素の誤りが半減する**（n=1,495 の実測）:

| 板 | 辞書の枠 | 行列 | entries | **音素の誤り** |
|---|---:|---|---:|---:|
| 2 MB（仮想。品番は無い） | 983,040 | affine | 21,000 | 6.75% |
| **2 MB（同枠の最善）** | 983,040 | **cluster:256** | **44,000** | **3.86%** |
| **4 MB / DevKit** | 3,080,192 | cluster:256 | **100,000** | **2.69%** |
...
**4 MB / DevKit は 135,000 entries（音素の誤り 1.94%）まで入る**（枠 3,014,656 B）。
...
**2 MB の枠でも通した**（M-106 §13。⚠️ **ESP32-S3 に 2 MB の品番は無い**ので、
大きい板の上で「枠に収まるか」を確かめただけ）: 44,000 entries / 977,456 B / **音素の誤り 3.86%**。
⚠️ **app の余りが 2% しかなく、M5Unified 版では成立しない。**
...
⚠️ **4 MB / 2 MB は第三者の実機で鳴った**（M-109。PSRAM 無しの ATOMS3 でも）。⚠️ **私は未再現で、checksum も xRT もアンダーランも報告に無い。**
⚠️ **`sdkconfig.kanji2mb` / `kanji4mb` の `CONFIG_ESPTOOLPY_FLASHSIZE` は
「実際に焼く板の容量」に書き換えること**（書き忘れるとブートループする。M-106 §13）。
⚠️ **`cluster:256` が最善なのはこの帯だけ。** [M-99](docs/measurements.md#m-99) §2 の
370,863 entries では affine の 5.3 倍文が壊れる。**動作点で最善手が入れ替わる。**

✅ **文脈 ID の詰め直しは完全に無損失で、C の変更が要らない**（`matrixa` は寸法をヘッダに持つ）:
16 MB で **−800,800 B** / 8 MB で **−583,984 B**。⚠️ **未実装。**

❌ **測って否定した 2 つ**: 「分解できる語を外す」（44,000 の枠内に 210 語しかない。
⚠️ **4 MB / 8 MB では効く可能性**）/「見出し語ごとに 1 エントリ」（文脈 ID が半減して**文一致が 631 → 176**）。

⚠️ **M5StampC5 は flash ではなく FPU と RAM で落ちる。** ESP32-C5 は `rv32imac`
（`-march` に `f` が無い = 浮動小数点ユニットが無い）で `saanotts.c` にソフト FP の呼び出しが
**106 か所**出る（陽性対照: FPU のある P4 では **0**）。RAM も**要求 370,980 B に対し SRAM 393,216 B**で
PSRAM が無い。**Stamp 型なら M5StampS3（ESP32-S3 / 8 MB）が今日そのまま動く。**

⚠️ **`/deep-research` を回したが、1 語 20.5 B を半減した先行事例は商用にも学術にも無かった**
（AqKanji2Koe-M は 17.6〜20.0 B/語でうちと同密度）。**報告の 3 点は再現できず訂正した**
（[C-067](docs/decisions.md#c-067) = dedup は動作点で符号が変わる /
[C-068](docs/decisions.md#c-068) = 母集団が 465 件ずれていた / うちの LOUDS は succinct trie の公表値より良い）。

⚠️ **これから板を買うなら N16R8。** 8 MB flash では**現行の辞書（13.7 MB）はそのままでは入らない**。
✅ **ただし 8 MB でも漢字は動く。実機で喋った**（M-104 = QEMU / **M-105 = 実機**。2026-09-05）。
✅ **4 MB と 2 MB 枠でも通した**（M-106 §11 / §13。**QEMU まで**）。
**エントリ数を落とせば入る。代償は読みの精度**（⚠️ **数値は n=1,495**。M-99 §4 / M-105 §4b / M-106 §14）:

| flash | 辞書 | entries | **音素の誤り** | 状態 |
|---|---:|---:|---:|---|
| 16 MB（出荷） | 13,702,320 | 438,750 | **0.63%** | ✅ 実機 |
| 8 MB | 7,123,088 | 228,000 | 1.01% | ✅ 実機 |
| **4 MB** | **3,006,656** | **135,000** | **1.94%** | ✅ **第三者の実機**（[M-109](docs/measurements.md#m-109)。⚠️ 未再現） |
| **2 MB 枠** | **977,456** | **44,000** | **3.86%** | ✅ **第三者の実機**（[M-109](docs/measurements.md#m-109)。⚠️ 未再現） |

⚠️ **4 MB が ESP32-S3 の下限**（WROOM-1 は N4 / N8 / N16。**2 MB の品番は無い**）。
2 MB の行は「**枠に収まる**」ことしか言っていない。
⚠️ **4 MB / 2 MB は第三者の実機で鳴った**（M-109）。⚠️ **私は未再現。速度は未測定。**
...
⚠️ ~~②〜④ の C リーダは今も無い~~ → ✅ **② `charr` と ③ `matrixc` は書いた**（M-106 §10）。**残るのは ④ レコード dedup だけ**で、しかも[C-067](docs/decisions.md#c-067)で**動作点によって符号が変わる**。
